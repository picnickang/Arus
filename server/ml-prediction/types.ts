...
export function structuredLog(data: {
  ...
  if (data.status === "error") {
    logger.error("MlPrediction", data.method, metadata);
  } else if (data.status === "warning") {
    logger.warn("MlPrediction", data.method, metadata);
  } else {
    logger.info("MlPrediction", data.method, metadata);
  }
}
...
export function sanitizeTelemetry(telemetry: EquipmentTelemetry[]): EquipmentTelemetry[] {
  ...
  if (invalidTimestampCount > 0 || invalidValueCount > 0) {
    structuredLog({
      method: "sanitizeTelemetry",
      status: "warning",
      details: {
        invalidTimestamps: invalidTimestampCount,
        invalidValues: invalidValueCount,
        totalInput: telemetry.length,
        totalOutput: sanitized.length,
      },
    });
  }
  return sanitized;
...
export function generateRecommendations(failureProbability: number, model: string): string[] {
  ...
  if (failureProbability > 0.7) {
    recommendations.push(
      `Critical: Schedule immediate inspection${suffix}`,
      "Prepare for possible equipment replacement"
    );
  } else if (failureProbability > 0.5) {
    recommendations.push(
      `Schedule maintenance within 7 days${suffix}`,
