...
interface OrtSessionLike {
  ...
  ): Promise<Record<string, { data: Float32Array | BigInt64Array }>>;
}
...
function loadOrt(): Promise<OrtModuleLike> {
  if (!ortModulePromise) {
    ortModulePromise = import("onnxruntime-node")
      .then(
        (m) =>
          (m as object as { default?: OrtModuleLike }).default ?? (m as object as OrtModuleLike)
      )
      .catch((err) => {
