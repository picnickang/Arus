...
describe("PdM Repository CRUD Tests", () => {
  ...
  beforeEach(() => {
    mockRepository = {
      getFleetHealthKpis: jest
        .fn<(orgId: string) => Promise<FleetHealthKpis>>()
        .mockResolvedValue(mockFleetHealthKpis),
      getRiskQueue: jest
        .fn<(orgId: string, status?: "new" | "active" | "resolved") => Promise<RiskQueueItem[]>>()
        .mockResolvedValue([createMockRiskQueueItem()]),
      getTelemetryCoverage: jest
        .fn<(orgId: string) => Promise<TelemetryCoverage>>()
        .mockResolvedValue(mockTelemetryCoverage),
      getModelHealth: jest
        .fn<(orgId: string) => Promise<ModelHealth>>()
        .mockResolvedValue(mockModelHealth),
      getMaintenancePipeline: jest
        .fn<(orgId: string) => Promise<MaintenancePipeline>>()
        .mockResolvedValue(mockMaintenancePipeline),
      getAssetDetail: jest
        .fn<(orgId: string, equipmentId: string) => Promise<AssetDetail | null>>()
        .mockResolvedValue(mockAssetDetail),
      acknowledgeRiskItem: jest
        .fn<(orgId: string, itemId: string, userId: string) => Promise<void>>()
        .mockResolvedValue(undefined),
      createWorkOrderFromRisk: jest
        .fn<(orgId: string, itemId: string, userId: string) => Promise<string>>()
  ...
  describe("READ Operations", () => {
    ...
    describe("getActiveAlerts", () => {
      ...
      it("should filter by equipment types", async () => {
        ...
        expect(mockRepository.getActiveAlerts).toHaveBeenCalledWith(TEST_ORG_ID, undefined, [
          "Engine",
        ]);
      });
  ...
  describe("CREATE Operations", () => {
    describe("createWorkOrderFromRisk", () => {
      ...
      it("should throw error for non-existent prediction", async () => {
        mockRepository.createWorkOrderFromRisk.mockRejectedValue(new Error("Prediction not found"));
      ...
      it("should return unique work order ID for each creation", async () => {
        ...
        const wo1 = await mockRepository.createWorkOrderFromRisk(
          TEST_ORG_ID,
          "risk-1",
          TEST_USER_ID
        );
        const wo2 = await mockRepository.createWorkOrderFromRisk(
          TEST_ORG_ID,
          "risk-2",
          TEST_USER_ID
        );
