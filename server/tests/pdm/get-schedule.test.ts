...
import { createGetScheduleUseCase, computeBufferDays, computeSchedulingWindow, determineBlockStatus } from '../../pdm/application/get-schedule.use-case';
...
describe('PdM Schedule Use Cases', () => {
  ...
  describe('greedy scheduler', () => {
    it('should sort scheduled tasks by preferred date', async () => {
      const alerts = [
        createMockAlert({ id: '1', rulEstimateDays: 20, rulConfidenceInterval: { lowDays: 15, highDays: 25 }, confidence: 80, vesselId: 'vessel-1' }),
        createMockAlert({ id: '2', rulEstimateDays: 5, rulConfidenceInterval: { lowDays: 3, highDays: 7 }, confidence: 80, vesselId: 'vessel-2' }),
        createMockAlert({ id: '3', rulEstimateDays: 10, rulConfidenceInterval: { lowDays: 7, highDays: 14 }, confidence: 80, vesselId: 'vessel-3' }),
      ];
      ...
      const preferredDates = result.data.scheduledTasks.map(t => t.schedulingWindow.preferredDate);
  ...
  describe('computeBufferDays invariants', () => {
    ...
    it('should never reduce buffer when confidence decreases', () => {
      const high = computeBufferDays({ confidence: 90, telemetryFreshness: 'online', severity: 'medium' });
      const medium = computeBufferDays({ confidence: 70, telemetryFreshness: 'online', severity: 'medium' });
      const low = computeBufferDays({ confidence: 40, telemetryFreshness: 'online', severity: 'medium' });
    ...
    it('should add buffer for delayed telemetry', () => {
      const online = computeBufferDays({ confidence: 80, telemetryFreshness: 'online', severity: 'medium' });
      const delayed = computeBufferDays({ confidence: 80, telemetryFreshness: 'delayed', severity: 'medium' });
      const offline = computeBufferDays({ confidence: 80, telemetryFreshness: 'offline', severity: 'medium' });
    ...
    it('should add buffer for critical severity', () => {
      const medium = computeBufferDays({ confidence: 80, telemetryFreshness: 'online', severity: 'medium' });
      const critical = computeBufferDays({ confidence: 80, telemetryFreshness: 'online', severity: 'critical' });
