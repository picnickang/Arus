...
import { TEST_ORG_ID, GOLDEN_SCENARIOS, MOCK_VESSELS, createMockRepository } from "./fixtures";
...
describe("PdM Schedule Golden Scenarios", () => {
  ...
  describe("Case A: Schedulable High Risk Task", () => {
    ...
    it("should compute correct scheduling window for schedulable task", async () => {
      ...
      const task = result.data.scheduledTasks[0];
      expect(task.schedulingWindow.earliestStart.getTime()).toBeLessThanOrEqual(
        task.schedulingWindow.preferredDate.getTime()
      );
      expect(task.schedulingWindow.preferredDate.getTime()).toBeLessThanOrEqual(
        task.schedulingWindow.latestFinish.getTime()
      );
    });
  });
