...
export function getCoreTablesSql(): SQL[] {
  return [
    ...
    sql`CREATE TABLE IF NOT EXISTS users (id TEXT PRIMARY KEY, org_id TEXT NOT NULL, email TEXT NOT NULL, username TEXT, name TEXT NOT NULL, password_hash TEXT, password_updated_at INTEGER, role TEXT NOT NULL DEFAULT 'viewer', job_title TEXT, phone TEXT, timezone TEXT DEFAULT 'UTC', is_active INTEGER DEFAULT 1, login_enabled INTEGER DEFAULT 1, must_change_password INTEGER DEFAULT 0, supervisor_user_id TEXT, hub_admin INTEGER DEFAULT 0, hub_access TEXT, last_login_at INTEGER, created_at INTEGER, updated_at INTEGER)`,
