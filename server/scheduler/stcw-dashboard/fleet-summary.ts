...
function buildVesselSummary(
  ...
  return {
    ...
    complianceRate:
      vesselCrewCount > 0 ? (vesselMetrics.compliantCrew / vesselCrewCount) * 100 : 100,
    ...
    avgRestPer24h:
      vesselMetrics.crewWithData > 0 ? vesselMetrics.totalRest24h / vesselMetrics.crewWithData : 0,
    avgRestPer7d:
      vesselMetrics.crewWithData > 0 ? vesselMetrics.totalRest7d / vesselMetrics.crewWithData : 0,
  };
}
...
export async function getFleetSTCWSummary(
  ...
  const vesselDataPromises = vessels.map((vessel) =>
    getCrewRestDataForVessel(orgId, vessel.id, startDate, endDate).then((crewData) => ({
      vessel,
      crewData,
    }))
  );
  ...
  const result: FleetSTCWSummary = {
    ...
    fleet: {
      ...
      overallComplianceRate:
        fleetMetrics.totalCrew > 0
          ? (fleetMetrics.compliantCrew / fleetMetrics.totalCrew) * 100
          : 100,
      ...
      avgRestPer24h:
        fleetMetrics.crewWithRestData > 0
          ? fleetMetrics.totalRest24h / fleetMetrics.crewWithRestData
          : 0,
      avgRestPer7d:
        fleetMetrics.crewWithRestData > 0
          ? fleetMetrics.totalRest7d / fleetMetrics.crewWithRestData
          : 0,
    },
