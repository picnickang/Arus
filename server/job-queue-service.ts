...
class JobQueueService {
  ...
  async startWorker(handler: (job: PgBoss.Job<DocumentIngestionJob>) => Promise<void>, concurrency: number = 5): Promise<void> {
    if (!this.boss) {
      throw new Error('Job queue not initialized');
    }
    ...
    const instrumentedHandler = async (job: PgBoss.Job<DocumentIngestionJob>) => {
      ...
      await withLoggingContext(
        { correlationId, orgId: job.data.orgId },
        async () => {
          try {
            await handler(job);
    ...
    await this.boss.work('document-ingestion', { teamSize: concurrency, teamConcurrency: 1 }, instrumentedHandler);
