...
function getEncryptionKey(): Buffer {
  ...
  if (sessionSecret.length < MIN_KEY_STRENGTH && isProduction) {
    throw new Error('SESSION_SECRET must be at least 32 characters when used for encryption in production');
  }
  
  const salt = SCRYPT_SALT_PREFIX + createHash('sha256').update(sessionSecret).digest('hex').substring(0, 16);
