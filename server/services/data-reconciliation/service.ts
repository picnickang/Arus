...
import {
  ...
  validateFailurePredictions,
  validateOrgConsistency,
} from "./validators.js";
...
export class DataReconciliationService {
  ...
  async runReconciliation(orgId: string): Promise<ReconciliationReport> {
    ...
    const endTimer = reconciliationMetrics.reconciliationDuration.startTimer({
    ...
    try {
      ...
      const dataQualityScore =
        recordsScanned > 0 ? Math.max(0, 1 - issues.length / recordsScanned) : 1;

      reconciliationMetrics.validationRuns.inc({ orgId, type: "full" });
      reconciliationMetrics.dataQualityScore.set(
        { orgId, dataType: "telemetry" },
        dataQualityScore
      );
      issues.forEach((issue) =>
        reconciliationMetrics.issuesDetected.inc({
          orgId,
  ...
  getStatus(): ReconciliationStatus {
    return {
      ...
      nextScheduledRun: this.lastRun
        ? new Date(this.lastRun.getTime() + 60 * 60 * 1000)
  ...
  startScheduledReconciliation(intervalMinutes: number = 60): void {
    ...
    setInterval(
      async () => {
        try {
          const orgs = await db
            .selectDistinct({ orgId: equipment.orgId })
            .from(equipment)
