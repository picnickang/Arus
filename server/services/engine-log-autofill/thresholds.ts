/**
 * Engine Log Auto-Fill - Anomaly Thresholds
 * Maritime industry standard parameter ranges
 */
...
export function checkAnomaly(
  ...
  if (threshold.min !== undefined && value < threshold.min) {
    return {
      ...
      reason: `Below minimum (${value} < ${threshold.min} ${threshold.unit})`,
    };
  }
  ...
  if (threshold.max !== undefined && value > threshold.max) {
    return {
      isAnomaly: true,
      severity: threshold.severity,
      reason: `Above maximum (${value} > ${threshold.max} ${threshold.unit})`,
    };
  }
