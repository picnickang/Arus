...
export function getSFOC(loadPercent: number): number {
  const loads = Object.keys(SFOC_CURVE).map(Number).sort((a, b) => a - b);
  ...
  if (lowerLoad === upperLoad) { return lowerSFOC; }
...
export function calculateEEOI(totalFuelMt: number, cargoTons: number, distanceNm: number): number | null {
...
export function calculateCII(totalCo2Mt: number, vesselDwt: number, distanceNm: number): number | null {
