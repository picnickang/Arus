...
export class StormGeoIntegrationService {
  ...
  async importCSV(orgId: string, vesselId: string, fileContent: string, fileName: string, initiatedBy?: string): Promise<StormgeoImportHistory> {
    ...
    try {
      ...
      const snapshots: InsertStormgeoSnapshot[] = [], errors: Array<{ row: number; field?: string; error: string }> = [];

      for (let i = 0; i < rows.length; i++) {
        try { snapshots.push(csvRowToSnapshot(rows[i], orgId, vesselId, fileName)); }
  ...
  async importJSON(orgId: string, vesselId: string, fileContent: string, fileName: string, initiatedBy?: string): Promise<StormgeoImportHistory> {
    ...
    try {
      ...
      for (let i = 0; i < data.waypoints.length; i++) {
        try { snapshots.push(jsonWaypointToSnapshot(data.waypoints[i], orgId, vesselId, fileName, data.route_id, data.route_name, data.departure_port, data.arrival_port, data.departure_time, data.arrival_time)); }
  ...
  async autoFillHourlyEntry(vesselId: string, logDate: string, hour: number, orgId: string): Promise<{ fields: Partial<DeckLogHourly>; source: string; snapshotId?: string } | null> {
    ...
    if (snapshot) {
      return {
        fields: {
          windDirection: snapshot.windDirection ? bearingToDirection(snapshot.windDirection) : undefined,
          windForce: snapshot.windForceBeaufort ?? (snapshot.windSpeed ? windSpeedToBeaufort(snapshot.windSpeed) : undefined),
          seaState: snapshot.seaState ?? (snapshot.waveHeight ? waveHeightToSeaState(snapshot.waveHeight) : undefined),
          swellDirection: snapshot.swellDirection ? bearingToDirection(snapshot.swellDirection) : undefined,
          swellHeight: snapshot.swellHeight ?? undefined, barometer: snapshot.barometer ?? undefined,
          airTemperature: snapshot.airTemperature ?? undefined, seaTemperature: snapshot.seaTemperature ?? undefined,
          visibility: snapshot.visibility ? Math.round(snapshot.visibility) : undefined, skyCondition: snapshot.skyCondition ?? undefined,
          humidity: snapshot.humidity ?? undefined, latitude: snapshot.latitude ?? undefined, longitude: snapshot.longitude ?? undefined,
        },
        source: 'stormgeo', snapshotId: snapshot.id,
      };
    }
    return null;
  }
