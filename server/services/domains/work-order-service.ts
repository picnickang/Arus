...
class WorkOrderService {
  ...
  async closeWorkOrder(id: string, closeData: { notes?: string; completedBy?: string }): Promise<WorkOrder> {
    const closedOrder = await db.transaction(async (tx) => {
      ...
      const partIdsArray = partIds.length > 0 ? sql`ARRAY[${sql.join(partIds.map(pid => sql`${pid}`), sql`, `)}]::text[]` : sql`ARRAY[]::text[]`;
  ...
  async deleteWorkOrderCascade(id: string): Promise<void> {
    ...
    if (r.length === 0) { throw new Error(`Work order ${id} not found`); }
    const ws = getWebSocketServer(); if (r[0]) { ws?.broadcastWorkOrderChange("delete", { id: r[0].id }); }
  }
  ...
  async completeWorkOrder(workOrderId: string, completionData: InsertWorkOrderCompletion): Promise<WorkOrderCompletion> {
    ...
    return db.transaction(async (tx) => {
      ...
      for (const woPart of woParts) { consumeMap.set(woPart.partId, (consumeMap.get(woPart.partId) || 0) + woPart.quantityUsed); }
  ...
  async getWorkOrderCompletionAnalytics(filters?: { equipmentId?: string; vesselId?: string; startDate?: Date; endDate?: Date; orgId?: string }): Promise<{ totalCompletions: number; avgDurationVariance: number; avgCostVariance: number; onTimeCompletionRate: number; totalDowntimeHours: number }> {
    ...
    if (c.length === 0) { return { totalCompletions: 0, avgDurationVariance: 0, avgCostVariance: 0, onTimeCompletionRate: 0, totalDowntimeHours: 0 }; }
