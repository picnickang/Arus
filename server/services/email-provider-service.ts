...
class EmailProviderService {
  ...
  private readonly providerHandlers: Record<EmailProvider, (config: EmailConfig, payload: EmailPayload) => Promise<SendResult>> = {
  ...
  private async sendViaSmtp(config: EmailConfig, payload: EmailPayload): Promise<SendResult> {
    ...
    if (!transporter) {
      ...
      transporter = createTransport({
        ...
        auth: smtpUser && decryptedPassword ? {
          user: smtpUser,
          pass: decryptedPassword,
        } : undefined,
        tls: smtpUseTls !== false ? {
    ...
    try {
      ...
      log('info', 'Email sent via SMTP', { messageId: info.messageId, recipients: payload.to.length });
      return { success: true, messageId: info.messageId, provider: 'smtp' };
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      const isRetriable = errorMessage.includes('ECONNREFUSED') || 
  ...
  private async sendViaSes(config: EmailConfig, payload: EmailPayload): Promise<SendResult> {
    ...
    try {
      ...
      log('info', 'Email sent via SES', { messageId: info.messageId, recipients: payload.to.length });
