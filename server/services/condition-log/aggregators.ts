...
import { db } from "../../db";
import { vibrationFeatures, vibrationAnalysis, conditionMonitoring, alertNotifications, equipment } from "@shared/schema";
import { eq, and, gte, lte, sql } from "drizzle-orm";
import type { VibrationAggregation, ConditionAggregation } from "./types.js";

export async function aggregateVibrationData(orgId: string, equipmentId: string, periodStart: Date, periodEnd: Date): Promise<VibrationAggregation | null> {
  ...
  }).from(vibrationFeatures).where(and(eq(vibrationFeatures.orgId, orgId), eq(vibrationFeatures.equipmentId, equipmentId), gte(vibrationFeatures.timestamp, periodStart), lte(vibrationFeatures.timestamp, periodEnd)));

  const analyses = await db.select({ id: vibrationAnalysis.id }).from(vibrationAnalysis).where(and(eq(vibrationAnalysis.orgId, orgId), eq(vibrationAnalysis.equipmentId, equipmentId), gte(vibrationAnalysis.analysisTimestamp, periodStart), lte(vibrationAnalysis.analysisTimestamp, periodEnd)));

  if (!features[0] || features[0].rmsAvg === null) { return null; }
...
export async function aggregateConditionData(orgId: string, equipmentId: string, periodStart: Date, periodEnd: Date): Promise<ConditionAggregation> {
  ...
  }).from(conditionMonitoring).where(and(eq(conditionMonitoring.orgId, orgId), eq(conditionMonitoring.equipmentId, equipmentId), gte(conditionMonitoring.timestamp, periodStart), lte(conditionMonitoring.timestamp, periodEnd)));

  const alertCounts = await db.select({
    total: sql<number>`count(*)`,
    critical: sql<number>`count(*) FILTER (WHERE ${alertNotifications.severity} = 'critical')`,
  }).from(alertNotifications).where(and(eq(alertNotifications.orgId, orgId), eq(alertNotifications.equipmentId, equipmentId), gte(alertNotifications.createdAt, periodStart), lte(alertNotifications.createdAt, periodEnd)));
...
export async function getMonitoredEquipment(vesselId: string) {
  return db.select({ id: equipment.id }).from(equipment).where(and(eq(equipment.vesselId, vesselId), eq(equipment.status, 'operational'), sql`${equipment.category} IN ('propulsion', 'auxiliary', 'deck_machinery')`));
}
