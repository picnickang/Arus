...
export async function getPreviousHealthIndex(orgId: string, equipmentId: string, currentPeriodStart: Date): Promise<{ healthIndex: number; periodStart: Date } | null> {
  const previous = await db.select({ healthIndex: conditionLogSummary.healthIndex, periodStart: conditionLogSummary.periodStart }).from(conditionLogSummary).where(and(eq(conditionLogSummary.orgId, orgId), eq(conditionLogSummary.equipmentId, equipmentId), sql`${conditionLogSummary.periodEnd} <= ${currentPeriodStart}`)).orderBy(sql`${conditionLogSummary.periodEnd} DESC`).limit(1);

  if (previous.length === 0 || previous[0].healthIndex === null) { return null; }
  return { healthIndex: previous[0].healthIndex, periodStart: previous[0].periodStart };
}

export async function createConditionLogEntry(orgId: string, vesselId: string, equipmentId: string, periodStart: Date, periodEnd: Date, periodType: 'hourly' | 'daily' = 'hourly'): Promise<string | null> {
  ...
  if (!vibrationData && conditionData.dataPoints === 0) { return null; }
  ...
  if (previousHealth) {
    const daysBetween = (periodStart.getTime() - previousHealth.periodStart.getTime()) / (1000 * 60 * 60 * 24);
    degradationRate = calculateDegradationRate(currentHealth, previousHealth.healthIndex, daysBetween);
  ...
  const logEntry: InsertConditionLogSummary = {
    orgId, vesselId, equipmentId, periodStart, periodEnd, periodType,
  ...
  const result = await db.insert(conditionLogSummary).values(logEntry).returning({ id: conditionLogSummary.id });
