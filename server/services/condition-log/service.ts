...
export class ConditionLogService {
  ...
  async createConditionLogEntry(
    ...
    return createConditionLogEntry(
      orgId,
      vesselId,
      equipmentId,
      periodStart,
    ...
    try {
      ...
      for (const equip of monitoredEquipment) {
        ...
        while (currentStart < endDate) {
          ...
          try {
            const logId = await createConditionLogEntry(
              orgId,
              vesselId,
              equip.id,
              currentStart,
              currentEnd > endDate ? endDate : currentEnd,
              periodType
            );
            if (logId) {
            ...
            result.errors.push(
              `Equipment ${equip.id}: ${error instanceof Error ? error.message : String(error)}`
            );
          }
