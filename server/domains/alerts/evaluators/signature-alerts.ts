...
export async function evaluateMissingSignatureAlerts(
  ...
  for (const log of deck) {
    results.push({
      triggered: true,
      alertType: "missing_signature_deck",
      alertKey: `missing_signature_deck_${log.id}`,
      severity: "warning",
      title: "Deck Log Missing Signature",
      message: `Deck log for ${log.logDate} requires signature`,
      entityId: log.id,
      entityType: "vessel",
      metadata: {
        ...
        logType: "deck",
        hoursOverdue:
          Math.floor((now.getTime() - new Date(log.logDate).getTime()) / (60 * 60 * 1000)) -
          signatureGraceHours,
      },
    });
  }
  ...
  for (const log of engine) {
    results.push({
      triggered: true,
      alertType: "missing_signature_engine",
      alertKey: `missing_signature_engine_${log.id}`,
      severity: "warning",
      title: "Engine Log Missing Signature",
      message: `Engine log for ${log.logDate} requires signature`,
      entityId: log.id,
      entityType: "vessel",
      metadata: {
        vesselId: log.vesselId,
