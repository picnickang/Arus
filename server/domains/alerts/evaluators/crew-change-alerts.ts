...
function getSeverityAndTitle(daysUntilEnd: number): {
  severity: "info" | "warning" | "critical";
  title: string;
} {
  if (daysUntilEnd === 0) {
    return { severity: "critical", title: "Crew Assignment Ends Today" };
  }
  if (daysUntilEnd <= 3) {
    return { severity: "critical", title: "Urgent Crew Change Reminder" };
  }
  if (daysUntilEnd <= 7) {
    return { severity: "warning", title: "Upcoming Crew Change" };
  }
...
function buildAlertResult(
  ...
  const message =
    daysUntilEnd === 0
      ? "Crew member assignment ends today"
      : `Crew member assignment ends in ${daysUntilEnd} days`;
  return {
    triggered: true,
    alertType: "crew_change_reminder",
    alertKey: `crew_change_${assignment.id}`,
    severity,
    title,
    message,
    entityId: assignment.crewId,
    entityType: "crew",
    metadata: {
      vesselId,
...
function processAssignment(
  ...
  now: Date,
  reminderDays: number
): CrewAlertResult | null {
  if (!isAssignmentActive(assignment)) {
    return null;
  }
  if (!hasStartedAssignment(assignment, now)) {
    return null;
  }
  if (!assignment.end) {
    return null;
  ...
  if (daysUntilEnd < 0 || daysUntilEnd > reminderDays) {
    return null;
  }
...
export async function evaluateCrewChangeReminders(
