...
import { db } from "../../../db";
import { mlModels, modelVersions, modelDeployments, type MlModel, type ModelVersion, type InsertModelVersion, type ModelDeployment } from "@shared/schema";
...
export class ModelRegistryAdapter implements ModelRegistryPort {
  async listModels(orgId: string): Promise<MlModel[]> {
    return db.select().from(mlModels).where(eq(mlModels.orgId, orgId)).orderBy(desc(mlModels.createdAt));
  }
  ...
  async listVersions(orgId: string, modelId: string): Promise<ModelVersion[]> {
    return db.select().from(modelVersions)
  ...
  async createVersion(data: InsertModelVersion): Promise<ModelVersion> {
    ...
    logger.info("[ModelRegistry] Version created", { modelId: data.modelId, version: data.version });
    return result;
  }

  async getActiveDeployment(orgId: string, modelId: string): Promise<ModelDeployment | null> {
    const [result] = await db.select().from(modelDeployments)
      .where(and(
  ...
  async deploy(orgId: string, modelId: string, modelVersionId: string, target: string): Promise<ModelDeployment> {
    ...
    const [result] = await db.insert(modelDeployments).values({
      orgId,
      modelId,
      modelVersionId,
    ...
    logger.info("[ModelRegistry] Model deployed", { orgId, modelId, modelVersionId, deploymentId: result.id });
    return result;
  }
  ...
  async rollback(orgId: string, deploymentId: number): Promise<ModelDeployment> {
    ...
    const [previous] = await db.select().from(modelDeployments)
      .where(and(
        eq(modelDeployments.orgId, orgId),
    ...
    if (previous) {
      const [restored] = await db.update(modelDeployments)
