...
export class CrewExtensionsEventPublisherAdapter
  implements ICrewExtensionsEventPublisher, ICrewExtensionsAuditPort
{
  async publish(event: CrewExtensionsDomainEvent): Promise<void> {
    ...
    await recordAndPublish(entityType, event.aggregateId, operation, event.payload, event.userId);
