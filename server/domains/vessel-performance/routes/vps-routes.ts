    ...
    withErrorHandling(
      ...
      async (req: AuthenticatedRequest, res: Response) => {
        ...
        for (const equipment of vesselEquipment) {
          // NOTE: the 4th parameter of getTelemetryByEquipmentAndDateRange
          // is an optional sensorType filter, NOT orgId — passing orgId
          // there silently filtered for sensor_type='<orgId>' and made this
          // analysis permanently empty. Org scoping comes from the
          // request's RLS tenant context.
    ...
    withErrorHandling(
      ...
      async (req: AuthenticatedRequest, res: Response) => {
        ...
        const vesselCount = vesselType
          ? vessels.filter((v) => (v as { vesselType?: string | null }).vesselType === vesselType)
              .length
          : vessels.length;
