...
function signalSeverity(signals: OperatorExperienceSignalSnapshot, key: string): number {
  ...
  if (key === "criticalItems" && value > 0) {
    return 3;
  }
  if (key === "conflicts" && value > 0) {
    return 3;
  }
  if (key === "blockedItems" && value > 0) {
    return 2;
  }
  if (key === "waitingOnParts" && value > 0) {
    return 2;
  }
  if (key === "dataQualityWarnings" && value > 0) {
    return 2;
  }
  if (key === "pdmRisks" && value > 0) {
    return 2;
  }
  if (key === "readyForCloseout" && value > 0) {
    return 1;
  }
  if (key === "handoverNotes" && value > 0) {
    return 1;
  }
  if (key === "offlinePending" && value > 0) {
    return 1;
  }
  if (key === "attentionItems" && value > 0) {
    return 1;
  }
  return 0;
}
...
function cta(definition: RoleInformationNeedDefinition, status: InformationNeedStatus): string {
  if (status === "critical") {
    return `Act now: ${definition.primaryAction}`;
  }
  if (status === "needs_attention") {
    return `Review today: ${definition.primaryAction}`;
  }
  if (status === "watch") {
    return `Keep visible: ${definition.primaryAction}`;
  }
...
export function prioritizeInformationNeed(
  ...
  const primaryMetric =
    definition.sourceSignals.length > 0 ? metric(signals, definition.sourceSignals[0]!) : null;
