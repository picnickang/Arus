...
export function computeExperiencePillarScores(
  signals: OperatorExperienceSignalSnapshot
): ExperiencePillarScore[] {
  const sourceFailures = Object.values(signals.sourceHealth).filter(
    (value) => value !== "ok"
  ).length;
  const clarityScore = 92 - Math.min(signals.attentionItems, 20) * 1.5 - sourceFailures * 8;
  const trustScore =
    88 - signals.dataQualityWarnings * 7 - sourceFailures * 10 - signals.conflicts * 6;
  const actionabilityScore =
    90 -
    signals.blockedItems * 5 -
    signals.waitingOnParts * 3 +
    Math.min(signals.readyForCloseout, 4) * 2;
  ...
  return [
    {
      ...
      label: "Source health",
      description:
        "Users know whether recommendations are based on healthy work-order, alert, equipment, and inventory sources.",
      status: sourceFailures === 0 ? "present" : "needs_attention",
      evidence:
        sourceFailures === 0
          ? "All workflow source adapters responded."
          : `${sourceFailures} source adapter(s) reported degraded status.`,
    },
    {
      id: "offline-state",
      label: "Offline state",
      ...
      evidence:
        signals.offlinePending > 0
          ? `${signals.offlinePending} change(s) pending sync.`
          : "No pending offline changes are visible.",
    },
    {
      id: "handover-context",
      ...
      status:
        signals.handoverNotes > 0 || signals.attentionItems > 0 ? "present" : "needs_attention",
      evidence:
        signals.handoverNotes > 0
          ? `${signals.handoverNotes} handover note(s) available.`
          : "No recent handover note is visible.",
    },
    {
      id: "pdm-evidence",
      label: "PdM evidence",
      ...
      evidence:
        signals.dataQualityWarnings > 0
          ? `${signals.dataQualityWarnings} data warning(s) require review.`
          : "No PdM data-quality warning is visible.",
    },
  ];
}
