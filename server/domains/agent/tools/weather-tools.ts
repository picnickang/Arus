...
async function fetchMarineWeather(lat: number, lng: number): Promise<MarineWeatherData> {
  ...
  const params = new URLSearchParams({
    lat: String(lat),
    lng: String(lng),
    params: "windSpeed,windDirection,waveHeight,wavePeriod,waveDirection,waterTemperature,airTemperature,visibility,humidity,pressure,cloudCover,precipitation",
  });
  ...
  if (!response.ok) {
    throw new Error(`Weather API returned ${response.status}: ${await response.text().catch(() => "")}`);
  }
...
registerTool({
  ...
  parameters: {
    ...
    properties: {
      vesselId: {
        type: "string",
        description: "Vessel ID — will look up the vessel's last known position. Provide either vesselId or lat/lng.",
      },
      lat: { type: "number", description: "Latitude (decimal degrees). Use with lng as an alternative to vesselId." },
      lng: { type: "number", description: "Longitude (decimal degrees). Use with lat as an alternative to vesselId." },
    },
    required: [],
  },
...
registerTool({
  ...
  parameters: {
    ...
    properties: {
      ...
      activityType: {
        type: "string",
        enum: ["deck_work", "crane_lifting", "hull_inspection", "enclosed_space", "engine_room", "general"],
  ...
  inputSchema: z.object({
    vesselId: z.string().optional(),
    activityType: z.enum(["deck_work", "crane_lifting", "hull_inspection", "enclosed_space", "engine_room", "general"]),
