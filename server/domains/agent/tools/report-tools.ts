...
registerTool({
  ...
  riskLevel: "read",
  description:
    "Generate a comprehensive fleet health report. Aggregates equipment status, active alerts, upcoming maintenance, and failure predictions into a summary.",
  parameters: {
    type: "object",
    properties: {
  ...
  async execute(input: { vesselId?: string; period?: string }, ctx) {
    ...
    if (input.vesselId) {
      eqConditions.push(eq(equipment.vesselId, input.vesselId));
    }
    ...
    const [allEquipment, activeAlerts, upcomingMaint, recentPredictions] = await Promise.all([
      ...
      db
        ...
        .where(
          and(eq(alertNotifications.orgId, ctx.orgId), gte(alertNotifications.createdAt, lookback))
        )
