...
export class AgentActivityService {
  ...
  async summary(orgId: string): Promise<ActivitySummary> {
    ...
    const successRate7d =
      raw.totalRuns7d > 0 ? Math.round((raw.successCount7d / raw.totalRuns7d) * 100) : 100;

    const avgTokensPerRun =
      raw.tokenRunCount > 0 ? Math.round(raw.totalTokens30d / raw.tokenRunCount) : 0;

    const estimatedCost30d =
      Math.round((raw.totalTokens30d / 1000) * COST_PER_1K_TOKENS * 100) / 100;
