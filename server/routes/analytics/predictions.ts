...
function mapPredictionToResult(
  p: FailurePredictionRow,
  ...
  const costImpactRaw = p.costImpact as {
...
export function mountPredictionsRoutes(router: Router) {
  router.get("/anomalies", async (req: Request, res: Response) => {
    try {
      ...
      const cacheKey = analyticsCacheKeys.anomalies(
        orgId,
  ...
  router.get("/failure-predictions", async (req: Request, res: Response) => {
    try {
      ...
      const cacheKey = analyticsCacheKeys.failurePredictions(
        orgId,
        equipmentId as string | undefined,
