...
export async function runHealthChecks() {
  const [database, telemetry, memory] = await Promise.all([
    checkDatabase(),
    checkTelemetry(),
    checkMemory(),
  ]);
...
export async function checkTelemetry(): Promise<CheckResult> {
  try {
    ...
    const bufferUtilization =
      stats.bufferSize > 0 ? ((stats as any).currentBufferSize / stats.bufferSize) * 100 : 0;
    if (bufferUtilization > 90) {
      return {
        status: "warn",
        message: "Telemetry buffer near capacity",
        details: { bufferUtilization: Math.round(bufferUtilization) },
      };
    }
    return {
      status: "pass",
      details: {
        ...
        totalWritten: (stats as any).totalWritten,
      },
    };
  } catch {
    return { status: "warn", message: "Telemetry service not available" };
  }
}
...
export async function checkMemory(): Promise<CheckResult> {
  ...
  if (utilizationPercent > 90) {
    return { status: "fail", message: "Critical memory pressure", details };
  }
  if (utilizationPercent > 70) {
    return { status: "warn", message: "Elevated memory usage", details };
  }
...
export async function checkServices(): Promise<ServiceStatus[]> {
  ...
  try {
    ...
    services.push({ name: "telemetry-batch-writer", status: "stopped" });
  }
  return services;
}
