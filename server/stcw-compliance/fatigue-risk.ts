...
import { STCW_MIN_REST_24, STCW_MIN_REST_7D, NIGHT_HOURS_START, NIGHT_HOURS_END, FULL_REST_THRESHOLD } from "./types";
...
function countNightWorkHours(day: RestDay): number {
  ...
  for (let h = 0; h < 24; h++) {
    ...
    if (!isRest && isNightHour(h)) {nightWork++;}
  }
  return nightWork;
}
...
export function calculateFatigueMetrics(days: RestDay[]): FatigueMetrics {
  if (days.length === 0) {
    return {
      sleepDebt24h: 0, sleepDebt7d: 0, consecutiveNightShifts: 0,
      timeSinceLastFullRest: 0, nightWorkRatio: 0,
      avgRestPer24h: STCW_MIN_REST_24, avgRestPer7d: STCW_MIN_REST_7D,
    };
  }

  let sleepDebt24h = 0, consecutiveNightShifts = 0, currentNightStreak = 0;
  let totalNightWork = 0, totalWork = 0, totalRest = 0;
  let timeSinceLastFullRest = 0, foundFullRest = false;
  ...
  for (let i = days.length - 1; i >= 0; i--) {
    ...
    if (!foundFullRest) {
      if (longestRest >= FULL_REST_THRESHOLD) {foundFullRest = true;}
      else {timeSinceLastFullRest += 24;}
    }
  }

  const last7Days = days.slice(-7);
  let rest7d = 0;
  for (const day of last7Days) {rest7d += countRestHours(day);}
  const sleepDebt7d = rest7d < STCW_MIN_REST_7D ? STCW_MIN_REST_7D - rest7d : 0;

  return {
    sleepDebt24h, sleepDebt7d, consecutiveNightShifts, timeSinceLastFullRest,
...
export function calculateFatigueRisk(crewId: string, days: RestDay[], crewName?: string): FatigueRiskResult {
  ...
  if (metrics.sleepDebt24h > 0) {
    ...
    factors.push(`Sleep debt: ${metrics.sleepDebt24h.toFixed(1)}h below minimum in recent 24h periods`);
  ...
  if (metrics.consecutiveNightShifts >= 5) {
    score += 30;
    factors.push(`Extended night watch: ${metrics.consecutiveNightShifts} consecutive night shifts`);
  ...
  if (metrics.nightWorkRatio > 0.5) {
    score += 15;
    factors.push(`High night work: ${(metrics.nightWorkRatio * 100).toFixed(0)}% of work during night hours`);
  ...
  if (score >= 60) {
    riskLevel = 'critical';
    if (recommendations.length === 0) {recommendations.push('Immediate action required');}
  } else if (score >= 40) {riskLevel = 'high';}
  else if (score >= 20) {riskLevel = 'medium';}
  else {
    riskLevel = 'low';
    if (factors.length === 0) {factors.push('Rest patterns within acceptable limits');}
  }
...
export function calculateVesselFatigueSummary(crewFatigueResults: FatigueRiskResult[]) {
  const summary = {
    totalCrew: crewFatigueResults.length, criticalCount: 0, highCount: 0,
    mediumCount: 0, lowCount: 0, averageScore: 0, highestRiskCrew: [] as FatigueRiskResult[],
  };

  if (crewFatigueResults.length === 0) {return summary;}
  ...
  for (const result of crewFatigueResults) {
    ...
    if (counterKey) { (summary as any)[counterKey]++; }
  }
