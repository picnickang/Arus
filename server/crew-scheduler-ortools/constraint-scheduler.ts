...
function initializeState(crew: CrewWithSkills[]): SchedulerState {
  ...
  crew.forEach((c) => {
    state.crewAssignments[c.id] = [];
    state.nightShiftCounts[c.id] = 0;
  });
  return state;
}

function meetsRankRequirement(crewMember: CrewWithSkills, rankMin?: string): boolean {
  if (!rankMin || !crewMember.rank) {
    return true;
  }
  ...
  if (crewRankIndex === -1 || minRankIndex === -1) {
    return true;
  }
...
function calculateCrewPenalty(
  ...
  if (crewPrefs) {
    if (crewPrefs.days_off?.includes(day)) {
      penalty += config.weights.pref_off;
    }
    if (crewPrefs.prefer_vessel && vesselId && vesselId !== crewPrefs.prefer_vessel) {
      penalty += config.weights.vessel_mismatch;
    }
  }
  return penalty;
}
...
function determineShortageReason(shift: SelectShiftTemplate, eligibleCount: number): string {
  ...
  if (shift.requiredSkills) {
    return `no crew with required skill: ${shift.requiredSkills}`;
  }
  if (shift.certRequired) {
    return `no crew with required certification: ${shift.certRequired}`;
  }
  if (shift.rankMin) {
    return `no crew with minimum rank: ${shift.rankMin}`;
  }
  return "insufficient eligible crew";
}
    ...
    for (const shift of shifts) {
      ...
      if (shortage > 0) {
        state.unfilled.push({
          ...
          shiftId: shift.id!,
          need: shortage,
          reason: determineShortageReason(shift, eligibleCrew.length),
        });
      }
    }
  }
