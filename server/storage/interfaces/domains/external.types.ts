...
export interface IExternalStorage {
  ...
  getStormgeoImportHistory(
    orgId: string,
    filters?: { vesselId?: string; status?: string; limit?: number }
  ): Promise<StormgeoImportHistory[]>;
  getStormgeoImportHistoryById(
    id: string,
    orgId: string
  ): Promise<StormgeoImportHistory | undefined>;
  createStormgeoImportHistory(history: InsertStormgeoImportHistory): Promise<StormgeoImportHistory>;
  updateStormgeoImportHistory(
    id: string,
    history: Partial<InsertStormgeoImportHistory>,
    orgId: string
  ): Promise<StormgeoImportHistory>;
}
