...
export interface IEquipmentStorage {
  ...
  checkOperatingConditions(equipmentId: string, telemetry?: { sensorType: string; value: number }[], orgId?: string): Promise<{ violations: Array<{ parameterId: string; parameterName: string; currentValue: number; thresholdType: "below_optimal" | "above_optimal" | "below_critical" | "above_critical"; severity: "info" | "warning" | "critical"; lifeImpact?: string; recommendedAction?: string }>; alertsCreated: number }>;

  // Context
  getVesselContext(vesselId: string, orgId?: string): Promise<{ vessel: any; ageYears: number; operatingConditions: string[]; environmentalFactors: string[]; maintenanceHistory: any[]; fleetPosition?: { lat: number; lng: number } }>;
