...
export interface IMlStorage {
  ...
  addMlModelAccuracyHistory(history: InsertMlModelAccuracyHistory, orgId: string): Promise<MlModelAccuracyHistory>;

  // Feature Importance
  createFeatureImportance(importance: InsertFeatureImportance): Promise<FeatureImportance>;
  getFeatureImportancesByPrediction(orgId: string, predictionId: number, predictionType: "real_time" | "batch" | "anomaly"): Promise<FeatureImportance[]>;
  getFeatureImportancesByEquipment(orgId: string, equipmentId: string, limit?: number): Promise<FeatureImportance[]>;
  getFeatureImportanceById(id: number, orgId: string): Promise<FeatureImportance | undefined>;

  // Calibration Curves
  getCalibrationCurves(orgId: string, modelType?: string, equipmentType?: string, status?: string): Promise<CalibrationCurve[]>;
  getCalibrationCurve(id: string, orgId: string): Promise<CalibrationCurve | undefined>;
  getBestCalibrationCurve(orgId: string, modelType: string, equipmentType: string): Promise<CalibrationCurve | undefined>;
  createCalibrationCurve(curve: InsertCalibrationCurve, orgId: string): Promise<CalibrationCurve>;
  updateCalibrationCurve(id: string, updates: Partial<InsertCalibrationCurve>, orgId: string): Promise<CalibrationCurve>;
  ...
  labelPredictionOutcome(predictionId: string, predictionType: "anomaly" | "failure", outcome: { resolvedByWorkOrderId?: string; actualFailureOccurred: boolean; outcomeLabel: "true_positive" | "false_positive" | "true_negative" | "false_negative"; predictionAccuracy?: number }, orgId?: string): Promise<void>;
  getPredictionAccuracy(predictionType: "anomaly" | "failure", equipmentId: string | undefined, horizonDays: number, orgId?: string): Promise<{ totalPredictions: number; truePositives: number; falsePositives: number; trueNegatives: number; falseNegatives: number; accuracy: number; precision: number; recall: number; f1Score: number }>;
}
