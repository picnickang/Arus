...
export interface IWorkOrderStorage {
  ...
  getWorkOrderCompletions(filters?: {
    equipmentId?: string;
    vesselId?: string;
    startDate?: Date;
    endDate?: Date;
  ...
  getWorkOrderCompletionAnalytics(filters?: {
    equipmentId?: string;
    vesselId?: string;
    startDate?: Date;
    endDate?: Date;
  ...
  updateWorkOrderChecklist(
    id: string,
    checklist: Partial<InsertWorkOrderChecklist>
  ...
  updateWorkOrderWorklog(
    id: string,
    worklog: Partial<InsertWorkOrderWorklog>
  ): Promise<WorkOrderWorklog>;
  deleteWorkOrderWorklog(id: string): Promise<void>;
  calculateWorklogCosts(
    workOrderId: string
  ...
  addPartToWorkOrderWithValidation(
    workOrderId: string,
    partId: string,
    quantity: number,
    usedBy: string,
  ...
  updateWorkOrderPart(
    id: string,
    workOrderPart: Partial<InsertWorkOrderParts>
  ): Promise<WorkOrderParts>;
  removePartFromWorkOrder(id: string, orgId?: string): Promise<void>;
  removePartAndRestoreInventory(
    workOrderPartId: string,
    orgId: string,
    performedBy: string
  ): Promise<void>;
  getPartsCostForWorkOrder(
    workOrderId: string
  ): Promise<{ totalPartsCost: number; partsCount: number }>;
  checkPartAvailabilityForWorkOrder(
    partId: string,
    quantity: number,
    orgId?: string
  ...
  addBulkPartsAndReserveInventory(
    workOrderId: string,
    partsToAdd: Array<{ partId: string; quantity: number; usedBy: string; notes?: string }>,
    orgId: string
  ): Promise<{ added: WorkOrderParts[]; updated: WorkOrderParts[]; errors: string[] }>;
  ...
  getInventoryMovementsByWorkOrder(
    workOrderId: string,
    orgId: string
