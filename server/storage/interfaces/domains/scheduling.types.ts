...
export interface ISchedulingStorage {
  ...
  findRecentSchedulerRunByHash(orgId: string, inputHash: string, hoursBack?: number): Promise<SelectSchedulerRun | undefined>;
  ...
  getSchedulerRunsByStatus(orgId: string, status: string, limit?: number): Promise<SelectSchedulerRun[]>;
  ...
  deleteScheduleAssignmentsByDateRange(orgId: string, start: Date, end: Date, mode?: string): Promise<number>;
