...
export interface IMaintenanceStorage {
  ...
  completeChecklistItem(workOrderId: string, itemId: string, completedBy: string | null, completedByName: string | null, passed?: boolean | null, actualValue?: string, notes?: string | null, photoUrls?: string[]): Promise<MaintenanceChecklistCompletion>;
  ...
  getChecklistCompletionProgress(workOrderId: string): Promise<{ totalItems: number; completedItems: number; pendingItems: number; skippedItems: number; failedItems: number; percentComplete: number }>;
  ...
  findOptimalMaintenanceWindow(equipmentId: string, estimatedDurationHours: number, requiredSkills: string[], requiredParts: Array<{ partId: string; quantity: number }>, orgId?: string): Promise<{ portCalls: SelectPortCall[]; drydockWindows: SelectDrydockWindow[]; availableCrew: SelectCrew[]; partsAvailable: boolean; recommendations: Array<{ type: "port" | "drydock" | "underway"; window: Date[]; score: number; reason: string }> }>;
}
