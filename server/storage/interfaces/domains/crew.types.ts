...
export interface ICrewStorage {
  ...
  validateCrewSkills(crewId: string, requiredSkills: string[], orgId?: string): Promise<{ hasAllSkills: boolean; missingSkills: string[]; matchingSkills: string[] }>;
  ...
  getCrewAssignmentsByDateRange(from: Date, to: Date, orgId?: string): Promise<SelectCrewAssignment[]>;
  ...
  acknowledgeCertificationAlert(certId: string, userId?: string, notes?: string): Promise<SelectCrewCertification>;
  updateCertificationAlertFlags(orgId: string): Promise<{ scanned: number; flagged: number; critical: number; warning: number; notice: number }>;
  ...
  acknowledgeDocumentAlert(docId: string, userId?: string, notes?: string): Promise<SelectCrewDocument>;
  updateDocumentAlertFlags(orgId: string): Promise<{ scanned: number; flagged: number; critical: number; warning: number; notice: number }>;
  ...
  upsertCrewNotificationSettings(crewId: string, orgId: string, data: { emailAlertsEnabled?: boolean; certExpiryEmailEnabled?: boolean; documentExpiryEmailEnabled?: boolean; complianceEmailEnabled?: boolean; overrideEmail?: string | null }): Promise<CrewNotificationSettings>;
