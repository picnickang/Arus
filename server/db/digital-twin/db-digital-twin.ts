...
export class DatabaseDigitalTwinStorage {
  async getDigitalTwins(
    ...
    const c = [eq(vessels.orgId, orgId)];
    if (vesselId) {
      c.push(eq(digitalTwins.vesselId, vesselId));
    }
    if (twinType) {
      c.push(eq(digitalTwins.twinType, twinType));
    }
    const r = await db
      .select()
      .from(digitalTwins)
      ...
      .where(and(...c))
      .orderBy(sql`${digitalTwins.updatedAt} DESC`);
    return r.map((r) => r.digital_twins);
  }
  async getDigitalTwin(id: string, orgId: string): Promise<DigitalTwin | undefined> {
    ...
    return r[0]?.digital_twins;
  }

  async getTwinSimulations(
    ...
    const c = [];
    if (digitalTwinId) {
      c.push(eq(twinSimulations.digitalTwinId, digitalTwinId));
    }
    if (scenarioType) {
      c.push(eq(twinSimulations.scenarioType, scenarioType));
    }
    if (status) {
    ...
    return q.orderBy(sql`${twinSimulations.startTime} DESC`);
  }
  async getTwinSimulation(id: string): Promise<TwinSimulation | undefined> {
    const r = await db.select().from(twinSimulations).where(eq(twinSimulations.id, id));
    return r[0];
  }
  ...
  async getErrorLogs(filters?: ErrorLogFilters): Promise<ErrorLog[]> {
    const c = [];
    if (filters?.orgId) {
      c.push(eq(errorLogs.orgId, filters.orgId));
    }
    if (filters?.severity) {
      c.push(eq(errorLogs.severity, filters.severity));
    }
    if (filters?.category) {
      c.push(eq(errorLogs.category, filters.category));
    }
    if (filters?.resolved !== undefined) {
      c.push(eq(errorLogs.resolved, filters.resolved));
    }
    if (filters?.fromDate) {
      c.push(gte(errorLogs.timestamp, filters.fromDate));
    }
    if (filters?.toDate) {
    ...
    q = q.orderBy(sql`${errorLogs.timestamp} DESC`);
    if (filters?.limit) {
  ...
  async getErrorLogStats(orgId: string, days: number = 7): Promise<ErrorLogStats> {
    const cutoffDate = new Date();
    cutoffDate.setDate(cutoffDate.getDate() - days);
    const logs = await db
      .select()
      .from(errorLogs)
      .where(and(eq(errorLogs.orgId, orgId), gte(errorLogs.timestamp, cutoffDate)));
    ...
    return { total: logs.length, byCategory, bySeverity, resolved, unresolved };
  }
  ...
  async getSystemPerformanceMetrics(
    ...
    if (category) {
      c.push(eq(systemPerformanceMetrics.category, category));
    }
    if (hours) {
    ...
    return q.orderBy(sql`${systemPerformanceMetrics.recordedAt} DESC`);
  }
    ...
    return db
      ...
      .where(
        and(
          ...
          eq(systemPerformanceMetrics.category, category)
        )
      )
      .orderBy(sql`${systemPerformanceMetrics.recordedAt} DESC`)
      .limit(10);
  }
}
