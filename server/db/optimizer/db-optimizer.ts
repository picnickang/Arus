...
export class DbOptimizerStorage {
  ...
  async createOptimizerConfiguration(
    ...
    if (!n) {
      throw new Error("Failed to create optimizer configuration");
    }
    return {
      ...n,
  ...
  async createResourceConstraint(
    ...
    if (!n) {
      throw new Error("Failed to create resource constraint");
    }
    return n;
  }
  ...
  async createOptimizationResult(result: InsertOptimizationResult): Promise<OptimizationResult> {
    const [n] = await db
      ...
      .returning();
    if (!n) {
      throw new Error("createOptimizationResult: insert returned no row");
    }
    return n;
  }
  ...
  async createScheduleOptimization(
    ...
    const [n] = await db
      ...
      .returning();
    if (!n) {
      throw new Error("createScheduleOptimization: insert returned no row");
    }
    return n;
  }
