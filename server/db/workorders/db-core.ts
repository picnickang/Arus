...
export class DbWorkOrderCore {
  ...
  async getWorkOrders(equipmentId?: string, orgId?: string, filters?: WorkOrderFilters): Promise<WorkOrder[]> {
    ...
    return conditions.length > 0 ? db.select().from(workOrders).where(and(...conditions)).orderBy(sql`created_at DESC`) : db.select().from(workOrders).orderBy(sql`created_at DESC`);
  }

  async getWorkOrdersPaginated(equipmentId: string | undefined, orgId: string | undefined, limit: number, offset: number, filters?: WorkOrderFilters): Promise<WorkOrderPaginationResult> {
  ...
  async getWorkOrder(orgId: string, workOrderId: string): Promise<WorkOrder | undefined> {
    ...
    const [result] = await db.select().from(workOrders).where(and(eq(workOrders.id, workOrderId), eq(workOrders.orgId, orgId)));
    return result;
  }

  async getWorkOrderById(id: string, orgId: string): Promise<WorkOrder | undefined> { return this.getWorkOrder(orgId, id); }

  async generateWorkOrderNumber(orgId: string): Promise<string> {
    const currentYear = new Date().getFullYear();
    const [countResult] = await db.select({ count: sql<number>`count(*)` }).from(workOrders).where(and(eq(workOrders.orgId, orgId), sql`EXTRACT(YEAR FROM ${workOrders.createdAt}) = ${currentYear}`));
  ...
  async createWorkOrder(order: InsertWorkOrder & { woNumber?: string }): Promise<WorkOrder> {
    const [newOrder] = await db.insert(workOrders).values({ id: randomUUID(), ...order, status: order.status ?? "open", priority: order.priority ?? 3, woNumber: order.woNumber ?? null, createdAt: new Date(), updatedAt: new Date() }).returning();
    broadcastChange("create", newOrder);
    return newOrder;
  }

  async updateWorkOrder(id: string, updates: Partial<InsertWorkOrder>): Promise<WorkOrder> {
    const [updated] = await db.update(workOrders).set({ ...updates, updatedAt: new Date() }).where(eq(workOrders.id, id)).returning();
