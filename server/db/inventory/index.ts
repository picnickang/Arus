...
export class DatabaseInventoryStorage extends DbPartsStorage {
  ...
  async addBulkPartsToWorkOrder(
    ...
    const result: { added: WorkOrderParts[]; updated: WorkOrderParts[]; errors: string[] } = {
    ...
    await db.transaction(async (tx) => {
      ...
      for (const partToAdd of partsToAdd) {
        try {
          const [inventoryPart] = await tx
            .select()
            .from(partsInventory)
            .where(and(eq(partsInventory.id, partToAdd.partId), eq(partsInventory.orgId, orgId)))
            .limit(1);

          if (!inventoryPart) {
            result.errors.push(`Part ${partToAdd.partId} not found or not accessible`);
            continue;
          }
          ...
          if (existing) {
            ...
            const [newPart] = await tx
              ...
              .values({
                id: randomUUID(), orgId, workOrderId,
    ...
    await db.transaction(async (tx) => {
      ...
      for (const [partId, totalQty] of partQuantities.entries()) {
        ...
        if (available < totalQty) {
          throw new Error(
            `Insufficient stock for part ${inventory.partName}: available=${available}, requested=${totalQty}`
          );
        }

        await tx
          .update(partsInventory)
          .set({ quantityReserved: (inventory.quantityReserved || 0) + totalQty, updatedAt: new Date() })
          .where(and(eq(partsInventory.id, partId), eq(partsInventory.orgId, orgId)));
      }
    });
  }
    ...
    await db.transaction(async (tx) => {
      ...
      for (const [partId, totalQty] of partQuantities.entries()) {
        ...
        if (inventory) {
          ...
          await tx
            .update(partsInventory)
            .set({ quantityReserved: newReserved, updatedAt: new Date() })
            .where(and(eq(partsInventory.id, partId), eq(partsInventory.orgId, orgId)));
        }
      }
    });
  ...
  async removePartAndRestoreInventory(
    ...
    await db.transaction(async (tx) => {
      ...
      await tx.insert(inventoryMovements).values({
        ...
        performedBy,
        notes: `Returned ${woPart.quantityUsed} units from work order`,
        createdAt: new Date(),
      });
      ...
      if (wo) {
        ...
        await tx
          .update(workOrders)
          .set({ totalPartsCost, totalCost: totalPartsCost + (wo.totalLaborCost || 0), updatedAt: new Date() })
          .where(and(eq(workOrders.id, woPart.workOrderId), eq(workOrders.orgId, orgId)));
      }
    });
  }
