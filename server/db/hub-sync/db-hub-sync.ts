...
export class DatabaseHubSyncStorage {
  ...
  async markOutboxItemsSynced(ids: string[]): Promise<void> { if (ids.length === 0) {return;} const idsArray = sql`ARRAY[${sql.join(ids.map(id => sql`${id}`), sql`, `)}]::text[]`; await db.update(syncOutbox).set({ status: 'synced', syncedAt: new Date(), updatedAt: new Date() }).where(sql`${syncOutbox.id} = ANY(${idsArray})`); }
