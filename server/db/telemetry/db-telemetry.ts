...
export class DatabaseTelemetryStorage {
  ...
  async getTelemetryByDateRange(startDate: Date, endDate: Date, orgId?: string): Promise<EquipmentTelemetry[]> { const conditions: any[] = [gte(equipmentTelemetry.ts, startDate), lte(equipmentTelemetry.ts, endDate)]; if (orgId) { conditions.push(eq(equipmentTelemetry.orgId, orgId)); } return db.select().from(equipmentTelemetry).where(and(...conditions)).orderBy(equipmentTelemetry.ts).limit(10000); }
  ...
  async getLatestTelemetryReadings(equipmentId?: string, limit: number = 100, vesselId?: string, sensorType?: string): Promise<EquipmentTelemetry[]> { const conditions: any[] = []; if (equipmentId) { conditions.push(eq(equipmentTelemetry.equipmentId, equipmentId)); } if (vesselId) { conditions.push(eq(equipmentTelemetry.vesselId, vesselId)); } if (sensorType) { conditions.push(eq(equipmentTelemetry.sensorType, sensorType)); } let query = db.select().from(equipmentTelemetry); if (conditions.length > 0) { query = query.where(and(...conditions)) as any; } return (query as any).orderBy(sql`${equipmentTelemetry.ts} DESC`).limit(limit); }
