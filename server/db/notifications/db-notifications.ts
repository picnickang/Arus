...
export class DatabaseNotificationsStorage {
  ...
  async getEmailQueue(status?: string, limit?: number, orgId?: string): Promise<EmailQueue[]> { const conditions: any[] = []; if (orgId) { conditions.push(eq(emailQueue.orgId, orgId)); } if (status) { conditions.push(eq(emailQueue.status, status)); } let query = db.select().from(emailQueue); if (conditions.length > 0) { query = query.where(and(...conditions)) as any; } query = query.orderBy(emailQueue.createdAt); if (limit) { query = query.limit(limit); } return query; }
