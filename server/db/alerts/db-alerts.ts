...
export class DatabaseAlertStorage {
  ...
  async getAlertConfigurations(
    ...
    if (equipmentId) {
      conditions.push(eq(alertConfigurations.equipmentId, equipmentId));
    }
    ...
    return db.select().from(alertConfigurations);
  }
  async getAlertConfiguration(id: string, orgId?: string): Promise<AlertConfiguration | undefined> {
    const conditions = orgId
      ? and(eq(alertConfigurations.id, id), eq(alertConfigurations.orgId, orgId))
      : eq(alertConfigurations.id, id);
    const [result] = await db.select().from(alertConfigurations).where(conditions);
    return result;
  }
  ...
  async deleteAlertConfiguration(id: string, orgId?: string): Promise<void> {
    this.validateOrgId(orgId, "deleteAlertConfiguration");
    const conditions = orgId
      ? and(eq(alertConfigurations.id, id), eq(alertConfigurations.orgId, orgId))
      : eq(alertConfigurations.id, id);
    await db.delete(alertConfigurations).where(conditions);
  }
  ...
  async getAlertNotificationsPaginated(
    ...
    const countQuery = db.select({ count: sql<number>`count(*)` }).from(alertNotifications);
    const countResult =
  ...
  async getAlertNotification(id: string, orgId?: string): Promise<AlertNotification | undefined> {
    const conditions = orgId
      ? and(eq(alertNotifications.id, id), eq(alertNotifications.orgId, orgId))
      : eq(alertNotifications.id, id);
    const [result] = await db.select().from(alertNotifications).where(conditions);
    return result;
  }
  ...
  async acknowledgeAlert(
    ...
    this.validateOrgId(orgId, "acknowledgeAlert");
    const conditions = orgId
      ? and(eq(alertNotifications.id, id), eq(alertNotifications.orgId, orgId))
      : eq(alertNotifications.id, id);
    // alert_notifications has no updated_at column in canonical Postgres.
  ...
  async getAlertComments(alertId: string): Promise<AlertComment[]> {
    return db
      ...
      .orderBy(sql`${alertComments.createdAt} DESC`);
  }
  ...
  async getActiveSuppressions(orgId?: string): Promise<AlertSuppression[]> {
    ...
    if (orgId) {
      return db
        ...
        .where(and(eq(alertSuppressions.orgId, orgId), gte(alertSuppressions.suppressUntil, now)));
    }
    return db.select().from(alertSuppressions).where(gte(alertSuppressions.suppressUntil, now));
  }
  async removeAlertSuppression(id: string, orgId?: string): Promise<void> {
    this.validateOrgId(orgId, "removeAlertSuppression");
    const conditions = orgId
      ? and(eq(alertSuppressions.id, id), eq(alertSuppressions.orgId, orgId))
      : eq(alertSuppressions.id, id);
    await db.delete(alertSuppressions).where(conditions);
  }
