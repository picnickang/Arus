...
const conditionMonitoringRepo = new BaseRepository<ConditionMonitoring, InsertConditionMonitoring>(conditionMonitoring, { updatedAtColumn: "updatedAt" });
...
export class DbConditionMonitoringStorage {
  ...
  async getOilAnalysis(id: string, orgId?: string): Promise<OilAnalysis | undefined> {
    if (orgId) { const r = await oilAnalysisRepo.getById(id, orgId); return r ?? undefined; }
  ...
  async getWearParticleAnalysis(id: string, orgId?: string): Promise<WearParticleAnalysis | undefined> {
    if (orgId) { const r = await wearParticleRepo.getById(id, orgId); return r ?? undefined; }
    const result = await db.select().from(wearParticleAnalysis).where(eq(wearParticleAnalysis.id, id)).limit(1);
    return result[0];
  }
  ...
  async getConditionMonitoringRecord(id: string, orgId?: string): Promise<ConditionMonitoring | undefined> {
    if (orgId) { const r = await conditionMonitoringRepo.getById(id, orgId); return r ?? undefined; }
    const result = await db.select().from(conditionMonitoring).where(eq(conditionMonitoring.id, id)).limit(1);
    return result[0];
  }
