...
router.get("/ml/accuracy-trend", async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    sendSuccess(res, trendData.flat().sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()));
  } catch (error) { handleError(error, res, "fetch accuracy trend"); }
});

router.get("/equipment/types", async (req: AuthenticatedRequest, res: Response) => {
  sendSuccess(res, ["Engine", "Compressor", "Pump", "Generator", "Hydraulic System", "Gearbox", "Propeller", "Steering Gear", "Boiler", "Heat Exchanger"]);
});
...
router.post("/ml/train", async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const modelData: InsertMlModel = {
      orgId: req.orgId, name: `${config.algorithm} ${config.equipmentType} Predictor`, type: config.algorithm.toLowerCase(),
      status: "training", equipmentType: config.equipmentType, dataWindowDays: config.dataWindowDays,
      hyperparameters: config.hyperparameters || null, version: "1.0",
    ...
    sendSuccess(res, { modelId: newModel.id, jobId: trainingJob.id, message: "Training started successfully", estimatedCompletionMinutes: Math.ceil(config.dataWindowDays / 10) });
  } catch (error) {
    if (error instanceof z.ZodError) {return sendBadRequest(res, "Invalid training configuration", error.errors);}
