...
import { validateStateConsistency, calculateTwinAccuracy, assimilateTelemetryData } from "./physics-calculations.js";
...
export class DigitalTwinService extends EventEmitter {
  ...
  async createDigitalTwin(vesselId: string, twinType: string, name: string, specifications: VesselSpecifications, physicsModel?: PhysicsModel): Promise<DigitalTwin> {
    ...
    const defaultPhysicsModel: PhysicsModel = {
      ...
      machinery: { mainEngines: [{ id: "MAIN_ENGINE_01", power: specifications.enginePower, efficiency: 0.42 }], auxiliaryPower: specifications.enginePower * 0.15, heatExchangers: [{ id: "HE_01", capacity: 1000 }] },
      environmental: { windResistance: 0.01, currentEffect: 0.5, waveHeight: 2 },
    };
    const initialState: TwinState = {
      position: { latitude: 0, longitude: 0 }, speed: 0, heading: 0,
      draft: specifications.displacement / (specifications.length * specifications.beam * 0.7), trim: 0, list: 0,
      machinery: { engines: { MAIN_ENGINE_01: { rpm: 0, load: 0, temperature: 85 } }, generators: { GEN_01: { load: 0, voltage: 440, frequency: 60 } }, pumps: { COOLING_PUMP_01: { flow: 0, pressure: 0, status: "standby" } } },
      cargo: { totalWeight: 0, distribution: [] },
      fuel: { totalCapacity: specifications.displacement * 0.15, currentLevel: specifications.displacement * 0.12, consumptionRate: 0 },
      crew: { onboard: 20, positions: {} },
    };
    const digitalTwin = await db.insert(digitalTwins).values({ vesselId, twinType, name, specifications, physicsModel: physicsModel || defaultPhysicsModel, currentState: initialState, simulationConfig: { updateInterval: 60, realTimeSync: true, dataAssimilation: true }, validationStatus: "active", accuracy: 0.85, metadata: { createdBy: "system", modelVersion: "2.0", lastCalibration: new Date().toISOString() } }).returning();
  ...
  async updateTwinState(twinId: string, telemetryData: Record<string, any>): Promise<void> {
    ...
    try {
      ...
      await db.update(digitalTwins).set({ currentState: validatedState, lastUpdate: new Date(), accuracy: calculateTwinAccuracy(telemetryData, validatedState) }).where(eq(digitalTwins.id, twinId));
  ...
  async runSimulation(twinId: string, scenarioName: string, scenario: SimulationScenario): Promise<TwinSimulation> {
    const twin = this.activeTwins.get(twinId);
    if (!twin) { throw new Error(`Digital twin ${twinId} not found`); }
    const simulation = await db.insert(twinSimulations).values({ digitalTwinId: twinId, scenarioName, scenarioType: scenario.scenarioType, inputParameters: scenario, status: "running", progressPercentage: 0, metadata: { startedBy: "system", estimatedDuration: scenario.duration, priority: "normal" } }).returning();
  ...
  private async processSimulation(simulationId: string, twin: DigitalTwin, scenario: SimulationScenario): Promise<void> {
    try {
      ...
      while (currentStep < totalSteps) {
        ...
        results.push({ time: timeElapsed, state: simulatedState, conditions: scenario.environmentalConditions });
        currentStep++;
        await db.update(twinSimulations).set({ progressPercentage: (currentStep / totalSteps) * 100 }).where(eq(twinSimulations.id, simulationId));
        if (currentStep % 10 === 0) {await new Promise((resolve) => setImmediate(resolve));}
      }
  ...
  private async checkCriticalConditions(twinId: string, state: TwinState): Promise<void> {
    ...
    for (const [engineId, engine] of Object.entries(state.machinery.engines)) {
      if (engine.temperature > 120) { alerts.push(`Engine ${engineId} overheating: ${engine.temperature}°C`); }
      if (engine.load > 0.95) { alerts.push(`Engine ${engineId} overload: ${(engine.load * 100).toFixed(1)}%`); }
    }

    if (state.fuel.currentLevel < state.fuel.totalCapacity * 0.1) { alerts.push(`Low fuel warning: ${state.fuel.currentLevel.toFixed(1)} tons remaining`); }
    if (Math.abs(state.list) > 15) { alerts.push(`Excessive list: ${state.list.toFixed(1)}°`); }
    if (alerts.length > 0) { this.emit("critical_condition", { twinId, alerts, state }); }
  }

  private async createDefaultVisualizationAssets(twinId: string, specifications: VesselSpecifications): Promise<void> {
    const assets = [
      { assetType: "3d_model", name: `${specifications.vesselType}_hull_model`, filePath: `/models/vessels/${specifications.vesselType}_hull.gltf`, fileFormat: "gltf", targetPlatform: "web", lodLevel: 2 },
      { assetType: "texture", name: `${specifications.vesselType}_hull_texture`, filePath: `/textures/vessels/${specifications.vesselType}_hull_diffuse.png`, fileFormat: "png", targetPlatform: "web", textureResolution: "2048x2048" },
      { assetType: "ar_overlay", name: `machinery_space_overlay`, filePath: `/ar/machinery_space_markers.json`, fileFormat: "json", targetPlatform: "ar" },
    ];
    for (const asset of assets) {await db.insert(visualizationAssets).values({ ...asset, fileSizeBytes: 1024 * 1024, boundingBox: { min: [-50, -10, -200], max: [50, 30, 200] }, compressionType: "gzip", optimizationLevel: "medium", metadata: { twinId, autoGenerated: true } });}
  }

  private async loadActiveTwins(): Promise<void> {
    if (!db) { console.warn("[Digital Twin] Disabled: database not initialized"); this.enabled = false; return; }
    try {
      const twins = await db.select().from(digitalTwins).where(eq(digitalTwins.validationStatus, "active"));
      for (const twin of twins) { this.activeTwins.set(twin.id, twin); }
      console.log(`[Digital Twin] Loaded ${twins.length} active digital twins`);
    } catch (error) { console.error("[Digital Twin] Error loading active twins:", error); }
  }

  private startRealTimeUpdates(): void {
    this.updateInterval = setInterval(async () => { await this.processRealTimeUpdates(); }, 60 * 1000);
  ...
  private async processRealTimeUpdates(): Promise<void> {
    try {
      for (const [twinId, twin] of this.activeTwins.entries()) {
        ...
        if (Object.keys(telemetryData).length > 0) { await this.updateTwinState(twinId, telemetryData); }
      }
    } catch (error) { console.error("[Digital Twin] Error processing real-time updates:", error); }
  }
  ...
  private async getLatestTelemetryForVessel(_vesselId: string): Promise<Record<string, any>> {
    ...
    } catch (error) { console.error("[Digital Twin] Error getting vessel telemetry:", error); return {}; }
  }

  async getDigitalTwins(vesselId?: string): Promise<DigitalTwin[]> {
    const query = db.select().from(digitalTwins);
    if (vesselId) { return query.where(eq(digitalTwins.vesselId, vesselId)); }
  ...
  async getSimulations(twinId: string, limit: number = 50): Promise<TwinSimulation[]> {
    return db.select().from(twinSimulations).where(eq(twinSimulations.digitalTwinId, twinId)).orderBy(sql`${twinSimulations.startTime} DESC`).limit(limit);
  }

  async getVisualizationAssets(assetType?: string): Promise<VisualizationAsset[]> {
    const query = db.select().from(visualizationAssets);
    if (assetType) { return query.where(eq(visualizationAssets.assetType, assetType)); }
  ...
  async updateFuelEfficiency(twinId: string, orgId: string, telemetryData: Record<string, any>): Promise<void> {
    try {
      ...
      const characteristics = { displacement: specs.displacement ?? 1000, length: specs.length ?? 50, beam: specs.beam ?? 10, draft: specs.draft ?? 4, enginePower: specs.enginePower ?? 500, specificFuelConsumption: specs.specificFuelConsumption ?? 210, hullCondition: specs.hullCondition ?? 0.85, propellerEfficiency: specs.propellerEfficiency ?? 0.65 };
      const state = { speed: telemetryData.speed ?? 10, engineRpm: telemetryData.engineRpm ?? 1500, engineLoad: telemetryData.engineLoad ?? 60, fuelRate: telemetryData.fuelRate, propellerPitch: telemetryData.propellerPitch, trim: telemetryData.trim };
      const conditions = { windSpeed: telemetryData.windSpeed, windDirection: telemetryData.windDirection, waveHeight: telemetryData.waveHeight, seaState: telemetryData.seaState };
  ...
  getHealthStatus(): { status: string; features: string[]; stats: any } {
    return {
      status: "operational",
      features: ["vessel_digital_twins", "physics_based_simulation", "real_time_state_sync", "maintenance_scenario_modeling", "failure_impact_analysis", "route_optimization", "3d_visualization_assets", "ar_maintenance_procedures", "enhanced_fuel_efficiency_calc"],
      stats: { activeTwins: this.activeTwins.size, runningSimulations: this.simulationQueue.size, realTimeUpdates: !!this.updateInterval },
    };
  }
