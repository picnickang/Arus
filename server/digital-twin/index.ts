...
import { buildDefaultPhysicsModel, buildInitialTwinState } from "./defaults.js";
...
export class DigitalTwinService extends EventEmitter {
  ...
  constructor() {
    ...
    if (
      ...
      this.enabled = false;
      logger.info("[Digital Twin] Background startup disabled");
      return;
    }
  ...
  async createDigitalTwin(
    ...
    const defaultPhysicsModel = buildDefaultPhysicsModel(specifications);
    const initialState = buildInitialTwinState(specifications);
    ...
    if (!twin) {
      throw new Error("createDigitalTwin: insert returned no row");
    }
  ...
  async runSimulation(
    ...
    if (!sim) {
      throw new Error("runSimulation: insert returned no row");
    }
  ...
  async updateFuelEfficiency(
    ...
    try {
      ...
      const daysInService = twin.lastUpdate
        ...
        : 30;
      const prediction = (
        predictFuelConsumption as object as (
          s: typeof state,
          c: typeof characteristics,
          cond: typeof conditions,
          days: number
        ) => { predictedFuelRate: number; efficiency: number; confidence: number }
      )(state, characteristics, conditions, daysInService);
      const updatedState = {
        ...((twin.currentState as Record<string, unknown>) ?? {}),
        fuel: {
          ...(((twin.currentState as Record<string, unknown>)?.["fuel"] as Record<
            string,
            unknown
          >) ?? {}),
