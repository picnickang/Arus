...
interface PredictableModel {
  ...
  ) => number | { failureRisk?: number; failureProbability?: number };
...
function coerceProb(value: unknown): number {
  ...
  if (value && typeof value === "object") {
    ...
    if (typeof v.failureRisk === "number") {
      return coerceProb(v.failureRisk);
    }
    if (typeof v.failureProbability === "number") {
      return coerceProb(v.failureProbability);
    }
  }
  return 0.1;
}
...
export async function storeFeatureImportances(
  ...
  if (!explanation.topFeatures.length) {
    return;
  }
