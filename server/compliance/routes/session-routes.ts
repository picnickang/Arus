...
router.get('/sessions', requireComplianceAccess, async (req: Request, res: Response) => {
  try {
    ...
    res.json({ success: true, data: sessions.map(s => ({ id: s.id, createdAt: s.createdAt, lastActivityAt: s.lastActivityAt, expiresAt: s.expiresAt, ipAddress: s.ipAddress, userAgent: s.userAgent, deviceFingerprint: s.deviceFingerprint, isRevoked: s.isRevoked, mfaVerified: s.mfaVerified })) });
  } catch (error) {
...
router.post('/sessions/validate', requireComplianceAccess, async (req: Request, res: Response) => {
  try {
    ...
    const result = await sessionManagementService.validateSession(sessionToken, { updateActivity: false, orgId });
    res.json({ success: true, valid: result.valid, expired: result.expired, revoked: result.revoked, inactivityTimeout: result.inactivityTimeout, error: result.error });
  } catch (error) {
...
router.post('/sessions/revoke-all', requireComplianceAccess, async (req: Request, res: Response) => {
  try {
    const orgId = req.headers['x-org-id'] as string;
    ...
    if (!orgId) { return res.status(401).json({ error: 'Organization ID required' }); }
    if (!userId) { return res.status(400).json({ error: 'User ID required' }); }
    await sessionManagementService.revokeAllUserSessions(orgId, userId, adminId, reason || 'Admin bulk revocation');
...
router.get('/login-events', requireComplianceAccess, async (req: Request, res: Response) => {
  try {
    const orgId = req.headers['x-org-id'] as string;
    if (!orgId) { return res.status(401).json({ error: 'Organization ID required' }); }
    const events = await sessionManagementService.getLoginEvents(orgId, { limit: Number.parseInt(req.query.limit as string) || 100, offset: Number.parseInt(req.query.offset as string) ?? 0 });
...
router.post('/sessions/suspicious', requireComplianceAccess, async (req: Request, res: Response) => {
  try {
    const orgId = req.headers['x-org-id'] as string;
    if (!orgId) { return res.status(401).json({ error: 'Organization ID required' }); }
    const { sessionId, flagType, metadata } = req.body;
    if (!sessionId) { return res.status(400).json({ error: 'Session ID required' }); }
    await sessionManagementService.flagSuspiciousSession(sessionId, flagType || 'manual_review', metadata);
