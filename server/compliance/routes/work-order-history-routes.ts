...
router.post('/work-orders/:workOrderId/history/verify', requireComplianceAccess, async (req: Request, res: Response) => {
  try {
...
router.post('/work-orders/history/verify-all', requireComplianceAccess, async (req: Request, res: Response) => {
  try {
    const orgId = req.headers['x-org-id'] as string;
    if (!orgId) { return res.status(401).json({ error: 'Organization ID required' }); }
    const { workOrderIds } = req.body;
    if (!workOrderIds || !Array.isArray(workOrderIds)) {return res.status(400).json({ error: 'Work order IDs array required' });}
    const results = await Promise.all(workOrderIds.map(async (id: string) => ({ workOrderId: id, ...(await workOrderHistoryHashService.verifyWorkOrderHistory(id, orgId)) })));
...
router.get('/work-orders/:workOrderId/history/stats', requireComplianceAccess, async (req: Request, res: Response) => {
  try {
    const orgId = req.headers['x-org-id'] as string;
    const { workOrderId } = req.params;
    if (!orgId) { return res.status(401).json({ error: 'Organization ID required' }); }
