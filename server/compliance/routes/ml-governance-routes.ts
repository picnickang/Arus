...
import {
  ...
  getEngineerOverrides as getProvenanceOverrides,
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      await recordEngineerOverride({
        overrideId: override.id,
        predictionId: override.predictionId || undefined,
        equipmentId: override.equipmentId,
        vesselId: undefined,
        workOrderId: override.workOrderId || undefined,
        overrideType: override.overrideType as "defer" | "escalate" | "dismiss" | "modify",
        originalRiskLevel: override.originalRiskLevel,
        newRiskLevel: override.newRiskLevel || undefined,
        originalConfidence: override.originalConfidence || undefined,
        justification: override.justification,
        engineerId: override.engineerId,
        engineerName: override.engineerName,
        engineerCertifications: override.engineerCertifications || undefined,
        originalPrediction: override.originalPrediction as Record<string, unknown>,
        orgId,
      });
      await auditService.logEvent({
        ...
        newState: {
          ...
          originalRiskLevel: override.originalRiskLevel,
          newRiskLevel: override.newRiskLevel,
          engineerId: override.engineerId,
          engineerName: override.engineerName,
        },
        performedBy: override.engineerId,
...
router.patch(
  "/ml-governance/overrides/:id/outcome",
  requireAdminAuth,
    ...
    try {
      ...
      await recordOverrideOutcome({
        ...
        originalOverrideType: existingOverride.overrideType as
          ...
          | "dismiss"
          | "modify",
        outcomeStatus: validatedData.outcomeStatus as
          | "pending"
          | "validated"
          | "failure_prevented"
          | "failure_occurred",
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const overrides = await getProvenanceOverrides({
        orgId,
        equipmentId: equipmentId as string,
        engineerId: engineerId as string,
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      for (const o of overrides) {
        ...
        if (o.outcomeStatus) {
          stats.byOutcome[o.outcomeStatus] = (stats.byOutcome[o.outcomeStatus] ?? 0) + 1;
        }
