...
router.post('/export/anonymized', requireAdminAuth, auditAdminAction('compliance_export_anonymized'), requireComplianceAccess, async (req: Request, res: Response) => {
  try {
    ...
    console.log(`[Compliance] Starting anonymized export for org: ${orgId}, level: ${options.anonymize}`);
    await auditService.logEvent({ orgId, eventCategory: 'compliance_event', eventType: 'data_export_initiated', entityType: 'organization', entityId: orgId, performedBy: exportedBy, performedByType: 'user', metadata: { anonymizationLevel: options.anonymize, includeTelemetry: options.includeTelemetry, telemetryDays: options.telemetryDays }, retentionRequired: true });
    const result = await exportService.exportOrg(orgId, { includeTelemetry: options.includeTelemetry, telemetryDays: options.telemetryDays, includeKnowledgeBase: options.includeKnowledgeBase, includeAuditLogs: options.includeAuditLogs, anonymize: options.anonymize as AnonymizationLevel, anonymizationConfig: { preserveIds: options.preserveIds, preserveTimestamps: options.preserveTimestamps, preserveTechnicalData: options.preserveTechnicalData } }, exportedBy);
    if (result.success) {
      await auditService.logEvent({ orgId, eventCategory: 'compliance_event', eventType: 'data_export_completed', entityType: 'organization', entityId: orgId, performedBy: exportedBy, performedByType: 'user', metadata: { exportId: result.exportId, duration: result.duration }, retentionRequired: true });
      res.json({ success: true, data: { exportId: result.exportId, filePath: result.filePath, duration: result.duration, anonymizationLevel: options.anonymize, manifest: result.manifest } });
    } else {
      res.status(500).json({ success: false, error: result.error, exportId: result.exportId });
    }
  } catch (error) {
    console.error('[Compliance] Anonymized export error:', error);
    res.status(500).json({ error: 'Failed to create anonymized export', details: error instanceof Error ? error.message : 'Unknown error' });
  }
});

router.get('/export/anonymized/:exportId', requireAdminAuth, auditAdminAction('compliance_export_download'), requireComplianceAccess, async (req: Request, res: Response) => {
  try {
    const orgId = req.headers['x-org-id'] as string;
    ...
    if (!fs.existsSync(exportPath)) { return res.status(404).json({ error: 'Export not found or expired' }); }
    await auditService.logEvent({ orgId, eventCategory: 'compliance_event', eventType: 'data_export_downloaded', entityType: 'data_export', entityId: exportId, performedBy: (req as AdminRequest).adminId || 'admin', performedByType: 'user', retentionRequired: true });
    res.download(exportPath, `${exportId}.tar.gz`);
  } catch (error) {
    console.error('[Compliance] Export download error:', error);
    res.status(500).json({ error: 'Failed to download export' });
  }
});

router.get('/anonymization/preview', requireAdminAuth, requireComplianceAccess, async (req: Request, res: Response) => {
  try {
    const level = (req.query.level as AnonymizationLevel) || 'full';
    const sampleRecord = { id: 'sample-123', name: 'John Doe', email: 'john.doe@example.com', phone: '+1-555-123-4567', address: '123 Main Street, City, State 12345', passportNumber: 'AB1234567', emergencyContact: 'Jane Doe', emergencyPhone: '+1-555-987-6543', notes: 'Sample notes with sensitive information', role: 'Chief Engineer', status: 'active', createdAt: new Date().toISOString() };
    const anonymizationService = new DataAnonymizationService();
    const { record: anonymized, result } = anonymizationService.anonymizeRecord(sampleRecord, 'crew_member', level, { preserveIds: true, preserveTimestamps: true, preserveTechnicalData: false });
    res.json({ success: true, data: { original: sampleRecord, anonymized, anonymizationResult: result, level } });
  } catch (error) {
    console.error('[Compliance] Anonymization preview error:', error);
    res.status(500).json({ error: 'Failed to preview anonymization' });
  }
});

router.get('/dsar', requireAdminAuth, auditAdminAction('dsar_list'), requireComplianceAccess, async (req: Request, res: Response) => {
  try {
    const orgId = req.headers['x-org-id'] as string;
    if (!orgId) { return res.status(401).json({ error: 'Organization ID required' }); }
    ...
    console.error('[Compliance] DSAR list error:', error);
    res.status(500).json({ error: 'Failed to retrieve DSAR requests' });
  }
});

router.get('/dsar/:id', requireAdminAuth, auditAdminAction('dsar_view'), requireComplianceAccess, async (req: Request, res: Response) => {
  try {
    const orgId = req.headers['x-org-id'] as string;
    const { id } = req.params;
    if (!orgId) { return res.status(401).json({ error: 'Organization ID required' }); }
    ...
    console.error('[Compliance] DSAR get error:', error);
    res.status(500).json({ error: 'Failed to retrieve DSAR request' });
  }
});

router.post('/dsar', requireAdminAuth, auditAdminAction('dsar_create'), requireComplianceAccess, async (req: Request, res: Response) => {
  try {
    const orgId = req.headers['x-org-id'] as string;
    if (!orgId) { return res.status(401).json({ error: 'Organization ID required' }); }
    ...
    console.error('[Compliance] DSAR create error:', error);
    res.status(500).json({ error: 'Failed to create DSAR request' });
  }
});

router.post('/dsar/:id/acknowledge', requireAdminAuth, auditAdminAction('dsar_acknowledge'), requireComplianceAccess, async (req: Request, res: Response) => {
  try {
    const orgId = req.headers['x-org-id'] as string;
    const { id } = req.params;
    if (!orgId) { return res.status(401).json({ error: 'Organization ID required' }); }
    ...
    console.error('[Compliance] DSAR acknowledge error:', error);
    res.status(500).json({ error: 'Failed to acknowledge DSAR request' });
  }
});

router.post('/dsar/:id/collect', requireAdminAuth, auditAdminAction('dsar_data_collection'), requireComplianceAccess, async (req: Request, res: Response) => {
  try {
    const orgId = req.headers['x-org-id'] as string;
    const { id } = req.params;
    ...
    const hasData = (obj: Record<string, unknown>, key: string): boolean => {
      const val = obj[key];
      return Array.isArray(val) ? val.length > 0 : (typeof val === 'object' && val !== null ? Object.keys(val).length > 0 : false);
    };
    await auditService.logEvent({ orgId, eventCategory: 'compliance_event', eventType: 'dsar_data_collected', entityType: 'dsar', entityId: id, performedBy: (req as AdminRequest).adminId || 'admin', performedByType: 'user', metadata: { identifier, identifierType: type, dataCategories: Object.keys(collectedDataObj).filter(k => hasData(collectedDataObj, k)) }, retentionRequired: true });
    res.json({ success: true, data: { dsarId: id, identifier, identifierType: type, collectedData, summary: { users: getArrayLength(collectedDataObj, 'users'), crewMembers: getArrayLength(collectedDataObj, 'crewMembers'), restRecords: getArrayLength(collectedDataObj, 'restRecords'), workOrders: getArrayLength(collectedDataObj, 'workOrders'), auditEvents: getArrayLength(collectedDataObj, 'auditEvents') } } });
  } catch (error) {
    console.error('[Compliance] DSAR data collection error:', error);
    res.status(500).json({ error: 'Failed to collect data for DSAR' });
  }
});

router.post('/dsar/:id/execute-erasure', requireAdminAuth, auditAdminAction('dsar_erasure'), requireComplianceAccess, async (req: Request, res: Response) => {
  try {
    const orgId = req.headers['x-org-id'] as string;
    const { id } = req.params;
    ...
    console.error('[Compliance] DSAR erasure error:', error);
    res.status(500).json({ error: 'Failed to execute data erasure' });
  }
});

router.post('/dsar/:id/complete', requireAdminAuth, auditAdminAction('dsar_complete'), requireComplianceAccess, async (req: Request, res: Response) => {
  try {
    const orgId = req.headers['x-org-id'] as string;
    const { id } = req.params;
    ...
    console.error('[Compliance] DSAR complete error:', error);
    res.status(500).json({ error: 'Failed to complete DSAR request' });
  }
});

router.post('/dsar/:id/reject', requireAdminAuth, auditAdminAction('dsar_reject'), requireComplianceAccess, async (req: Request, res: Response) => {
  try {
    const orgId = req.headers['x-org-id'] as string;
    const { id } = req.params;
    ...
    console.error('[Compliance] DSAR reject error:', error);
    res.status(500).json({ error: 'Failed to reject DSAR request' });
  }
});

router.get('/dsar/statistics', requireAdminAuth, requireComplianceAccess, async (req: Request, res: Response) => {
  try {
    const orgId = req.headers['x-org-id'] as string;
    if (!orgId) { return res.status(401).json({ error: 'Organization ID required' }); }
    ...
    for (const req of allRequests) {
      ...
      if (req.completedAt && req.createdAt) { completedDays += (new Date(req.completedAt).getTime() - new Date(req.createdAt).getTime()) / (1000 * 60 * 60 * 24); completedCount++; }
      if (req.status !== 'completed' && req.status !== 'rejected' && req.dueDate && new Date(req.dueDate) < now) {stats.overdueCount++;}
    }
    stats.avgCompletionDays = completedCount > 0 ? Math.round(completedDays / completedCount * 10) / 10 : 0;
    res.json({ success: true, data: stats });
  } catch (error) {
    console.error('[Compliance] DSAR statistics error:', error);
