...
export function registerDataPrivacyDsarRoutes(router: Router): void {
  ...
  // `/dsar/statistics` is a literal path and must be declared before the
  // `/dsar/:id` route below; otherwise ":id" captures "statistics" and the
  // stats endpoint 404s as a missing DSAR request.
  router.get(
    "/dsar/statistics",
    requireAdminAuth,
    requireComplianceAccess,
