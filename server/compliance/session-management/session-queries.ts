...
export async function cleanupExpiredSessions(): Promise<number> {
  ...
  const expiredSessions = await db
    ...
    .from(userSessions)
    .where(and(lt(userSessions.expiresAt, now), eq(userSessions.isRevoked, false)));

  await db
    .update(userSessions)
    .set({ isRevoked: true, revokedAt: now, revokedBy: "system", revokedReason: "Session expired" })
    .where(and(lt(userSessions.expiresAt, now), eq(userSessions.isRevoked, false)));
...
export async function getUserSessions(orgId: string, userId: string): Promise<UserSession[]> {
  return db
    ...
    .orderBy(sql`${userSessions.lastActivityAt} DESC`);
}
...
export async function getSessionByRefreshToken(refreshTokenHash: string) {
  const sessions = await db
    .select()
    .from(userSessions)
    .where(eq(userSessions.refreshToken, refreshTokenHash))
    .limit(1);
  return sessions[0];
}
