...
export async function searchEquipment(query: string, orgId: string, limit: number): Promise<SearchResult[]> {
  ...
  if (vesselIds.length > 0) {
    const vesselData = await db.select({ id: vessels.id, name: vessels.name }).from(vessels).where(and(eq(vessels.orgId, orgId), sql`${vessels.id} = ANY(${vesselIds})`));
...
export async function searchAlerts(query: string, orgId: string, limit: number): Promise<SearchResult[]> {
  ...
  if (equipmentIds.length > 0) {
    const equipmentData = await db.select({ id: equipment.id, name: equipment.name }).from(equipment).where(and(eq(equipment.orgId, orgId), sql`${equipment.id} = ANY(${equipmentIds})`));
