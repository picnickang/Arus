...
export function calculateBurnRate(availability: number, availabilityTarget: number): number {
  ...
  if (errorBudget === 0) { return availability < 1 ? Infinity : 0; }
  const actualErrorRate = 1 - availability;
  return actualErrorRate / errorBudget;
}

export function recordLatencySample(route: string, durationMs: number, isError: boolean = false): void {
  ...
  if (isError) { currentBucket.errorCount++; }
