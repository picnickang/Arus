...
export function prepareSequences(data: TimeSeriesFeatures[], sequenceLength: number, featureNames: string[]): { sequences: number[][][]; labels: number[] } {
  const sequences: number[][][] = [];
  const labels: number[] = [];

  const equipmentGroups = new Map<string, TimeSeriesFeatures[]>();
  for (const point of data) {
    if (!equipmentGroups.has(point.equipmentId)) {equipmentGroups.set(point.equipmentId, []);}
  ...
  for (const [_, points] of equipmentGroups.entries()) {
    ...
    for (let i = sequenceLength; i < points.length; i++) {
      ...
      for (let j = i - sequenceLength; j < i; j++) {
        ...
        for (const featureName of featureNames) {features.push(points[j].features[featureName] ?? 0);}
