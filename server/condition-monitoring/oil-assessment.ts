...
function assessViscosity(oil: OilAnalysis, state: AssessmentState): void {
  if (!oil.viscosity40C || !oil.viscosityIndex) {return;}
  if (Math.abs(oil.viscosityIndex - 100) <= 15) {return;}
...
function assessWaterContent(oil: OilAnalysis, state: AssessmentState): void {
  if (!oil.waterContent || oil.waterContent <= 0.05) {return;}
  ...
  if (oil.waterContent > 0.1) {state.changeNeeded = true;}
}

function assessFuelDilution(oil: OilAnalysis, state: AssessmentState): void {
  if (!oil.fuelDilution || oil.fuelDilution <= 2) {return;}
  ...
  if (oil.fuelDilution > 5) {state.changeNeeded = true;}
}
...
function assessWearMetals(oil: OilAnalysis, state: AssessmentState): void {
  ...
  if (totalExcess > 2) {state.changeNeeded = true;}
}

function assessAdditives(oil: OilAnalysis, state: AssessmentState): void {
  if (!oil.calcium || !oil.zinc) {return;}
...
function assessOxidation(oil: OilAnalysis, state: AssessmentState): void {
  if (oil.oxidation && oil.oxidation > 20) {
    ...
    if (oil.oxidation > 50) {state.changeNeeded = true;}
  }
  if (oil.acidNumber && oil.acidNumber > 2.5) {
    ...
    if (oil.acidNumber > 4) {state.changeNeeded = true;}
  }
}
...
function calculateRemainingLife(condition: "normal" | "marginal" | "critical", serviceHours?: number | null): number {
  if (condition === "critical") {return 30;}
  if (condition === "marginal") {return 90;}
...
export function assessOilCondition(oilAnalysis: OilAnalysis): OilConditionAssessment {
  const state: AssessmentState = { scores: { viscosity: 100, contamination: 100, wearMetals: 100, additive: 100, oxidation: 100 }, concerns: [], recs: [], changeNeeded: false };
  ...
  const overallScore = Math.round(state.scores.viscosity * 0.25 + state.scores.contamination * 0.25 + state.scores.wearMetals * 0.25 + state.scores.additive * 0.15 + state.scores.oxidation * 0.1);
  ...
  return {
    overallScore, viscosityScore: Math.round(state.scores.viscosity), contaminationScore: Math.round(state.scores.contamination),
    wearMetalsScore: Math.round(state.scores.wearMetals), additiveScore: Math.round(state.scores.additive), oxidationScore: Math.round(state.scores.oxidation),
    condition, primaryConcerns: state.concerns, recommendations: state.recs, changeRecommended: state.changeNeeded, estimatedRemainingLife: Math.round(estimatedRemainingLife)
  };
}
