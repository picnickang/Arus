...
export class VesselIntelligenceService {
  ...
  async getHistoricalContext(vesselId: string): Promise<HistoricalContext> {
    ...
    const maintenanceHistory = {
      scheduled: workOrders.filter(
        (wo) => (wo as Record<string, unknown>)["workOrderType"] === "scheduled"
      ).length,
      unscheduled: workOrders.filter(
        (wo) => (wo as Record<string, unknown>)["workOrderType"] === "unscheduled"
      ).length,
      emergency: workOrders.filter((wo) => (wo.priority as unknown) === "critical").length,
      preventive: workOrders.filter(
        (wo) => (wo as Record<string, unknown>)["workOrderType"] === "preventive"
      ).length,
    };
