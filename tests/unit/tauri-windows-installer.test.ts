...
type TauriConf = {
  ...
  app: { windows: { label: string; title: string; width: number; height: number; minWidth: number; minHeight: number }[]; security: { csp: string } };
  bundle: {
    active: boolean;
    targets: string;
    icon: string[];
...
describe('Tauri Windows Installer — Configuration', () => {
  ...
  it('vessel uses offlineInstaller while cloud uses downloadBootstrapper', () => {
    expect(configs['src-tauri/tauri.vessel.conf.json'].bundle.windows.nsis.installWebview2Mode).toBe('offlineInstaller');
    expect(configs['src-tauri/tauri.cloud.conf.json'].bundle.windows.nsis.installWebview2Mode).toBe('downloadBootstrapper');
  });
...
describe('Tauri Windows Installer — WiX Service Fragment', () => {
  let wxs: string;
  beforeAll(() => { wxs = read('src-tauri/windows/wix/service-component.wxs'); });
  ...
  describe('Install lifecycle — Custom Actions', () => {
    ...
    it('install actions use "NOT Installed AND NOT REMOVE" condition', () => {
      const installSequence = wxs.match(
        /Action="CreateARUSDataDir"[^>]*>([^<]+)</
      );
    ...
    it('install chain is ordered (each After= references the previous)', () => {
      ...
      const sequenceBlock = wxs.match(/<InstallExecuteSequence>([\s\S]*?)<\/InstallExecuteSequence>/);
...
describe('Tauri Windows Installer — Capabilities', () => {
  ...
  beforeAll(() => { caps = json('src-tauri/capabilities/default.json'); });
  ...
  it('shell:allow-execute restricts to arus-server sidecar only', () => {
    const exec = caps.permissions.find(
      (p) => typeof p === 'object' && (p as Record<string, unknown>).identifier === 'shell:allow-execute'
...
describe('Tauri Windows Installer — Rust Sidecar Logic', () => {
  let libRs: string;
  beforeAll(() => { libRs = read('src-tauri/src/lib.rs'); });
...
describe('Tauri Windows Installer — Cargo.toml Dependencies', () => {
  let cargo: string;
  beforeAll(() => { cargo = read('src-tauri/Cargo.toml'); });
