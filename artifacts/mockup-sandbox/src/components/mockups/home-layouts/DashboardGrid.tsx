      ...
      <header className="bg-slate-900 border-b border-slate-800 px-4 py-2 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            <Badge
              variant="outline"
              className="bg-slate-800 text-cyan-300 border-cyan-900 ml-2 py-0 h-5"
            >
              {MOCK_DATA.role}
            </Badge>
          </div>
                ...
                <tbody className="divide-y divide-slate-800/50">
                  {MOCK_DATA.tasks.map((task, idx) => (
                    <tr key={idx} className="hover:bg-slate-800/50 transition-colors group">
                      ...
                      <td className="px-4 py-2.5 font-medium text-slate-200">{task.title}</td>
                      <td className="px-4 py-2.5 text-slate-400">{task.equipment}</td>
