...
import { db } from '../db';
import { loadPartUsageHistory, loadPartCosts, loadCurrentStock, optimizeInventoryLevels } from '../inventory';
...
async function saveOptimizationResults(
  orgId: string,
  jobId: string,
  results: any[]
) {
      ...
      for (const job of jobs) {
        try {
          ...
          const parts = await db
            ...
            .where(
              and(
                eq(parts.orgId, job.orgId),
                inArray(parts.category, job.partCategories)
              )
            );
          ...
          if (job.notifyOnCritical) {
            const criticalItems = results.filter(
              r => r.recommendation === 'critical_reorder'
            );
          ...
          console.log(`[Scheduled Optimization] Completed job ${job.id}: ${results.length} parts optimized`);
        } catch (error) {
