...
| **3** | Telemetry/sensing: `devices`, `telemetry` (partial→full), `sensors` full (landed); `sensor-management` remains; `iot-processing` flat (no storage — orchestration, deferred) | Highest-volume context layered |
