...
| **2** | High-traffic flat domains: `notifications`, `condition-monitoring`, `compliance` (landed), `logbook` corrections slice (landed; deck/engine routes flat support); `scheduling`, `sync` remain (cross-domain storage → need composition seam) | Largest leak concentration removed |
