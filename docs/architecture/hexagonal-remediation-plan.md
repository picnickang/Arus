...
| **2** | High-traffic flat domains: `notifications` (landed), `logbook` corrections slice (landed; deck/engine routes flat support); `compliance`, `condition-monitoring`, `scheduling`, `sync` remain | Largest leak concentration removed |
