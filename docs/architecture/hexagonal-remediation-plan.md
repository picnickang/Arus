...
| **4** | ML/analytics: `insights` + `ml-pipeline` leak-decoupled (landed; flat); `equipment` partial — lifecycle slice + all hex-storage entries cleared (landed; repository cross-domain + routes→interfaces remain for full); `ml-analytics`, `pdm-platform` remain | Predictive path layered |
