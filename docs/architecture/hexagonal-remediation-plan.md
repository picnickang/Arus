...
| **4** | ML/analytics: `insights` + `ml-pipeline` leak-decoupled via composition (landed; flat — no own storage); `ml-analytics`, `pdm-platform`, flat `equipment` family remain | Predictive path layered |
