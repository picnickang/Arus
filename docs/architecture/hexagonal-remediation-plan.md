...
| **4** | ML/analytics: `insights` + `ml-pipeline` leak-decoupled (landed; flat); `equipment` (landed) — lifecycle slice + all hex-storage + cross-domain entries cleared (partial; routes→interfaces remain for full); `ml-analytics` (landed) **partial→full**; `pdm-platform` (landed) **partial→full** — meta-domain already leak-free + storage-confined; added aggregated top-level `domain/`/`application/`/`interfaces/` barrels over its layered sub-contexts, registry now mounts the 15 sub-routers via the `interfaces/` barrel | Predictive path layered |
