...
| **5** | Remaining flat tail + partials: `workflow` (landed) **partial→full** — top-level `infrastructure/` barrel aggregating the operator-experience sub-context adapters; `system-admin` (landed) **partial→full** — `routes/`→`interfaces/`, `ThresholdCalibrator` port extracted to `domain/`, telemetry-simulation writer decoupled to `composition/system-admin-telemetry-data.ts` (B−1, C−1); `me-portal` (landed) **partial→full** — service/task-feed→`application/`, routes→`interfaces/`, `domain/` ports added, vessels/system-admin/alerts/maintenance/work-order reads decoupled to `composition/me-portal-data.ts` (C−3); `permissions` (large, ~48 importers — dedicated pass; **slice 1 landed**: `repository.ts`→`infrastructure/` with root shim, service raw-db reads relocated to `infrastructure/repository-access-queries.ts` so the service layer is db-free — hex-storage 141→139; application/interfaces/domain layering remains) and the flat tail remain; resolve the 5 cross-domain import allowlist entries | `flat`/`partial` → near zero |
