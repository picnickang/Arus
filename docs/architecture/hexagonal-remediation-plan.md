...
| **4** | ML/analytics: `insights` + `ml-pipeline` leak-decoupled (landed; flat); `equipment` (landed) — lifecycle slice + all hex-storage + cross-domain entries cleared (partial; routes→interfaces remain for full); `ml-analytics` (landed) **partial→full** — routes→`interfaces/`, application service + domain ports added, all cross-domain storage moved to `composition/ml-analytics-data.ts`; `pdm-platform` remains | Predictive path layered |
