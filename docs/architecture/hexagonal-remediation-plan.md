...
| **4** | ML/analytics: `insights` + `ml-pipeline` leak-decoupled (landed; flat); `equipment` lifecycle slice → partial (landed; core routes/service remain flat); `ml-analytics`, `pdm-platform` remain | Predictive path layered |
