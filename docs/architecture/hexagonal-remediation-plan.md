...
| **2** | High-traffic flat domains: `notifications`, `condition-monitoring` (landed), `logbook` corrections slice (landed; deck/engine routes flat support); `compliance`, `scheduling`, `sync` remain | Largest leak concentration removed |
