...
**Source:** Enterprise-Grade Audit (Nov 2026) §16 "Final Verdict" — *"ARUS is two strategic engineering pushes away from being genuinely best-in-class."*
...
2. **A1.6 (closed-loop retraining cron) needs B3.2 (event spine)** for clean outcome→retraining triggers, *but* can ship without it by polling the `prediction_outcomes` table.
