...
| ---- | -------------------------------------------------------------- | -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| A1.1 | **ONNX runtime integration**                                   | —                    | Keep `onnxruntime-node` for server inference. Define `MLInferenceAdapter` port in `server/domains/pdm-platform/ports.ts`. Wire to existing `ml_models` registry (Wave 3.2). Browser offline scoring remains a later client adapter, not an installed dependency today. | A no-op ONNX model loads and serves predictions through the existing `serveWithShadowOrCanary` (Wave 3.3) path. No UX change.             |
