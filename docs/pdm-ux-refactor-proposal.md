...
export class InsightTranslator {
  ...
  private generateExplanation(
    ...
    const confidenceText = prediction.confidence > 0.8 
...
app.get("/api/insights/fleet-overview", async (req, res) => {
  try {
    ...
    const fleetOverview = await Promise.all(vessels.map(async (vessel) => {
      ...
      const systemStatus = await Promise.all(
        Object.entries(systemGroups).map(async ([systemType, equipment]) => {
          ...
          // Aggregate status
          const criticalCount = componentInsights.filter(i => i.statusLevel === 'critical').length;
          const actionCount = componentInsights.filter(i => i.statusLevel === 'action_required').length;
          const monitorCount = componentInsights.filter(i => i.statusLevel === 'monitor').length;
          
          const overallStatus = criticalCount > 0 ? 'critical'
            : actionCount > 0 ? 'action_required'
            : monitorCount > 0 ? 'monitor'
            : 'normal';
...
4. **Trustworthy** - Explains *why* it's making recommendations
...
*Document prepared by ARUS MLOps & UX Engineering Team*  
