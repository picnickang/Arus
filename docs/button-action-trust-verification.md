...
real**. The "removed as dead" report reflects an _intermediate_ state — git
...
| Unauthorized access fails correctly     | ✅ — the routes that _do_ exist are `requireAdminAuth`-gated; the missing routes simply 404.                                                                                                                                                                                         |
...
The residual gap is purely _silence_ on the near-unreachable empty/failure paths.
