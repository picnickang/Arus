...
**How to apply:** If a task asks to _hide/show_ hubs or control admin-portal navigation → frontend policy + the `hubAdmin`/`hubAccess` grant. If a task asks to _block data/actions_ → server RBAC (`requirePermission`/`requireRole`/role changes), do not rely on hub gating.
