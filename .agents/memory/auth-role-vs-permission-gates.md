...
**Why:** dev grants superuser *permissions* but the *role name* must also be an admin-portal role for role-name gates to agree. The dev auth bypass already authenticates as the admin identity `dev-admin-user`, so the dev role is set to `system_admin`.
