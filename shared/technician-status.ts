/**
 * Technician-Friendly Status System
 * Translates technical ML outputs into color-coded, actionable status levels
 * for non-technical vessel technicians and operations staff
 */

export type StatusLevel = "normal" | "monitor" | "action_required" | "critical";
export type StatusColor = "green" | "yellow" | "orange" | "red";
export type Priority = "monitor" | "scheduled" | "urgent" | "immediate";
...
/**
 * Color-coded status system for equipment health
 */
...
/**
 * System type categorization for equipment hierarchy
 */
export const SYSTEM_TYPES: Record<string, SystemTypeDefinition> = {
  propulsion: {
    label: "Propulsion",
    icon: "engine",
    color: "#3b82f6", // blue
...
/**
 * Determine status level from ML prediction
 */
export function determineStatusLevel(failureProbability: number, confidence: number): StatusLevel {
...
/**
 * Get priority from status level
 */
export function getPriorityFromStatus(status: StatusLevel): Priority {
  const priorityMap: Record<StatusLevel, Priority> = {
    critical: "immediate",
 ...
 */
export function getStatusColorClass(status: StatusLevel): string {
  return STATUS_DEFINITIONS[status].borderClass;
}

/**
 * Get system type display info
 */
export function getSystemTypeInfo(systemType: string): SystemTypeDefinition {
  return (
...
/**
 * Format failure probability as plain language
 */
export function formatFailureProbability(probability: number): string {
