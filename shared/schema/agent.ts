...
export const agentConfig = pgTable(
  ...
  (table) => [index("idx_agent_config_org").on(table.orgId)]
);
...
export const insertAgentDraftSchema = createInsertSchema(agentDrafts).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
  reviewedById: true,
...
export const insertAgentBriefingSchema = createInsertSchema(agentBriefings).omit({
  id: true,
  createdAt: true,
});
