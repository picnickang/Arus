...
(async () => {
  ...
  expect("POST /api/work-orders/:id/service-requests returns 200/201", [200, 201].includes(create.status), `status=${create.status}`);
  ...
  const approve = await http("POST", `/api/service-requests/${srId}/approve`, { reviewNotes: "ok" });
  expect("POST /approve returns 200", approve.status === 200, `status=${approve.status}`);
  expect("SR status is approved", approve.body?.status === "approved", `status=${approve.body?.status}`);
  ...
  expectT(["T007"], "PATCH approved SR returns 200", edit1.status === 200, `status=${edit1.status}`);
  ...
  expectT(["T007"], "approved SR persisted new estimatedCost (list)", Number(srAfterEdit?.estimatedCost) === 1500, `cost=${srAfterEdit?.estimatedCost}`);
  const srDetailAfterEdit = await http("GET", `/api/service-requests/${srId}`);
  expectT(["T007"], "GET /api/service-requests/:id after edit returns 200", srDetailAfterEdit.status === 200, `status=${srDetailAfterEdit.status}`);
  // Detail endpoint returns snake_case; accept either casing for resilience.
  const detailSd = srDetailAfterEdit.body?.serviceDetails ?? srDetailAfterEdit.body?.service_details;
  const detailSr = srDetailAfterEdit.body?.specialRequirements ?? srDetailAfterEdit.body?.special_requirements;
  expectT(["T007"], "approved SR persisted new serviceDetails (detail)", detailSd === "Updated scope after approval", `value=${JSON.stringify(detailSd)}`);
  expectT(["T007"], "approved SR cleared specialRequirements to null (detail)", detailSr == null, `value=${JSON.stringify(detailSr)}`);
  ...
  expectT(["T001"], "SO row carries originatingRequestId", soRow?.originatingRequestId === srId, `got=${soRow?.originatingRequestId}`);
  expectT(["T001"], "SO row carries originatingRequestNumber", soRow?.originatingRequestNumber === srNumber, `got=${soRow?.originatingRequestNumber}`);
  expectT(["T001"], "SO row carries originatingRequestStatus", typeof soRow?.originatingRequestStatus === "string", `got=${soRow?.originatingRequestStatus}`);

  // SO detail must expose ALL three back-pointer fields (not just id).
  const soDetail = await http("GET", `/api/service-orders/${soId}`);
  expectT(["T001"], "GET /api/service-orders/:id returns 200", soDetail.status === 200, `status=${soDetail.status}`);
  expectT(["T001"], "SO detail carries originatingRequestId", soDetail.body?.originatingRequestId === srId, `got=${soDetail.body?.originatingRequestId}`);
  expectT(["T001"], "SO detail carries originatingRequestNumber", soDetail.body?.originatingRequestNumber === srNumber, `got=${soDetail.body?.originatingRequestNumber}`);
  expectT(["T001"], "SO detail carries originatingRequestStatus", typeof soDetail.body?.originatingRequestStatus === "string", `got=${soDetail.body?.originatingRequestStatus}`);
  ...
  expectT(["T004"], "converted SR row has serviceOrderId", srRow?.serviceOrderId === soId, `got=${srRow?.serviceOrderId}`);
  expectT(["T004"], "converted SR row has serviceOrderNumber", srRow?.serviceOrderNumber === soNumber, `got=${srRow?.serviceOrderNumber}`);
  expectT(["T004"], "converted SR row has serviceOrderStatus", typeof srRow?.serviceOrderStatus === "string", `got=${srRow?.serviceOrderStatus}`);
  ...
  expectT(["T003"], "T003 data prereq: SO carries SR cross-link for 'From SR-…' badge", soRow?.originatingRequestId === srId && soRow?.originatingRequestNumber === srNumber);
  expectT(["T005"], "T005 data prereq: both pages have a stable id to target via ?focus", !!srId && !!soId);

  step(7, "T006: Revert SO -> SR (eligible while draft)");
  const revert = await http("POST", `/api/service-orders/${soId}/revert-to-request`);
  expectT(["T006"], "POST /revert-to-request returns 200", revert.status === 200, `status=${revert.status} body=${JSON.stringify(revert.body)?.slice(0,180)}`);
  ...
  expectT(["T006"], "SO detail returns 404 after revert", soAfterDetail.status === 404, `status=${soAfterDetail.status}`);
  expectT(["T006"], "SO no longer appears in SO list after revert", !soStillListed, `stillListed=${soStillListed}`);
  // SR back to approved with cleared cross-link.
  const srBack = (await http("GET", "/api/service-requests")).body.find((s) => s.id === srId);
  expectT(["T006"], "SR is back to approved", srBack?.status === "approved", `status=${srBack?.status}`);
  expectT(["T006"], "SR.serviceOrderId cleared", srBack?.serviceOrderId == null, `value=${srBack?.serviceOrderId}`);
  expectT(["T006"], "SR.serviceOrderNumber cleared", srBack?.serviceOrderNumber == null, `value=${srBack?.serviceOrderNumber}`);

  step(8, "Edit the SR again, then re-convert");
  const edit2 = await http("PATCH", `/api/service-requests/${srId}`, { estimatedCost: 1750 });
  expectT(["T007"], "PATCH after revert returns 200", edit2.status === 200, `status=${edit2.status}`);
  const reconv = await http("POST", `/api/service-requests/${srId}/convert`, {
    serviceProviderId: supplier.id,
    scope: "From smoke test (re-converted)",
    estimatedCost: 1750,
  });
  expectT(["T006"], "re-convert (after revert) returns 200", reconv.status === 200, `status=${reconv.status}`);
  const so2Id = reconv.body?.serviceOrder?.id;
  expectT(["T006"], "re-convert produced a NEW SO id (round-trip)", !!so2Id && so2Id !== soId, `id=${so2Id} prev=${soId}`);
  ...
  for (const sib of sibList) {
    ...
    const c = await http("POST", `/api/service-orders/${sib.id}/cancel`, { reason: "smoke cleanup" });
    expect(`cancel sibling SO ${sib.soNumber || sib.id} returns 200`, c.status === 200, `status=${c.status}`);
  }
  ...
  expect("SO is in 'sent' after /send", sendIt.body?.status === "sent", `status=${sendIt.body?.status}`);
  ...
  expect("SO is in 'confirmed' after /confirm", confirmIt.body?.status === "confirmed", `status=${confirmIt.body?.status}`);
  ...
  expect("SO is in 'in_progress' after /start", startIt.body?.status === "in_progress", `status=${startIt.body?.status}`);
  ...
  expectT(["T002"], "POST /complete returns 200", complete.status === 200, `status=${complete.status} body=${JSON.stringify(complete.body)?.slice(0,180)}`);
  expectT(["T002"], "SO is in 'completed' after /complete", complete.body?.status === "completed", `status=${complete.body?.status}`);
  ...
  expectT(["T008"], "End-to-end round-trip completed with no prior failures", priorFailures === 0, `priorFailures=${priorFailures}`);
  ...
  if (failed.length) {
    ...
    for (const f of failed) console.log("   - " + f.label + (f.detail ? colors.dim(" (" + f.detail + ")") : ""));
    process.exit(1);
  }
  const missingTasks = Object.entries(taskHits).filter(([, n]) => n === 0).map(([t]) => t);
