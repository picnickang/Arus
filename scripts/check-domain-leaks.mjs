...
async function main() {
  ...
  for (const file of allFiles) {
    ...
    while ((m = DYNAMIC_IMPORT_RE.exec(content)) !== null) {
      ...
      if (!target.startsWith(".") && !target.startsWith("@shared") && !target.startsWith("@server")) continue;
    ...
    if (sourceDomain) {
      ...
      while ((m = DB_STORAGE_RE.exec(content)) !== null) {
        ...
        seen.add(key);
        crossDomainStorage.push({ file: rel, storage, fromDomain: sourceDomain, toDomain: targetDomain });
      }
    }
  }
  ...
  if (writeBaseline) {
    const payload = {
      _comment: "Domain leak baseline. Counts must monotonically decrease. Regenerate with: node scripts/check-domain-leaks.mjs --write-baseline",
  ...
  for (const k of ["dynamicImports", "routeStorageImports", "crossDomainStorage"]) {
    if (totals[k] > baseline.totals[k]) {
      regressions.push(`  ${k}: ${baseline.totals[k]} → ${totals[k]} (+${totals[k] - baseline.totals[k]})`);
    }
  }
  ...
  for (const k of ["dynamicImports", "routeStorageImports", "crossDomainStorage"]) {
    if (totals[k] < baseline.totals[k]) {
      reductions.push(`  ${k}: ${baseline.totals[k]} → ${totals[k]} (-${baseline.totals[k] - totals[k]})`);
    }
  }
