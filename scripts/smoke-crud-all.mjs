...
const RESOURCES = [
  ...
  // /api/scheduled-reports for ML jobs only. Removed from sweep.)
  {
    ...
    create: {
      path: "/api/work-orders/:workOrderId/service-requests",
      payload: () => ({
...
const record = (resource, op, ok, detail) => results.push({ resource, op, ok, detail });
...
async function runResource(r) {
  ...
  console.log(
    `  ${listOk ? colors.ok("PASS") : colors.bad("FAIL")} LIST ${r.list} -> ${list.status}`
  );
  ...
  if (r.resolveFks) {
    try {
      fk = (await r.resolveFks()) || {};
    } catch (e) {
      fk = {};
    }
  }
  const missingFk = Object.entries(fk)
    .filter(([, v]) => v == null)
    .map(([k]) => k);
  if (missingFk.length) {
    record(r.name, "CREATE", false, `missing FK: ${missingFk.join(",")}`);
    console.log(
      `  ${colors.skip("SKIP")} CREATE — missing FK ${missingFk.join(",")} (need seed data)`
    );
    return;
  }
  ...
  const createOk =
    [200, 201].includes(created.status) && (created.body?.id || created.body?.data?.id);
  const id = created.body?.id || created.body?.data?.id;
  record(
    r.name,
    "CREATE",
    !!createOk,
    `status=${created.status} id=${id} body=${JSON.stringify(created.body)?.slice(0, 160)}`
  );
  console.log(
    `  ${createOk ? colors.ok("PASS") : colors.bad("FAIL")} CREATE ${createPath} -> ${created.status}${id ? " id=" + id : ""}`
  );
  if (!createOk) {
  ...
  if (r.getById) {
    ...
    const bodyId =
      g.body?.id || g.body?.data?.id || g.body?.part?.id || g.body?.item?.id || g.body?.result?.id;
