...
function reasonFor(file, lane) {
  ...
  if (file.includes("/forms/")) {
    return "Uses forms helper with direct pg Pool cleanup and live-server assumptions; confirmed in the 2026-06 sweep — helpers dial 127.0.0.1:5432 (ECONNREFUSED) under the embedded env.";
  }
  ...
  if (file.includes("/journeys/")) {
    return "Legacy journey flow not yet proven under the embedded deterministic bootstrap; swept 2026-06 with the forms lane — direct pg Pool connections ECONNREFUSED under the embedded env.";
  }
  ...
  if (file.includes("schematic-layout-crud")) {
    return "Swept 2026-06 under the embedded env (0/11 green): every route 500s because vessels.schematic_layout is missing from the sqlite schema; the deterministic :5000 bootstrap itself works.";
  }
  if (file.includes("validate-response")) {
    return "Swept 2026-06 under the embedded env (9/13 green): pdm/dashboard and permissions/me 500 on sqlite schema gaps (failure_predictions join, users.hub_admin/hub_access), permissions/audit 403 on an unseeded grant, risk-queue 400; envelope assertions pass.";
  }
  if (file.endsWith("/work-orders.test.ts")) {
    return "Swept 2026-06 under the embedded env (7/13 green): list routes 500 on a sqlite work_orders column gap and create/auth status expectations are stale (400 vs 201/401); envelope unwraps verified correct.";
  }
  if (file.endsWith("/crew-scheduling.test.ts")) {
    return "Swept 2026-06 under the embedded env (8/12 green): /api/crew reads 403 on unseeded permission grants and assignments 500 on a sqlite crew_assignment column gap; envelope unwraps verified correct.";
  }
...
function statusFor(lane) {
  if (lane === "embedded") {
    return "Passing as of the 2026-06 sweep: 17 suites, 144 tests.";
  }
  if (lane === "postgres") {
    return "Not part of default gate; fail-fast when DATABASE_URL is absent.";
  }
  if (lane === "server") {
    return "Passing in this stabilization pass: 4 suites, 48 tests.";
  }
...
function migrationFor(file, lane) {
  ...
  if (lane === "server") {
    return "Keep self-starting with embedded SQLite; add future server-style HTTP/browser checks here before promoting them into the default gate.";
  }
