 ...
 *
 * Ensures no code imports from the retired server/storage.ts path.
...
if (violations.length > 0) {
  ...
  console.log("\nFix: Import from server/repositories.ts instead.");
  process.exit(1);
} else {
  console.log("All storage facade imports eliminated. server/storage.ts deleted.");
  process.exit(0);
}
