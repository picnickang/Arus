...
async function generateFinalReport() {
  ...
  const routeInventory = JSON.parse(fs.readFileSync('reports/route-inventory-improved.json', 'utf-8'));
  ...
  md += `### Health Score\n\n`;
  const schemaHealth = ((schemaDiff.summary.totalTables - schemaDiff.summary.missingOrgId) / schemaDiff.summary.totalTables * 100).toFixed(1);
  const apiHealth = (routeInventory.summary.withOrgScope / routeInventory.summary.totalRoutes * 100).toFixed(1);
  ...
  const orgScopedPct = (routeInventory.summary.withOrgScope / routeInventory.summary.totalRoutes * 100).toFixed(1);
  const rateLimitPct = (routeInventory.summary.withRateLimit / routeInventory.summary.totalRoutes * 100).toFixed(1);
  ...
  const unprotectedRoutes = routeInventory.summary.totalRoutes - routeInventory.summary.withOrgScope;
