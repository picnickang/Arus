...
async function auditSchema(): Promise<SchemaAuditResult> {
  ...
  for (const tableRow of tablesQuery.rows) {
    ...
    audit.foreignKeys = fkQuery.rows.map(
      (r) => `${r.column_name} -> ${r.foreign_table_name}(${r.foreign_column_name})`
    );
    ...
    if (hasOrgId) {
      const hasOrgIdIndex = audit.indexes.some(
        (idx) => idx.toLowerCase().includes("org_id") || idx.toLowerCase().includes("orgid")
      );
      if (!hasOrgIdIndex) {
    ...
    for (const fk of audit.foreignKeys) {
      ...
      const hasFkIndex = audit.indexes.some((idx) =>
    ...
    const numericColumns = columns.filter(
      (c) => c.type.includes("numeric") || c.type.includes("decimal") || c.type.includes("real")
    );
  ...
  const summary = {
    ...
    missingOrgId: tableAudits.filter(
      (t) => !t.hasOrgId && !MULTI_TENANT_EXEMPT_TABLES.some((e) => t.tableName.includes(e))
    ).length,
