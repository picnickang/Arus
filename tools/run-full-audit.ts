...
class AuditRunner {
  ...
  private async testDatabaseConnection() {
    ...
    try {
      ...
      this.recordResult('Database Connection', passed, passed ? 'Connected successfully' : 'Connection failed', Date.now() - start);
    } catch (error) {
  ...
  private async testRedisConnection() {
    ...
    try {
      ...
      this.recordResult('Redis Connection', passed, passed ? 'Connected successfully' : 'Redis unavailable (optional)', Date.now() - start);
    } catch (error) {
      this.recordResult('Redis Connection', true, 'Redis unavailable (optional)', Date.now() - start);
    }
  }
