...
function hasAuthMiddleware(middlewareChain: string, mountMiddleware: string[]): boolean {
  return middlewareChain.includes('requireAuth') ||
...
function hasOrgScopeMiddleware(middlewareChain: string, mountMiddleware: string[]): boolean {
  return middlewareChain.includes('requireOrgId') ||
...
function hasValidationMiddleware(middlewareChain: string): boolean {
  return middlewareChain.includes('validate') ||
...
function hasRateLimitMiddleware(middlewareChain: string, mountMiddleware: string[]): boolean {
  return middlewareChain.includes('rateLimit') ||
...
function buildMiddlewareList(hasAuth: boolean, hasOrgScope: boolean, hasValidation: boolean, hasRateLimit: boolean): string[] {
...
async function analyzeRouterFile(filePath: string): Promise<{
  ...
  const routePattern = /(app|router)\.(get|post|put|patch|delete)\s*\(\s*['"`]([^'"`]+)['"`]\s*,\s*([^{]+)/g;
  let match;
...
async function main() {
  try {
    ...
    const unprotected = result.routes.filter(r => 
      !r.hasAuth && !r.path.includes('/health') && !r.path.includes('/metrics') && !r.path.includes('/public') && r.method !== 'GET'
    );
    report += formatRouteList(unprotected, 'Unprotected Write Operations', `Found ${unprotected.length} write operations (POST/PUT/PATCH/DELETE) without authentication`);
    
    const noOrgScope = result.routes.filter(r => !r.hasOrgScope && r.hasAuth && !r.path.includes('/admin') && !r.path.includes('/system'));
    report += formatRouteList(noOrgScope, 'Authenticated but Not Org-Scoped', `Found ${noOrgScope.length} authenticated routes without org-scoping`);
