...
async function generateReport(): Promise<AuditReport> {
  ...
  const routeInventory = JSON.parse(fs.readFileSync("reports/route-inventory.json", "utf-8"));
...
async function main() {
  try {
    ...
    const schemaHealth = (
      ((report.schema.totalTables - report.schema.missingOrgId) / report.schema.totalTables) *
      100
    ).toFixed(1);
