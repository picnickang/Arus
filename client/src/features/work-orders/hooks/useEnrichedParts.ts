...
export function useEnrichedWorkOrderParts(workOrderId: string | undefined) {
  return useQuery<EnrichedWorkOrderPart[]>({
    ...
    queryFn: () =>
      apiRequest<EnrichedWorkOrderPart[]>("GET", `/api/work-orders/${workOrderId}/parts/enriched`),
    enabled: !!workOrderId,
  });
}
...
export function useOutOfStockSuggestions(workOrderId: string | undefined) {
  return useQuery<OutOfStockSuggestion[]>({
    ...
    queryFn: () =>
      apiRequest<OutOfStockSuggestion[]>(
        "GET",
        `/api/work-orders/${workOrderId}/parts/out-of-stock-suggestions`
      ),
    enabled: !!workOrderId,
  });
}
