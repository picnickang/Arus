...
import type { MobileReadinessScreens, QueueItem, ReadinessTone } from "./mobile-readiness-model";
import {
  AssetImage,
  Content,
  KpiStrip,
        ...
        <Content className="space-y-2 pt-2">
          ...
          <div className="flex gap-4 overflow-x-auto border-b border-slate-200">
            {work.filters.map((filter) => (
              <button
                key={filter.id}
                className="shrink-0 border-b-2 border-transparent px-1 pb-1.5 text-sm font-semibold text-slate-600 first:border-primary first:text-primary"
              >
...
function workQueueStripeClass(tone: ReadinessTone): string {
...
function workQueueCommentCount(itemId: string): string {
  if (itemId === "sr-1258") {
    return "2";
  }
  if (itemId === "so-4481") {
        ...
        <Content className="space-y-2 pb-28 pt-2">
          ...
          <section className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
            ...
            <div className="grid grid-cols-4 gap-2 p-2">
              ...
              <button className="grid h-16 place-items-center rounded-lg border border-slate-200 text-primary">
          ...
          <div className="sticky bottom-16 z-20 grid grid-cols-[0.8fr_1fr_1.1fr] gap-2 bg-background/95 py-1.5">
            <button className="min-h-10 rounded-lg border border-slate-200 bg-white text-xs font-semibold text-slate-600">
              Actions
            </button>
            <button className="min-h-10 rounded-lg border border-slate-200 bg-white text-xs font-semibold text-primary">
              {execution.offlineDraftAction}
            </button>
            <button className="min-h-10 rounded-lg bg-primary text-xs font-semibold text-white">
...
function autofillTrustDotClass(tone: ReadinessTone): string {
  if (tone === "good") {
      ...
      <div data-testid="mobile-readiness-screen-logs">
        ...
        <Content className="space-y-2 pb-20 pt-2">
            ...
            {logs.tabs.map((tab, index) => (
              <button
                ...
                className={cn(
                  ...
                  index === 0 ? "border-primary text-primary" : "border-transparent text-slate-600"
                )}
              >
          ...
          <SectionCard
            ...
            action={<button className="text-sm font-semibold text-primary">View All</button>}
          >
                  ...
                  {logs.autofillTrust.map((metric) => (
                    <div key={metric.id} className="flex items-center justify-between text-xs">
                      <span className="flex items-center gap-2">
                        <span
                          className={cn("h-2 w-2 rounded-full", autofillTrustDotClass(metric.tone))}
                        />
                        {metric.label}
                      </span>
          ...
          >
            <div className="space-y-2 p-3">
              ...
              <button className="text-xs font-semibold text-primary">View More History</button>
            </div>
          </SectionCard>
