  ...
  const reportMutation = useMutation({
    ...
    onSuccess: (record) => {
      ...
      toast({
        ...
        description:
          "Open the suggested workflow to turn it into a work order, finding, log note, or handover item.",
      });
    },
    onError: (error) => {
      toast({
        ...
        description:
          error instanceof Error ? error.message : "The workflow issue report was rejected.",
        variant: "destructive",
      });
    },
  });
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Capture a field issue once, then open the right downstream workflow with context
          preserved.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-3">
        <div className="grid gap-2 text-sm md:grid-cols-3">
          ...
          <div className="rounded-lg border p-3">
            3. Route to work order, finding, log note, or handover.
          </div>
        </div>
        ...
        {!expanded ? (
          <div className="flex flex-wrap gap-2">
            ...
            <Button
              variant="outline"
              onClick={() => setLocation("/findings?action=create&flow=report-issue")}
            >
          ...
          <div className="space-y-3 rounded-lg border p-3">
            ...
            <div className="grid gap-3 md:grid-cols-4">
              <Select
                value={severity}
                onValueChange={(value) => setSeverity(value as IssueReportRecord["severity"])}
              >
                <SelectTrigger>
              ...
              <Select
                value={target}
                onValueChange={(value) => setTarget(value as IssueReportRecord["target"])}
              >
                <SelectTrigger>
            ...
            <Textarea
              ...
              placeholder="Safety, compliance, reliability, or operational impact..."
            />
            <Textarea
              value={evidenceNote}
              onChange={(event) => setEvidenceNote(event.target.value)}
              placeholder="Evidence note, photo reference, reading, log entry, or inspection detail..."
            />
            <div className="flex flex-wrap gap-2">
              <Button
                onClick={() => reportMutation.mutate()}
                disabled={!summary.trim() || reportMutation.isPending}
              >
            ...
            {createdIssue && (
              <p className="text-xs text-muted-foreground">
                Saved issue {createdIssue.id.slice(0, 8)} as{" "}
                {createdIssue.target.replace(/_/g, " ")} draft.
              </p>
            )}
          </div>
        )}
