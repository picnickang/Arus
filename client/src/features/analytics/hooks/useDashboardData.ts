...
export function useDashboardData() {
  ...
  const criticalWorkOrdersCount = (Array.isArray(workOrders) ? workOrders : []).filter((wo) => (wo.priority === "high" || wo.priority === "2" || wo.priority === 2) && wo.status !== "completed").length;
  ...
  const criticalWorkOrders = workOrders?.filter((wo) => (wo.priority === "high" || wo.priority === "2" || wo.priority === 2) && wo.status !== "completed") ?? [];

  const getVesselName = (vesselId: string): string => { if (vesselId === "all") {return "All Vessels";} const vessel = allVessels?.find((v) => v.id === vesselId); return vessel?.name || vesselId; };
  const getEquipmentName = (equipmentId: string | null | undefined): string => { if (!equipmentId) {return "Unknown";} const eqHealthArray = Array.isArray(equipmentHealth) ? equipmentHealth : []; const healthItem = eqHealthArray.find((eq) => eq.id === equipmentId); if (healthItem?.name) {return healthItem.name;} const equipment = equipmentRegistry?.find((eq) => eq.id === equipmentId); if (equipment?.name) {return equipment.name;} return equipmentId; };
  const getPriorityText = (priority: string | number | null | undefined): string => { if (priority === null || priority === undefined) {return "N/A";} const priorityStr = String(priority).toLowerCase(); if (priorityStr === "high" || priorityStr === "2") {return "HIGH";} if (priorityStr === "medium" || priorityStr === "1") {return "MEDIUM";} if (priorityStr === "low" || priorityStr === "0") {return "LOW";} return String(priority).toUpperCase(); };
  const shouldShowSection = (sectionType: "critical" | "normal") => { if (!isFocusMode) {return true;} return sectionType === "critical"; };
  const refreshData = () => { toast({ title: "Refreshing data...", description: "Dashboard data is being updated" }); queryClient.invalidateQueries(); setTimeout(() => { toast({ title: "Data refreshed", description: "Dashboard updated successfully" }); }, 500); };
  const dismissAlert = () => setAlertBanner(null);

  useEffect(() => { if (isConnected) { subscribe("alerts"); subscribe("dashboard"); } return () => { unsubscribe("alerts"); unsubscribe("dashboard"); }; }, [isConnected, subscribe, unsubscribe]);

  useEffect(() => {
    if (latestAlert && !latestAlert.acknowledged) { setAlertBanner(latestAlert); const alertType = latestAlert.alertType || "info"; toast({ title: `${alertType.toUpperCase()} Alert`, description: latestAlert.message, variant: alertType === "critical" ? "destructive" : "default" }); if (alertType !== "critical") { setTimeout(() => setAlertBanner(null), 10000); } }
  }, [latestAlert, toast]);

  useEffect(() => { if (allVessels?.length > 0) { const isValidFilter = selectedVessel === "all" || allVessels.some((v) => v.id === selectedVessel); if (!isValidFilter) { setSelectedVessel("all"); } } }, [allVessels]);
  useEffect(() => { updatePreference("vesselFilter", selectedVessel); }, [selectedVessel, updatePreference]);
  ...
  return {
    ...
    getVesselName, getEquipmentName, getPriorityText, shouldShowSection, refreshData, dismissAlert,
    operatingAlerts, insightsSnapshot, insightsJobStats, stcwSummary, stcwTrends, equipmentRegistry,
  };
}
