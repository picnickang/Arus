...
export function useMaintenanceModeData() {
  ...
  const metrics = useMemo(() => {
    ...
    const overdueOrders = workOrders.filter((wo: WorkOrderData) => {
      ...
      return (
        (wo.priority === 1 && ageHours > 24) ||
        (wo.priority === 2 && ageHours > 72) ||
  ...
  const failureChartData = useMemo(
    () =>
      failurePatterns.map((pattern) => ({
        ...
        prevented: pattern.preventedFailures || 0,
      })),
    [failurePatterns]
  );
      ...
      pdmScores
        ...
        .map((score: PdmScoreData) => {
          ...
          return {
            ...
            equipmentName: score.equipmentName || score.equipmentId,
            failureRisk: score.failureRisk,
            recommendedWindow: `${daysUntilCritical} days`,
