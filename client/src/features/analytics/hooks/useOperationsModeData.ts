...
interface EquipmentHealthData { id: string; name?: string; healthIndex: number; failureRisk?: number; }
interface TelemetryTrendData { baseline?: number; average?: number; currentValue?: number; sensorType?: string; equipmentId?: string; }
interface FailurePredictionData { id?: string; confidence?: number; equipmentId?: string; equipmentName?: string; predictedDate?: string; failureType?: string; timestamp?: string; }
interface PdmScoreData { equipmentId: string; timestamp: string; healthScore?: number; }
interface AnomalyData { id?: string; equipmentId: string; equipmentName?: string; sensorType?: string; value?: number; unit?: string; zscore?: number; timestamp: string; }
...
export function useOperationsModeData() {
  ...
  const getAnomalyId = useCallback((a: AnomalyData) => a.id || `${a.equipmentId}-${a.sensorType || "unknown"}-${a.timestamp}`, []);

  const unacknowledgedAnomalies = useMemo(() => anomalies.filter((a) => {
  ...
  const watchingAnomalies = useMemo(() => anomalies.filter((a) => {
    const id = getAnomalyId(a);
  ...
  const handleWatch = useCallback((anomalyId: string) => {
    setAnomalyStatus(anomalyId, "watching");
  }, [setAnomalyStatus]);

  const getAnomalyAckStatus = useCallback((anomalyId: string): AnomalyAckStatus => {
    return anomalyAckState.get(anomalyId) ?? "unacknowledged";
  }, [anomalyAckState]);

  return { isConnected, latestTelemetry, healthLoading, equipmentHealth, telemetryTrends, failurePredictions, ...equipmentCategories, ...metrics, equipmentHealthTrends, unacknowledgedAnomalies, watchingAnomalies, acknowledgedCount, anomalyAckState, handleAcknowledge, handleWatch, getAnomalyAckStatus, getAnomalyId, getDriftThreshold };
}
