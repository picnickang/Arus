...
export interface ThresholdOptimization {
  ...
  currentThresholds: ThresholdValues;
  optimizedThresholds: ThresholdValues;
...
export function useSensorOptimizationData() {
  ...
  const {
    data: statisticalRecs = [],
    isLoading: statsLoading,
    refetch: refetchStats,
  } = useQuery<ThresholdOptimization[]>({
  ...
  const {
    ...
    error: aiError,
    refetch: refetchAI,
  } = useQuery<{ recommendations: SensorRecommendation[] }>({
  ...
  const analyzeMutation = useCustomMutation({
    mutationFn: async (equipmentId: string) => {
      return apiRequest("POST", `/api/sensor-optimization/analyze/${equipmentId}`, {
        daysOfHistory: 30,
      });
    },
    ...
    errorMessage: (error: unknown) =>
      error instanceof Error ? error.message : "Failed to analyze sensor data",
    onSuccess: () => {
      refetchStats();
    },
  });
  const applyOptimizationMutation = useCustomMutation({
    mutationFn: async (optimizationId: number) => {
      return apiRequest("POST", `/api/sensor-optimization/apply/${optimizationId}`, undefined);
    },
    ...
    errorMessage: (error: unknown) =>
      error instanceof Error ? error.message : "Could not apply optimization",
    onSuccess: () => {
      refetchStats();
    },
  });
  const rejectOptimizationMutation = useCustomMutation({
    mutationFn: async ({ id, reason }: { id: number; reason: string }) => {
      return apiRequest("POST", `/api/sensor-optimization/reject/${id}`, { reason });
    },
    invalidateKeys: [["/api/sensor-optimization/recommendations"]],
  ...
  const applyAIRecommendationMutation = useCustomMutation({
    mutationFn: async ({
      ...
      equipmentId: string;
      sensorType: string;
      parameters: SensorRecommendation["parameters"];
    }) => {
      return apiRequest("POST", `/api/sensor-tuning/apply/${equipmentId}/${sensorType}`, {
        parameters,
      });
    },
    ...
    errorMessage: (error: unknown) =>
      error instanceof Error ? error.message : "Could not apply AI recommendations",
  });

  const getConfidenceBadgeVariant = useCallback(
    (confidence: string): "default" | "secondary" | "outline" => {
  ...
  const formatSensorName = useCallback((sensorType: string): string => {
    if (!sensorType) {
  ...
  return {
    ...
    rejectOptimizationMutation,
    applyAIRecommendationMutation,
    refetchAI,
    handleSort,
