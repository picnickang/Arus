...
export function useFleetData() {
  ...
  const { data: vessels = [], isLoading: vesselsLoading } = useQuery<VesselData[]>({
    queryKey: ["/api/vessels"],
    refetchInterval: 120000,
  });
  const { data: allEquipmentHealth = [], isLoading: healthLoading } = useQuery<EquipmentHealth[]>({
  ...
  const { data: insightsData, isLoading: insightsLoading } = useQuery<FleetOverviewResponse>({
  ...
  const equipmentHealth = useMemo(
    () =>
      selectedVesselId === "all"
        ? allEquipmentHealth
        : allEquipmentHealth.filter((eq) => eq.vesselId === selectedVesselId),
    [allEquipmentHealth, selectedVesselId]
  );

  const fleetSummary = useMemo(() => {
    const targetEquipment = selectedVesselId === "all" ? allEquipmentHealth : equipmentHealth;
    return {
      total: targetEquipment.length,
      healthy: targetEquipment.filter((eq) => eq.status === "healthy").length,
      warning: targetEquipment.filter((eq) => eq.status === "warning").length,
      critical: targetEquipment.filter((eq) => eq.status === "critical").length,
      avgHealth:
        targetEquipment.length > 0
          ? Math.round(
              targetEquipment.reduce((sum, eq) => sum + (eq.healthScore ?? 0), 0) /
                targetEquipment.length
            )
          : 0,
    };
  ...
  const handleRefresh = useCallback(async () => {
    ...
    await Promise.all([
      ...
      queryClient.invalidateQueries({ queryKey: ["/api/telemetry/latest"] }),
      queryClient.invalidateQueries({ queryKey: ["/api/insights/v2/fleet-overview"] }),
    ]);
