...
import { businessGoalLabel, priorityVariant, statusVariant } from "../lib/informationNeedsViewModel";
  ...
  return (
    <Card data-testid="card-role-information-needs">
      <CardHeader>
        ...
        <CardDescription>
          Role-specific information model built from operational signals, UX goals, trust evidence, and the next action path.
        </CardDescription>
      </CardHeader>
        ...
        <div className="grid gap-3 lg:grid-cols-2">
          {summary.topNeeds.map((need) => (
            <div key={need.id} className="rounded-lg border p-3">
              ...
              <p className="mt-2 text-xs"><strong>Why now:</strong> {need.reason}</p>
              <div className="mt-3 space-y-2 text-xs">
                <div><strong>Needs:</strong> {need.informationNeeded.join(", ")}</div>
                <div><strong>UI pattern:</strong> {need.uiPattern}</div>
                <div><strong>Trust evidence:</strong> {need.trustEvidence.join(", ")}</div>
              </div>
        ...
        <div className="grid gap-3 lg:grid-cols-2">
          ...
          <div className="rounded-lg border p-3 text-sm">
            ...
            <div className="mt-2 space-y-1 text-muted-foreground">
              <p><strong>Clarity:</strong> {summary.uxGuidance.clarity}</p>
              <p><strong>Speed:</strong> {summary.uxGuidance.speed}</p>
              <p><strong>Simplicity:</strong> {summary.uxGuidance.simplicity}</p>
              <p><strong>Trust:</strong> {summary.uxGuidance.trust}</p>
              <p><strong>Retention:</strong> {summary.uxGuidance.retention}</p>
            </div>
          </div>
        </div>
      </CardContent>
