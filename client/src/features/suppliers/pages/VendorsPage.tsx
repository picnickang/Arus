...
export function VendorsPage() {
  ...
  const { suppliers, serviceProviders } = useMemo(() => {
    if (!allVendors) {
      return { suppliers: [], serviceProviders: [] };
    }
  ...
  const handleCreate = (data: SupplierFormData) => {
    createMutation.mutate(data, {
      onSuccess: () => {
        toast({
          title: `${data.type === "service_provider" ? "Service provider" : "Supplier"} created successfully`,
        });
        setIsFormOpen(false);
      },
        ...
        {vendors.length === 0 && (
          <Card className="col-span-full">
            <CardContent className="py-12 text-center">
              ...
              <Button variant="outline" className="mt-4" onClick={() => handleOpenForm(addType)}>
                Add your first {emptyLabel.slice(0, -1)}
              </Button>
            </CardContent>
          </Card>
        )}
    ...
    <div className="p-6 space-y-6">
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as typeof activeTab)}>
        <div className="flex items-center justify-between gap-4 flex-wrap">
          ...
          <Button
            onClick={() =>
              handleOpenForm(activeTab === "service_providers" ? "service_provider" : "supplier")
            }
      ...
      <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              Add New {formDefaultType === "service_provider" ? "Service Provider" : "Supplier"}
            </DialogTitle>
          </DialogHeader>
          <SupplierForm
      ...
      <Dialog open={!!editingSupplier} onOpenChange={() => setEditingSupplier(null)}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              Edit {editingSupplier?.type === "service_provider" ? "Service Provider" : "Vendor"}
            </DialogTitle>
          </DialogHeader>
          {editingSupplier && (
            <SupplierForm
