...
export const partFormSchema = z.object({
  ...
  standardCost: z
    .number({
      required_error: "Standard cost is required",
      invalid_type_error: "Standard cost must be a number",
    })
...
export function useInventoryManagementData() {
  ...
  const filteredParts = useMemo(() => {
    const filtered = filterParts(partsInventory, filters);
    return sortParts(filtered, sortField, sortDirection);
  }, [partsInventory, filters, sortField, sortDirection]);
  ...
  const createPartMutation = useCustomMutation({
    mutationFn: async (data: PartFormData) => {
      const result = await apiRequest("POST", "/api/parts-inventory", {
  ...
  const updatePartMutation = useCustomMutation({
    mutationFn: async ({ id, data }: { id: string; data: PartFormData }) => {
      const result = await apiRequest("PUT", `/api/parts-inventory/${id}`, {
        partNumber: data.partNumber,
        partName: data.partName,
        category: data.category,
        unitCost: data.standardCost,
      ...
      const preferredSupplierId = supplierIds.includes(data.preferredSupplierId || "")
        ? data.preferredSupplierId
        : supplierIds[0] || undefined;
      await apiRequest("PUT", `/api/inventory/${id}/suppliers`, {
        supplierIds,
        preferredSupplierId,
      });
      return result;
  ...
  const deletePartMutation = useCustomMutation({
    mutationFn: async (id: string) => apiRequest("DELETE", `/api/parts-inventory/${id}`),
    invalidateKeys: ["/api/parts-inventory", "/api/parts-inventory/filters"],
  ...
  const handleRowClick = useCallback((part: PartsInventoryItem) => {
  ...
  const onSubmitPart = useCallback(
    ...
    [editingPart, updatePartMutation, createPartMutation]
  );
  ...
  const handlePartDialogClose = useCallback(
    (open: boolean) => {
      if (!open) {
        setIsAddPartDialogOpen(false);
        setIsEditPartDialogOpen(false);
  ...
  const handlePartDetailEdit = useCallback(
    (part: PartsInventoryItem) => {
