...
export function ServiceOrderFormDialog({
  ...
  const { data: suppliers, isLoading: loadingSuppliers } = useQuery<{ id: string; name: string }[]>(
  ...
  const { data: workOrders, isLoading: loadingWOs } = useQuery<
    { id: string; woNumber: string; reason?: string }[]
  >({ queryKey: ["/api/work-orders"] });
  const { data: vessels, isLoading: loadingVessels } = useQuery<{ id: string; name: string }[]>({
  ...
  useEffect(() => {
    ...
    if (mode === "edit" && serviceOrder) {
      form.reset({
        ...
        vesselId: (serviceOrder as { vesselId?: string }).vesselId || "__none__",
        ...
        scheduledStartDate: serviceOrder.scheduledStartDate
          ...
          : "",
        scheduledEndDate: serviceOrder.scheduledEndDate
          ? new Date(serviceOrder.scheduledEndDate).toISOString().split("T")[0]
          : "",
        ...
        downtimeWindowStart: serviceOrder.downtimeWindowStart
          ? new Date(serviceOrder.downtimeWindowStart).toISOString().split("T")[0]
          : "",
        downtimeWindowEnd: serviceOrder.downtimeWindowEnd
          ? new Date(serviceOrder.downtimeWindowEnd).toISOString().split("T")[0]
          : "",
        justification: serviceOrder.justification || "",
        responseDeadline: serviceOrder.responseDeadline
          ? new Date(serviceOrder.responseDeadline).toISOString().split("T")[0]
          : "",
      });
    } else if (mode === "create") {
      form.reset({
  ...
  const onSubmit = async (values: SOFormValues) => {
    const baseData = {
      ...
      scheduledStartDate: values.scheduledStartDate
        ? new Date(values.scheduledStartDate)
        : undefined,
      ...
      estimatedDurationHours: values.estimatedDurationHours
        ? parseFloat(values.estimatedDurationHours)
        : undefined,
      ...
      downtimeWindowStart: values.downtimeWindowStart
        ? new Date(values.downtimeWindowStart)
        : undefined,
    ...
    const data = mode === "create" ? { ...baseData, status: "draft" } : baseData;
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      toast({
        title: "Service Order Sent",
        description: "The service order has been sent to the provider.",
      });
      onOpenChange(false);
      onSuccess?.();
    } catch (error) {
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "The service order has been cancelled.",
      });
      onOpenChange(false);
      onSuccess?.();
    } catch (error) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {mode === "edit" ? "Edit Service Order" : "Create Service Order"}
          </DialogTitle>
          <DialogDescription>
            {mode === "edit"
              ? "Update the service order details"
              : "Create a new service order for external service providers"}
          </DialogDescription>
        </DialogHeader>
        ...
        {showPermissionWarning && (
          <Alert variant="destructive" data-testid="alert-no-edit-permission">
            ...
            <AlertDescription>
              This service order has been submitted. Only users with approval permissions can modify
              it.
            </AlertDescription>
          </Alert>
        )}
