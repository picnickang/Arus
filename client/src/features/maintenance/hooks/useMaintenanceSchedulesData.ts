...
export function useMaintenanceSchedulesData() {
  ...
  const [createForm, setCreateForm] = useState<Partial<InsertMaintenanceSchedule> & { scheduledDate?: Date | string }>({ equipmentId: "", scheduledDate: "", maintenanceType: "preventive", priority: 2, description: "" });
  ...
  const createMutation = useCreateMutation<InsertMaintenanceSchedule>("/api/maintenance-schedules", { successMessage: "✓ Schedule created successfully", onSuccess: () => { setCreateModalOpen(false); setCreateForm({ equipmentId: "", scheduledDate: "", maintenanceType: "preventive", priority: 2, description: "" }); } });
  const updateMutation = useUpdateMutation<Partial<InsertMaintenanceSchedule>>("/api/maintenance-schedules", { successMessage: "✓ Schedule updated successfully", onSuccess: () => { setEditModalOpen(false); setSelectedSchedule(null); setEditForm({}); } });
  const deleteMutation = useDeleteMutation("/api/maintenance-schedules", { successMessage: "✓ Schedule deleted successfully" });
  ...
  const getStatusBadge = (status: string) => {
    const styles = { scheduled: "bg-blue-500/10 text-blue-700 dark:text-blue-300 border-blue-500/30", in_progress: "bg-amber-500/10 text-amber-700 dark:text-amber-300 border-amber-500/30", completed: "bg-green-500/10 text-green-700 dark:text-green-300 border-green-500/30", cancelled: "bg-gray-500/10 text-gray-700 dark:text-gray-300 border-gray-500/30" };
    return { className: `border ${styles[status as keyof typeof styles] || styles.scheduled}`, label: status.replace("_", " ") };
  };

  const getPriorityBadge = (priority: number) => {
    const config = { 1: { label: "High", className: "bg-red-500/10 text-red-700 dark:text-red-300 border-red-500/30" }, 2: { label: "Medium", className: "bg-amber-500/10 text-amber-700 dark:text-amber-300 border-amber-500/30" }, 3: { label: "Low", className: "bg-blue-500/10 text-blue-700 dark:text-blue-300 border-blue-500/30" } };
    const p = config[priority as keyof typeof config] || config[2];
    return { className: `border ${p.className}`, label: p.label };
  };

  return {
    schedules, isLoading, error, equipment, upcomingSchedules,
    selectedSchedule, setSelectedSchedule, viewModalOpen, setViewModalOpen, editModalOpen, setEditModalOpen, createModalOpen, setCreateModalOpen,
    editForm, setEditForm, createForm, setCreateForm,
    searchText, setSearchText, statusFilter, setStatusFilter, priorityFilter, setPriorityFilter, viewType, setViewType,
    createMutation, updateMutation, deleteMutation,
    getEquipmentName, handleViewSchedule, handleEditSchedule, handleDeleteSchedule, handleCreateSubmit, handleEditSubmit,
    filteredSchedules, getStatusBadge, getPriorityBadge,
  };
}
