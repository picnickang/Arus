...
export function useNotificationSettings() {
  ...
  const createMutation = useMutation({
    mutationFn: async (data: Partial<NotificationSetting>) =>
      apiRequest("/api/notifications/settings", { method: "POST", body: JSON.stringify(data) }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/notifications/settings"] });
      toast({ title: "Notification setting created" });
      setIsDialogOpen(false);
      resetForm();
    },
    onError: () => {
      toast({ title: "Failed to create notification setting", variant: "destructive" });
    },
  });
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: Partial<NotificationSetting> }) =>
      apiRequest(`/api/notifications/settings/${id}`, {
        method: "PATCH",
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) =>
      apiRequest(`/api/notifications/settings/${id}`, { method: "DELETE" }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/notifications/settings"] });
      toast({ title: "Notification setting deleted" });
    },
    onError: () => {
  ...
  const openEditDialog = useCallback((setting: NotificationSetting) => {
    ...
    setFormData({
      ...
      vesselId: setting.vesselId,
      enabled: setting.enabled ?? true,
      minSeverity: setting.minSeverity || "warning",
      recipientEmails: (setting.recipientEmails as string[]) ?? [],
      deliveryMethod: setting.deliveryMethod || "email",
      digestMode: setting.digestMode ?? false,
    });
    setIsDialogOpen(true);
  }, []);
  const handleAddEmail = useCallback(() => {
    if (emailInput?.includes("@")) {
      setFormData((prev) => ({ ...prev, recipientEmails: [...prev.recipientEmails, emailInput] }));
      setEmailInput("");
    }
  }, [emailInput]);
  ...
  const handleSubmit = useCallback(() => {
    const payload = { ...formData, recipientEmails: formData.recipientEmails };
    if (editingSetting) {
      updateMutation.mutate({ id: editingSetting.id, data: payload });
    } else {
      createMutation.mutate(payload);
    }
  }, [formData, editingSetting, createMutation, updateMutation]);
  ...
  const handleTestEmail = useCallback(() => {
    const email = prompt("Enter email for test notification:");
    if (email) {
