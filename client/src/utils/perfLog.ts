...
export function useEffectTime(effectName: string, deps: unknown[]): void {
  useEffect(() => {
    ...
    return () => {
      ...
      if (duration > 16) {
        // Log if effect takes longer than 1 frame
  ...
  start(label: string): void {
    ...
    if (isPerfDebugEnabled()) {
      ...
      if (duration > warnThresholdMs) {
        perfLog(
          "Timer",
          `${label} took ${duration.toFixed(2)}ms (> ${warnThresholdMs}ms threshold)`,
          undefined,
          "slow"
        );
      }
  ...
  measure(name: string, startMark: string, endMark?: string): number {
    ...
    try {
      ...
      performance.measure(
        measureName,
        `perf:${startMark}`,
        endMark ? `perf:${endMark}` : undefined
      );
...
export function trackQueryRefetch(queryKey: string): void {
  ...
  if (timeSinceLast < 5000 && existing.count > 1) {
    perfLog(
      "Query",
      `${queryKey} refetched after ${timeSinceLast}ms (${existing.count} total)`,
      undefined,
      "warn"
    );
...
if (typeof globalThis !== "undefined") {
  ...
  if (isPerfDebugEnabled()) {
    console.info(
      "%c🔍 Performance Debugging Enabled",
