                  ...
                  <div key={r.date} role="button" tabIndex={0} className={`group hover:bg-slate-50 dark:hover:bg-slate-850 transition-colors ${selectedDay === actualIndex ? "bg-blue-50 dark:bg-blue-950" : ""}`} onClick={() => setSelectedDay(actualIndex)} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setSelectedDay(actualIndex); } }}>
                    <div style={{ display: "grid", gridTemplateColumns: `110px repeat(24, ${hourW}px) 75px 75px` }}>
                      ...
                      {hours.map((h) => {
                        ...
                        return (
                          <div key={h} role="checkbox" aria-checked={isRest} aria-label={`Hour ${String(h).padStart(2, "0")}:00, ${isRest ? "Rest" : "Work"}${isNightHour ? " (Night)" : ""}`} tabIndex={0} onMouseDown={(e) => { e.preventDefault(); startDrag(actualIndex, h); }} onMouseEnter={() => onDrag(actualIndex, h)} onTouchStart={(e) => { e.preventDefault(); startDrag(actualIndex, h); }} onTouchMove={(e) => { const touch = e.touches[0]; const el = document.elementFromPoint(touch.clientX, touch.clientY); const testId = el?.getAttribute("data-testid"); if (testId?.startsWith("grid-cell-")) { const parts = testId.split("-"); onDrag(Number(parts[2]), Number(parts[3])); } }} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); startDrag(actualIndex, h); } }}
