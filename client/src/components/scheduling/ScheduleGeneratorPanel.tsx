...
export function ScheduleGeneratorPanel({ isOpen, onOpenChange }: ScheduleGeneratorPanelProps) {
  ...
  const simulateMutation = useMutation({
    ...
    onSuccess: (data: SimulationResult) => {
      ...
      toast({
        ...
        description: `Generated ${data.stats.proposed} proposals`,
      });
    },
    onError: (error: Error) =>
  ...
  const applyMutation = useMutation({
    ...
    onSuccess: (data: ApplyResult) => {
      ...
      toast({
        title: "Applied as Drafts",
        description: `Created ${data.applied} draft assignments`,
      });
    },
    onError: (error: Error) =>
  ...
  if (!isOpen) {
    return (
      <div className="hidden md:flex border-l bg-muted/30 flex-col items-center p-2">
        ...
        <div
          className="mt-2 writing-mode-vertical text-xs text-muted-foreground flex items-center gap-1"
          ...
          <TabsContent value="preview" className="p-3 m-0 space-y-3">
            <Collapsible open={configOpen} onOpenChange={setConfigOpen}>
              <div className="border rounded-md">
                <CollapsibleTrigger
                  className="flex items-center justify-between w-full p-3 hover-elevate text-sm"
                ...
                <CollapsibleContent>
                  <div className="px-3 pb-3 space-y-3">
                    <div>
                      ...
                      <Select
                        value={days.toString()}
                        onValueChange={(v) => setDays(Number.parseInt(v))}
                      >
                    ...
                    <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
                      <CollapsibleTrigger
                        className="flex items-center gap-1.5 text-xs text-muted-foreground hover-elevate"
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      <Checkbox
                        checked={
                          selectedVesselsForApply.size === vesselGroups.size &&
                          vesselGroups.size > 0
                        }
                  ...
                  {Array.from(vesselGroups.entries()).map(([vesselId, group]) => (
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        />
                        <CollapsibleTrigger
                          className="flex items-center justify-between flex-1 py-1.5 hover-elevate text-sm"
                          data-testid={`trigger-vessel-${vesselId}`}
                        >
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            <ChevronDown
                              className={`h-3 w-3 transition-transform ${expandedVessels.has(vesselId) ? "" : "-rotate-90"}`}
                            />
                          </div>
                        </CollapsibleTrigger>
                      </div>
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="text-[10px] text-muted-foreground mt-0.5">
                                {assignment.role} | {format(new Date(assignment.start), "MMM d")} -{" "}
                                {format(new Date(assignment.end), "MMM d")}
                              </div>
      ...
      <Dialog open={compareModalOpen} onOpenChange={setCompareModalOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>Baseline (older) vs Candidate (newer) comparison</DialogDescription>
          </DialogHeader>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="border rounded-md p-3 space-y-1">
                  ...
                  <div className="text-sm font-medium">
                    {format(new Date(comparisonData.baseline.createdAt), "MMM d, h:mm a")}
                  </div>
                  <Badge
                    className={cn(
                      "text-[10px]",
                ...
                <div className="border rounded-md p-3 space-y-1 border-primary/50">
                  ...
                  <div className="text-sm font-medium">
                    {format(new Date(comparisonData.candidate.createdAt), "MMM d, h:mm a")}
                  </div>
                  <Badge
                    className={cn(
                      "text-[10px]",
                  ...
                  <tbody>
                    <tr className="border-t">
                      ...
                      <td className="p-2 text-center text-xs">
                        {comparisonData.baseline.stats?.proposed || 0}
                      </td>
                      <td className="p-2 text-center text-xs">
                        {comparisonData.candidate.stats?.proposed || 0}
                      </td>
                      <td className="p-2 text-center">
                        <div className="flex items-center justify-center gap-1 text-xs">
                          ...
                          <span
                            className={
                              comparisonData.deltas.proposed > 0
                                ? "text-green-400"
                                : comparisonData.deltas.proposed < 0
                                  ? "text-amber-400"
                                  : ""
                          ...
                          >
                            {comparisonData.deltas.proposed > 0 ? "+" : ""}
                            {comparisonData.deltas.proposed}
                          </span>
                        </div>
                      </td>
                    </tr>
                    <tr className="border-t">
                      ...
                      <td className="p-2 text-center text-xs">
                        {comparisonData.baseline.stats?.unfilled || 0}
                      </td>
                      <td className="p-2 text-center text-xs">
                        {comparisonData.candidate.stats?.unfilled || 0}
                      </td>
                      <td className="p-2 text-center">
                        <div className="flex items-center justify-center gap-1 text-xs">
                          ...
                          <span
                            className={
                              comparisonData.deltas.unfilled < 0
                                ? "text-green-400"
                                : comparisonData.deltas.unfilled > 0
                                  ? "text-red-400"
                                  : ""
                          ...
                          >
                            {comparisonData.deltas.unfilled > 0 ? "+" : ""}
                            {comparisonData.deltas.unfilled}
                          </span>
                        </div>
                      </td>
                    </tr>
                    <tr className="border-t">
                      ...
                      <td className="p-2 text-center text-xs">
                        {comparisonData.baseline.stats?.collisions || 0}
                      </td>
                      <td className="p-2 text-center text-xs">
                        {comparisonData.candidate.stats?.collisions || 0}
                      </td>
                      <td className="p-2 text-center">
                        <div className="flex items-center justify-center gap-1 text-xs">
                          ...
                          <span
                            className={
                              comparisonData.deltas.collisions < 0
                                ? "text-green-400"
                                : comparisonData.deltas.collisions > 0
                                  ? "text-red-400"
                                  : ""
                          ...
                          >
                            {comparisonData.deltas.collisions > 0 ? "+" : ""}
                            {comparisonData.deltas.collisions}
                          </span>
                        </div>
                      </td>
                    </tr>
      ...
      <Dialog open={applyConfirmModalOpen} onOpenChange={setApplyConfirmModalOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Review the assignments before applying</DialogDescription>
          </DialogHeader>

          <div className="space-y-3">
            <div className="text-sm">You are about to apply draft assignments to:</div>
            ...
            <div className="border rounded-md p-3 bg-muted/30 space-y-1">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-medium text-green-400">
                  {selectedVesselStats.totalAssignments}
                </span>
              </div>
