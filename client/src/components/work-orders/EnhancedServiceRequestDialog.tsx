...
export function EnhancedServiceRequestDialog({
  ...
  const [showAdvanced, setShowAdvanced] = useState(() => {
    ...
    if (sessionAdvancedState !== null) {
      return sessionAdvancedState;
    }
    return false;
  });
  ...
  if (open && isEditing && initialData && !initialized) {
    if (initialData.serviceProviderId) {
      setProviderId(initialData.serviceProviderId);
    }
    if (initialData.scope) {
      setSymptomDescription(initialData.scope);
    }
    if (initialData.scheduledStartDate) {
      setRequestedStartDate(new Date(initialData.scheduledStartDate));
    }
    if (initialData.scheduledEndDate) {
      setRequestedEndDate(new Date(initialData.scheduledEndDate));
    }
    if (initialData.estimatedDurationHours) {
      setEstimatedHours(String(initialData.estimatedDurationHours));
    }
    setInitialized(true);
  }
  ...
  const updateCertificate = useCallback(
    (id: string, field: keyof CertificateItem, value: string | Date | undefined) => {
  ...
  const resetForm = useCallback(() => {
    ...
    setProbableCause("");
    setActionTakenSoFar("");
    setIsRecurringDefect(false);
    setRequestedStartDate(undefined);
    setRequestedEndDate(undefined);
    setEstimatedHours("");
    setQuotedAmount("");
    setNotes("");
  ...
  const handleSubmit = () => {
    ...
    if (showAdvanced) {
      ...
      data.certificateItems =
        showCertificates && certificateItems.length > 0
          ? certificateItems
              ...
              <div ref={advancedSectionRef} className="space-y-6 pt-4">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      <Label>Action Taken So Far</Label>
                      <Textarea
                        value={actionTakenSoFar}
                        onChange={(e) => setActionTakenSoFar(e.target.value)}
                  ...
                  <div className="flex items-center space-x-2">
                    <Switch
                      id="recurring"
                      checked={isRecurringDefect}
                      onCheckedChange={setIsRecurringDefect}
                      data-testid="switch-recurring"
                    />
                    <Label htmlFor="recurring">Recurring Defect</Label>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center space-x-2">
                    ...
                    />
                    <Label htmlFor="moc">MOC (Management of Change) Required</Label>
                  </div>
                  {mocRequired && (
                    <div>
                      ...
                      {certificateItems.map((cert) => (
                        <div key={cert.id} className="grid grid-cols-4 gap-2 items-end">
                          ...
                          <DatePickerField
                            ...
                            onChange={(d) => updateCertificate(cert.id, "expiryDate", d)}
                            testId={`cert-expiry-${cert.id}`}
                            compact
                          />
                          <div>
                            ...
                            <Input
                              value={cert.remarks}
                              onChange={(e) =>
                                updateCertificate(cert.id, "remarks", e.target.value)
                              }
                              placeholder="Remarks"
                              data-testid={`input-cert-remarks-${cert.id}`}
                            />
                          </div>
                          <Button
                            variant="ghost"
                            ...
                            data-testid={`btn-remove-cert-${cert.id}`}
                          >
