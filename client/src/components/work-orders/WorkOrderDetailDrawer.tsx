...
export function WorkOrderDetailDrawer({
  ...
  const getEquipmentName = (equipmentId: string) =>
    equipment.find((e) => e.id === equipmentId)?.name || equipmentId?.slice(0, 8) || "Unknown";
  const getEquipmentType = (equipmentId: string) =>
    equipment.find((e) => e.id === equipmentId)?.type || "Equipment";
  const getVesselName = (vesselId: string | null) =>
    !vesselId
      ? "Not assigned"
      : vessels.find((v) => v.id === vesselId)?.name || vesselId.slice(0, 8);
  const getCrewName = (crewId: string | null) =>
    !crewId ? "Unassigned" : crew.find((c) => c.id === crewId)?.name || crewId.slice(0, 8);
  const getCrewHourlyRate = (crewId: string | null) =>
    !crewId ? null : crew.find((c) => c.id === crewId)?.hourlyRate || null;
  ...
  return (
    <Sheet open={open} onOpenChange={onClose}>
      ...
      >
          ...
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 sm:gap-4">
            <div className="min-w-0 flex-1">
              ...
              <p className="text-xs sm:text-sm text-muted-foreground mt-1 truncate">
                {getEquipmentName(workOrder.equipmentId)}
              </p>
            </div>
              ...
              <div>
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div
                    ...
                    data-testid="cost-internal-labor"
                  >
          ...
          <div className="flex flex-wrap gap-2 justify-end">
            {workOrder.status !== "completed" && workOrder.status !== "cancelled" && (
              <Button
                variant="outline"
                size="sm"
                onClick={() => setLinkTemplateDialogOpen(true)}
        ...
        <LinkTemplateDialog
          workOrderId={workOrder.id}
          equipmentType={getEquipmentType(workOrder.equipmentId)}
