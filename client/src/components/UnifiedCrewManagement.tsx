...
export function UnifiedCrewManagement() {
  ...
  const [lifecycleAction, setLifecycleAction] = useState<
    "retire" | "cancel" | "reinstate" | "delete" | null
  >(null);
  ...
  const handleLifecycleAction = () => {
    if (!lifecycleCrewId || !lifecycleAction) {
      return;
    }
    ...
    switch (lifecycleAction) {
      case "retire":
        retireMutation.mutate(
          { crewId: lifecycleCrewId, notes: lifecycleNotes || undefined },
          { onSuccess: closeLifecycleDialog }
        );
        break;
      case "cancel":
        cancelMutation.mutate(
          { crewId: lifecycleCrewId, notes: lifecycleNotes || undefined, applyPenalty },
          { onSuccess: closeLifecycleDialog }
        );
        break;
      case "reinstate":
  ...
  const getLifecycleDialogTitle = () => {
    switch (lifecycleAction) {
      case "retire":
        return "Retire Crew Member";
      case "cancel":
        return "Cancel Contract";
      case "reinstate":
        return "Reinstate Crew Member";
      case "delete":
        return "Delete Former Crew Record";
      default:
        return "";
    }
  };
  ...
  const getLifecycleDialogDescription = () => {
    switch (lifecycleAction) {
      case "retire":
        return `${lifecycleCrewName} will be moved to the former crew roster.`;
      case "cancel":
        return `${lifecycleCrewName}'s contract will be terminated and they will be moved to the former crew roster.`;
      case "reinstate":
        return `${lifecycleCrewName} will be restored to the active crew roster.`;
      case "delete":
        return `This will permanently delete ${lifecycleCrewName}'s record. This action cannot be undone.`;
      default:
        return "";
    }
  };
                        ...
                        <>
                          ...
                          <TableCell className="text-muted-foreground">
                            {member.employmentPeriods?.[0]?.endDate
                              ? format(new Date(member.employmentPeriods[0].endDate), "MMM d, yyyy")
                              : "-"}
                          </TableCell>
                        </>
                      )}
                          ...
                          <DropdownMenuContent align="end">
                            {!isFormerView && (
                              <>
                                ...
                                <DropdownMenuItem
                                  onClick={() =>
                                    openLifecycleDialog("retire", member.id, member.name)
                                  }
                                ...
                                <DropdownMenuItem
                                  onClick={() =>
                                    openLifecycleDialog("cancel", member.id, member.name)
                                  }
                            ...
                            {isFormerView && (
                              <>
                                ...
                                <DropdownMenuItem
                                  onClick={() =>
                                    openLifecycleDialog("reinstate", member.id, member.name)
                                  }
                                ...
                                <DropdownMenuItem
                                  onClick={() =>
                                    openLifecycleDialog("delete", member.id, member.name)
                                  }
                                  className="text-destructive"
        ...
        footer={
          <div className="flex gap-2 w-full">
            ...
            <Button
              type="button"
              variant={
                lifecycleAction === "delete" || lifecycleAction === "cancel"
                  ? "destructive"
                  : "default"
              }
