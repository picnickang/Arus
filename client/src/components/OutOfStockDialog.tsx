...
export function OutOfStockDialog({
  ...
  const createPRMutation = useMutation({
    mutationFn: async () => {
      if (!partInfo) {
        throw new Error("Part information not loaded");
      }
  ...
  const extendCompletionDateMutation = useMutation({
    mutationFn: async () => {
      ...
      const response = await apiRequest(
        "PATCH",
        `/api/work-orders/${workOrderId}/extend-completion-date`,
        {
          additionalDays,
      ...
      <DialogContent className="sm:max-w-md">
        ...
        {isLoading ? (
          ...
          <div className="py-8 text-center text-muted-foreground">
            Part information not available
          </div>
        ) : (
          <div className="space-y-4 py-4">
                ...
                <div className="text-sm">
                  ...
                  {partInfo.estimatedLeadTimeDays > 0 && (
                    <span className="text-muted-foreground">
                      {" "}
                      - Estimated {partInfo.estimatedLeadTimeDays} day lead time
                    </span>
                  )}
                </div>
              </div>
            ...
            <div className="text-sm text-muted-foreground">
              Would you like to create a purchase request for this part? The work order completion
              date will be extended by <strong>{partInfo.estimatedLeadTimeDays} days</strong> to
