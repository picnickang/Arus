...
export const GridRow = React.memo(function GridRow({
  ...
  const violationReasons = (): string[] => {
    ...
    if (c.restTotal < 10) {
      reasons.push(`Rest ${c.restTotal}h < 10h minimum`);
    }
    if (c.minRest24 < 10) {
      reasons.push(`24h window: ${c.minRest24.toFixed(1)}h < 10h`);
    }
    if (!c.splitOK) {
      reasons.push("Rest blocks: split rule violated");
    }
    return reasons;
  };
  ...
  const dateTitle =
    !dayOK && liveCheck
      ? `⚠ ${(() => {
          const reasons: string[] = [];
          if (c.restTotal < 10) {
            reasons.push(`Only ${c.restTotal}h rest`);
          }
          if (c.minRest24 < 10) {
            reasons.push(`24h window: ${c.minRest24.toFixed(1)}h`);
          }
          if (!c.splitOK) {
            reasons.push("Block split violated");
          }
