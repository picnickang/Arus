    ...
    >
      ...
      {primaryRisk && (
        <div className="flex items-center gap-2 bg-red-950/70 border border-red-500/60 rounded-lg px-3 py-1.5 text-red-300 flex-shrink-0">
          ...
          <span className="font-medium">{primaryRisk.label}</span>
          {primaryRisk.confidence && (
            <span className="text-xs text-red-400/80">{primaryRisk.confidence}%</span>
          )}
          <div className="flex gap-1 ml-2">
            <button
              onClick={() => onAction?.("accept-risk", primaryRisk)}
              ...
              aria-label={`Accept work order for ${primaryRisk.label}`}
            >
              Accept WO
            </button>
            <button
              onClick={() => onAction?.("snooze-risk", primaryRisk)}
