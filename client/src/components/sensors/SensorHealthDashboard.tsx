...
export function SensorHealthDashboard({ equipmentId }: SensorHealthDashboardProps) {
  ...
  return (
    <QueryBoundary
      isLoading={isLoading}
      loadingVariant="card"
      error={error ? "Unable to retrieve sensor health metrics. Please try again." : null}
      errorTitle="Failed to load sensor health"
      data={metrics}
    >
      {(metrics) => {
                  ...
                  {metrics.recentAnomalies > 0 && (
                    <div className="mt-4 p-3 bg-orange-50 dark:bg-orange-950 border border-orange-200 dark:border-orange-800 rounded-md">
                      <p className="text-xs text-orange-800 dark:text-orange-200">
                        {metrics.recentAnomalies} anomal
                        {metrics.recentAnomalies === 1 ? "y" : "ies"} detected. Review sensor data
