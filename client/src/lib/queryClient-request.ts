...
interface ApiRequestInit extends ApiRequestOptions {
  method?: string;
  body?: unknown;
}

interface QueuedApiResponse {
  queuedForSync: true;
  offline: true;
  id: string;
  entityType: string;
...
function isPayloadRecord(data: unknown): data is Record<string, unknown> {
  return data !== null && typeof data === "object" && !Array.isArray(data);
}

function asPayloadRecord(data: unknown): Record<string, unknown> | undefined {
  return isPayloadRecord(data) ? data : undefined;
}
