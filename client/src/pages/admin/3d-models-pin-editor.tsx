...
function PinRow({
  ...
  const updatePinAxis = (axis: "x" | "y" | "z", value: number) => {
    if (axis === "x") {
      onUpdatePin(index, { x: value });
    } else if (axis === "y") {
      onUpdatePin(index, { y: value });
    } else {
      onUpdatePin(index, { z: value });
    }
  };
      ...
      {(["x", "y", "z"] as const).map((axis) => (
        <TableCell key={axis}>
          <Input
            ...
            step="0.01"
            value={pin[axis]}
            onChange={(e) => updatePinAxis(axis, Number(e.target.value))}
