...
function VesselEquipmentSheet({
  ...
  const deleteEquipmentMutation = useDeleteMutation("/api/equipment", {
    ...
    onError: (error) => { toast({ title: "Error", description: error instanceof Error ? error.message : "Failed to delete", variant: "destructive" }); },
  });
  ...
  return (
    <>
        ...
        <SheetContent side="right" className="w-full sm:max-w-2xl lg:max-w-4xl overflow-y-auto" data-testid="vessel-detail-sheet">
          ...
          <div className="mt-6 space-y-6">
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              <div><p className="text-sm text-muted-foreground">Vessel Class</p><p className="font-medium">{vessel.vesselClass ? formatVesselClass(vessel.vesselClass) : "Not specified"}</p></div>
              <div><p className="text-sm text-muted-foreground">Condition</p><div className="mt-1">{getConditionBadge(vessel)}</div></div>
              <div><p className="text-sm text-muted-foreground">Online Status</p><div className="mt-1">{getStatusBadge(vessel)}</div></div>
              <div><p className="text-sm text-muted-foreground">Utilization</p><p className="font-medium"><Utilization vessel={vessel} /></p></div>
              <div><p className="text-sm text-muted-foreground">Last Heartbeat</p><p className="font-medium">{vessel.lastHeartbeat ? formatDistanceToNow(new Date(vessel.lastHeartbeat), { addSuffix: true }) : "Never"}</p></div>
            </div>

            <div className="border-t pt-4">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold flex items-center gap-2" data-testid="vessel-equipment-heading">
                  <Server className="h-5 w-5" />
                  Equipment ({vesselEquipment.length})
                </h3>
                <PermissionGate resource="equipment" action="create">
                  <Button size="sm" onClick={() => setEqCreateOpen(true)} data-testid="button-add-vessel-equipment">
              ...
              {eqLoading ? (
                ...
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p>{searchQuery ? "No equipment matches your search" : "No equipment assigned to this vessel"}</p>
                </div>
              ) : (
                        ...
                        <TableRow key={eq.id} data-testid={`row-vessel-eq-${eq.id}`}>
                          <TableCell>
                            ...
                            {(eq.manufacturer || eq.model) && (
                              <div className="text-xs text-muted-foreground">{eq.manufacturer}{eq.model && ` • ${eq.model}`}</div>
                            )}
                          </TableCell>
                          <TableCell><Badge variant="outline">{eq.type || "Unknown"}</Badge></TableCell>
                          <TableCell><HealthBadge health={eq.health} /></TableCell>
                          <TableCell>
                            <Badge className={(eq.isActive ?? true) ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" : ""} variant={(eq.isActive ?? true) ? "default" : "secondary"}>
            ...
            <div className="pt-2">
              <Button variant="outline" className="w-full" asChild>
                <Link href={`/vessels/${vessel.id}`} data-testid="button-vessel-full-dashboard">View Full Dashboard</Link>
              </Button>
            </div>
          </div>
        </SheetContent>
      ...
      <EquipmentCreateDialog
        ...
        vessels={vessels}
        onSuccess={() => { setEqCreateOpen(false); refetchEquipment(); }}
      />
      {selectedEquipment && (
        <EquipmentEditDialog
          open={eqEditOpen}
          ...
          vessels={vessels}
          onSuccess={() => { setEqEditOpen(false); refetchEquipment(); }}
        />
      )}
      {selectedEquipment && (
        <SensorSetupWizard
          ...
          onSuccess={() => { setSensorWizardOpen(false); refetchEquipment(); }}
        />
      )}
      <EquipmentDecommissionDialog
        ...
        onSubmit={async (data) => {
          try {
            ...
            toast({ title: "Equipment Decommissioned", description: `${selectedEquipment?.name} has been decommissioned.` });
