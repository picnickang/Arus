...
import { AnomaliesTab, MaintenanceHistoryTab } from "./pdm-equipment-detail-tabs";
...
function OverviewTab({
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-semibold mb-4">Sensor Health Overview</h2>
        <SensorHealthDashboard equipmentId={equipmentId} />
      </div>
      <MultiSensorChart
        ...
        data-testid="chart-sensor-correlation"
      />
      <Card>
        <CardHeader>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <MetricCard
            ...
            normalizedValue={healthData?.healthScore ?? 0}
          />
          <MetricCard
            label="Failure Probability"
            value={healthData?.pFail30d ?? 0}
            unit="%"
            status={
              (healthData?.pFail30d ?? 0) > 70
                ? "critical"
                : (healthData?.pFail30d ?? 0) > 40
                  ? "warning"
                  : "normal"
            }
          />
          <MetricCard label="Equipment Type" value={equipment.type} status="normal" />
          <MetricCard
            label="Status"
            value={equipment.isActive ? "Active" : "Inactive"}
            status={equipment.isActive ? "normal" : "warning"}
          />
        </div>
      </div>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Button
              ...
              variant="default"
              size="sm"
              data-testid="button-configure-sensors"
            >
        ...
        <CardContent>
          {sensorConfigs && sensorConfigs.length > 0 ? (
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead className="w-12">
                      <Checkbox
                        checked={
                          selectedSensorIds.length === sensorConfigs.length &&
                          sensorConfigs.length > 0
                        }
                        onCheckedChange={handleSelectAll}
                        disabled={isBulkOperationDisabled}
                ...
                <TableBody>
                  {sensorConfigs.map((sensor) => (
                    <TableRow key={sensor.id} data-testid={`sensor-row-${sensor.sensorType}`}>
                      <TableCell>
                        <Checkbox
                          checked={selectedSensorIds.includes(sensor.id)}
                          onCheckedChange={(checked) =>
                            handleSelectSensor(sensor.id, checked as boolean)
                          }
                          disabled={isBulkOperationDisabled}
                          data-testid={`checkbox-sensor-${sensor.sensorType}`}
                        />
                      </TableCell>
                      <TableCell className="font-medium">{sensor.sensorType}</TableCell>
                      <TableCell>{sensor.targetUnit || sensor.unit || "N/A"}</TableCell>
                      <TableCell>
                        <StatusBadge status={sensor.enabled === false ? "offline" : "online"} />
                      </TableCell>
            ...
            <div className="text-center py-8">
              <Gauge className="h-12 w-12 mx-auto mb-4 opacity-50 text-muted-foreground" />
              <p className="text-muted-foreground">No sensors configured for this equipment.</p>
              <p className="text-sm text-muted-foreground mt-2">
                Click "Configure Sensors" to set up sensor monitoring.
              </p>
            </div>
          )}
        </CardContent>
        ...
        description={
          <div className="space-y-2">
            <p>
              ...
              {selectedSensors.length === 1 ? "sensor" : "sensors"}? This action cannot be undone.
            </p>
            <div className="mt-3 p-3 bg-muted rounded-md">
              <p className="text-sm font-medium mb-2">Sensors to be deleted:</p>
              <ul className="text-sm space-y-1">
                {selectedSensors.map((sensor) => (
                  <li key={sensor.id} className="flex items-center gap-2">
                    <span className="font-mono text-xs">{sensor.sensorType}</span>
