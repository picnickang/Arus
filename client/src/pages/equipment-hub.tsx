...
function HealthRing({ value, size = 72, stroke = 6 }: { value: number; size?: number; stroke?: number }) {
  ...
  return (
    <svg width={size} height={size} style={{ transform: "rotate(-90deg)" }} data-testid={`health-ring-${value}`}>
      ...
      <text x={size / 2} y={size / 2} textAnchor="middle" dominantBaseline="central" fill={color} fontSize={size * 0.28} fontWeight={800} style={{ transform: "rotate(90deg)", transformOrigin: "center" }}>{value}</text>
    </svg>
  );
}
...
function StatusBadge({ status }: { status: string }) {
  ...
  return <span className={`px-2 py-0.5 rounded text-[10px] font-semibold uppercase border ${variants[status] || "bg-slate-500/10 text-slate-400 border-slate-500/20"}`}>{status}</span>;
}
...
export default function EquipmentHub() {
      ...
      <IntelligenceLayout>
        <div className="min-h-screen flex items-center justify-center" data-testid="error-state">
          <div className="text-center">
            ...
            <Button variant="outline" className="mt-3" onClick={() => navigate("/equipment-intelligence")} data-testid="button-back-to-fleet">
              Back to Fleet
            </Button>
          </div>
        </div>
  ...
  return (
    <IntelligenceLayout>
      <div className="max-w-6xl mx-auto px-4 md:px-6 py-4 space-y-5" data-testid="equipment-hub-page">
        {/* Breadcrumb */}
        <nav className="flex items-center gap-1.5 text-xs text-slate-500" data-testid="equipment-breadcrumb">
          <Link href="/equipment-intelligence" className="hover:text-slate-300 transition-colors">Fleet</Link>
          ...
          <div className="flex items-center gap-4 flex-1">
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                <h1 className="text-xl font-bold text-slate-100" data-testid="equipment-name">{data.name}</h1>
                <Badge variant={riskBadgeVariant(data.risk)} className="text-[10px] uppercase" data-testid="risk-badge">{data.risk}</Badge>
              </div>
              <div className="text-xs text-slate-500">{data.vessel} · {data.type}</div>
              <div className="flex items-center gap-4 mt-2 text-xs">
                <div><span className="text-slate-600">RUL</span> <span className={`font-bold ${riskColor(data.risk)}`} data-testid="rul-value">{data.rul}d</span></div>
                <div><span className="text-slate-600">Confidence</span> <span className="font-bold text-slate-200" data-testid="confidence-value">{data.confidence}%</span></div>
                <div><span className="text-slate-600">Trend</span> <span className={`font-semibold ${data.trend === "declining" ? "text-red-400" : data.trend === "improving" ? "text-green-400" : "text-slate-400"}`}>{data.trend === "declining" ? "↘ Declining" : data.trend === "improving" ? "↗ Improving" : "→ Stable"}</span></div>
              </div>
            </div>
          </div>
        ...
        <Card className="bg-white/[0.02] border-slate-700/15" data-testid="assessment-section">
          <CardContent className="p-4">
            <div className="text-[10px] font-semibold text-slate-600 uppercase tracking-wider mb-2">AI Assessment</div>
            <p className="text-sm text-slate-300 leading-relaxed mb-3" data-testid="assessment-text">{data.assessment}</p>
            <div className={`p-3 rounded-lg border text-xs ${riskBg(data.risk)}`}>
              ...
              <span className="text-slate-400 ml-1" data-testid="recommended-action">{data.recommendedAction}</span>
            </div>
            <div className="flex gap-4 mt-3 text-[11px] text-slate-500">
              <div><span className="text-slate-600">Last Service:</span> {data.lastService || "No data"}</div>
              <div><span className="text-slate-600">Next Due:</span> {data.nextDue || "Not scheduled"}</div>
            </div>
          </CardContent>
        </Card>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3" data-testid="operational-context">
          <div className="p-3 rounded-lg bg-white/[0.02] border border-slate-700/10">
            <div className="flex items-center gap-1.5 mb-1"><Ship className="h-3.5 w-3.5 text-blue-400" /><span className="text-[10px] text-slate-600 uppercase">Vessel Status</span></div>
            <div className="text-sm font-semibold text-slate-200" data-testid="vessel-status">{data.operationalContext.vesselStatus}</div>
          </div>
          <div className="p-3 rounded-lg bg-white/[0.02] border border-slate-700/10">
            <div className="flex items-center gap-1.5 mb-1"><MapPin className="h-3.5 w-3.5 text-violet-400" /><span className="text-[10px] text-slate-600 uppercase">Next Port</span></div>
            <div className="text-sm font-semibold text-slate-200" data-testid="next-port">{data.operationalContext.nextPort || "—"}</div>
            {data.operationalContext.nextPortEta && <div className="text-[10px] text-slate-500">ETA: {data.operationalContext.nextPortEta}</div>}
          </div>
          <div className="p-3 rounded-lg bg-white/[0.02] border border-slate-700/10">
            <div className="flex items-center gap-1.5 mb-1"><Package className="h-3.5 w-3.5 text-orange-400" /><span className="text-[10px] text-slate-600 uppercase">Parts</span></div>
            <div className="text-sm font-semibold text-slate-200" data-testid="parts-availability">{data.operationalContext.partsAvailability === "unknown" ? "Check inventory" : data.operationalContext.partsAvailability.replace("_", " ")}</div>
          </div>
          <div className="p-3 rounded-lg bg-white/[0.02] border border-slate-700/10">
            <div className="flex items-center gap-1.5 mb-1"><Calendar className="h-3.5 w-3.5 text-emerald-400" /><span className="text-[10px] text-slate-600 uppercase">Maint. Window</span></div>
            <div className="text-sm font-semibold text-slate-200" data-testid="maintenance-window">{data.operationalContext.maintenanceWindow || "—"}</div>
          </div>
        </div>
        ...
        {data.signals.length > 0 && (
          <Card className="bg-white/[0.02] border-slate-700/15" data-testid="evidence-section">
            <CardHeader className="pb-2">
              <CardTitle className="text-xs text-slate-600 uppercase tracking-wider">Evidence & Signals</CardTitle>
            </CardHeader>
            <CardContent className="space-y-1.5">
              {data.signals.map((sig, i) => (
                <div key={i} className="px-3 py-2 rounded-md bg-white/[0.015] border border-slate-700/8 text-xs text-slate-400 flex items-center gap-2" data-testid={`signal-${i}`}>
                  <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${data.risk === "critical" ? "bg-red-500" : data.risk === "warning" ? "bg-yellow-500" : "bg-green-500"}`} />
                  {sig}
                </div>
              ))}
            </CardContent>
              ...
              {data.workOrders.length > 0 ? (
                <div className="space-y-1.5">
                  {data.workOrders.slice(0, 5).map((wo) => (
                    <div key={wo.id} className="flex justify-between items-center px-3 py-2 rounded-md bg-white/[0.015] border border-slate-700/8" data-testid={`work-order-${wo.id}`}>
                      <div>
              ...
              {data.serviceOrders.length > 0 ? (
                <div className="space-y-1.5">
                  {data.serviceOrders.slice(0, 5).map((so) => (
                    <div key={so.id} className="flex justify-between items-center px-3 py-2 rounded-md bg-white/[0.015] border border-slate-700/8" data-testid={`service-order-${so.id}`}>
                      <div>
                        <div className="text-xs font-medium text-slate-200">{so.title}</div>
                        <div className="text-[10px] text-slate-500">{so.vendorName || "—"}{so.eta ? ` · ETA: ${so.eta}` : ""}</div>
                      </div>
            ...
            {data.diagnosticRuns.length > 0 ? (
              <div className="space-y-1.5">
                {data.diagnosticRuns.map((diag) => (
                  <div key={diag.id} className="px-3 py-2 rounded-md bg-white/[0.015] border border-slate-700/8" data-testid={`diagnostic-run-${diag.id}`}>
                    <div className="flex justify-between items-center mb-1">
                      <span className="text-xs font-medium text-slate-200 capitalize">{diag.analysisType} Analysis</span>
                      <span className="text-[10px] text-slate-500">{diag.createdAt}</span>
                    </div>
                    {diag.summary && <p className="text-[11px] text-slate-400 leading-relaxed">{diag.summary}</p>}
                  </div>
                ))}
              </div>
            ) : (
              <p className="text-xs text-slate-600 py-3 text-center">No diagnostic history. Run an analysis above.</p>
            )}
          </CardContent>
        </Card>
                ...
                {data.activityTimeline.map((event, i) => (
                  <div key={event.id} className="flex gap-3 py-2" data-testid={`timeline-event-${i}`}>
                    <div className="flex flex-col items-center pt-0.5">
                      ...
                      {i < data.activityTimeline.length - 1 && <div className="w-px flex-1 bg-slate-700/30 mt-1" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2">
                        <span className="text-xs font-medium text-slate-200 truncate">{event.title}</span>
                        <span className="text-[10px] text-slate-600 shrink-0">{new Date(event.timestamp).toLocaleDateString()}</span>
                      </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2" data-testid="related-tools">
          ...
          <Link href={`/pdm-dashboard?equipmentId=${data.id}`}>
            <div className="p-3 rounded-lg bg-white/[0.015] border border-slate-700/10 text-xs text-slate-400 hover:bg-blue-400/5 hover:border-blue-400/12 hover:text-blue-300 transition-colors cursor-pointer flex items-center gap-2" data-testid="link-schedule">
