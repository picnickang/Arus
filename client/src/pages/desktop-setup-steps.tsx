...
function getStepDotClasses(isComplete: boolean, isCurrent: boolean): string {
  if (isComplete) {
    return "bg-primary text-primary-foreground";
  }
  if (isCurrent) {
    return "bg-primary text-primary-foreground ring-2 ring-primary/30 ring-offset-2 ring-offset-background";
  }
...
function getStepAriaLabel(
  ...
  if (isComplete) {
    return `Step ${stepNumber}: ${label} (completed)`;
  }
  if (isCurrent) {
    return `Step ${stepNumber}: ${label} (current)`;
  }
        ...
        return (
          <div key={label} className="flex items-center gap-2">
            <div className="flex items-center gap-1.5">
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium transition-colors ${getStepDotClasses(isComplete, isCurrent)}`}
                data-testid={`step-dot-${i}`}
                aria-label={getStepAriaLabel(i + 1, label, isComplete, isCurrent)}
              >
                {isComplete ? <CheckCircle2 className="h-4 w-4" /> : i + 1}
              </div>
              <span
                className={`text-xs hidden sm:inline ${isCurrent ? "text-foreground font-medium" : "text-muted-foreground"}`}
              >
                {label}
              </span>
            </div>
            {i < steps.length - 1 && (
              <div className={`w-8 h-px ${isComplete ? "bg-primary" : "bg-muted"}`} />
            )}
          </div>
        );
      })}
...
export function BackendStep({
  ...
  const initialUrl = existing.length > 0 ? existing : "http://localhost:5000";
