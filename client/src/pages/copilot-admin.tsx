...
function StatusSidebar({ config, usage, schedules }: {
  ...
  return (
    <div className="space-y-4" data-testid="status-sidebar">
      ...
      <div className="p-3 rounded-lg border bg-card">
        <div className="text-[10px] text-muted-foreground uppercase tracking-wider">Tokens Today</div>
      ...
      <div className="p-3 rounded-lg border bg-card">
        <div className="text-[10px] text-muted-foreground uppercase tracking-wider">Est. Cost (30d)</div>
        <div className="text-lg font-bold mt-0.5">
                    ...
                    <div className="flex items-center gap-2">
                      <div className="w-20 h-1.5 bg-muted rounded-full overflow-hidden">
                        <div
                          ...
                          style={{ width: `${Math.min(100, (tool.count / (usage.topTools[0]?.count || 1)) * 100)}%` }}
                        />
                      </div>
          ...
          <CardContent className="p-3">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div>
                ...
                <p className="text-lg font-bold text-green-600">{usage.approvalStats?.approved || 0}</p>
              </div>
              <div>
                <span className="text-muted-foreground">Rejected</span>
                <p className="text-lg font-bold text-red-600">{usage.approvalStats?.rejected || 0}</p>
              </div>
              <div>
                <span className="text-muted-foreground">Pending</span>
                <p className="text-lg font-bold text-yellow-600">{usage.approvalStats?.pending || 0}</p>
              </div>
              <div>
...
function SchedulesList({
  schedules, onToggle, onRun, onDelete,
}: {
  schedules: Schedule[];
  ...
  return (
    <div data-testid="schedules-list">
      <div className="flex items-center justify-between mb-3">
        ...
        <Button variant="outline" size="sm" className="text-xs" disabled data-testid="button-new-schedule">
      ...
      {schedules.length === 0 ? (
        ...
        <div className="space-y-1.5">
          {schedules.map((s) => (
            <div key={s.id} className="flex items-center justify-between p-3 rounded-lg border hover:bg-accent/30 transition-colors" data-testid={`schedule-${s.id}`}>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {s.consecutiveFailures >= 3 && (
                    <Badge variant="destructive" className="text-[9px]">Auto-disabled</Badge>
                  )}
                </div>
              ...
              <div className="flex items-center gap-1 shrink-0">
                <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => onRun(s.id)} data-testid={`button-run-${s.id}`}>
                  <Play className="h-3.5 w-3.5" />
                </Button>
                <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => onToggle(s.id, !s.enabled)} data-testid={`button-toggle-${s.id}`}>
                  {s.enabled ? <Pause className="h-3.5 w-3.5" /> : <Play className="h-3.5 w-3.5 text-green-500" />}
                </Button>
                <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => onDelete(s.id)} data-testid={`button-delete-${s.id}`}>
...
function ConfigDialog({
  open, onClose, config, availableTools,
}: {
  open: boolean;
  onClose: () => void;
  config: AgentConfig | undefined;
      ...
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
        ...
        <div className="space-y-6">
          <div className="space-y-3">
            <h3 className="text-sm font-semibold flex items-center gap-2"><Bot className="h-4 w-4" /> Model</h3>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                <Label className="text-xs">Default Model</Label>
                <Select value={merged.defaultModel || "gpt-4o-mini"} onValueChange={(v) => setFormData((p) => ({ ...p, defaultModel: v }))}>
              ...
              <div className="space-y-1">
                ...
                <Input type="number" min={1} max={50} value={merged.maxIterationsPerRun || 10} onChange={(e) => setFormData((p) => ({ ...p, maxIterationsPerRun: parseInt(e.target.value) || 10 }))} data-testid="input-max-iterations" />
              </div>
            </div>
          </div>

          <div className="space-y-3">
            <h3 className="text-sm font-semibold flex items-center gap-2"><Zap className="h-4 w-4" /> Token Budgets</h3>
            <div className="grid grid-cols-3 gap-3">
              <div className="space-y-1">
                <Label className="text-xs">Per Conversation</Label>
                <Input type="number" value={merged.maxTokensPerConversation || 50000} onChange={(e) => setFormData((p) => ({ ...p, maxTokensPerConversation: parseInt(e.target.value) || 50000 }))} data-testid="input-tokens-conv" />
              </div>
              <div className="space-y-1">
                <Label className="text-xs">Daily Limit</Label>
                <Input type="number" value={merged.dailyTokenLimit || 500000} onChange={(e) => setFormData((p) => ({ ...p, dailyTokenLimit: parseInt(e.target.value) || 500000 }))} data-testid="input-tokens-daily" />
              </div>
              <div className="space-y-1">
                <Label className="text-xs">Monthly Limit</Label>
                <Input type="number" value={merged.monthlyTokenLimit || 5000000} onChange={(e) => setFormData((p) => ({ ...p, monthlyTokenLimit: parseInt(e.target.value) || 5000000 }))} data-testid="input-tokens-monthly" />
              </div>
            </div>
          </div>

          <div className="space-y-3">
            <h3 className="text-sm font-semibold flex items-center gap-2"><Shield className="h-4 w-4" /> Permission Tier</h3>
            <Select value={merged.permissionTier || "strict"} onValueChange={(v) => setFormData((p) => ({ ...p, permissionTier: v }))}>
              <SelectTrigger data-testid="select-permission-tier"><SelectValue /></SelectTrigger>
              <SelectContent>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">Summarize older messages to reduce token usage</p>
              </div>
              <Switch checked={merged.contextCompaction !== false} onCheckedChange={(v) => setFormData((p) => ({ ...p, contextCompaction: v }))} data-testid="switch-compaction" />
            </div>
          </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">Investigate high-risk failures automatically</p>
              </div>
              <Switch checked={merged.autoTriggerEnabled === true} onCheckedChange={(v) => setFormData((p) => ({ ...p, autoTriggerEnabled: v }))} data-testid="switch-auto-trigger" />
            </div>
            {merged.autoTriggerEnabled && (
              <div className="flex items-center gap-3">
                <Label className="text-xs shrink-0">Threshold:</Label>
                <input type="range" min={0.80} max={1.0} step={0.01} value={merged.autoTriggerThreshold ?? 0.85} onChange={(e) => setFormData((p) => ({ ...p, autoTriggerThreshold: parseFloat(e.target.value) }))} className="flex-1 accent-primary" data-testid="input-threshold" />
                <span className="text-sm font-mono w-10 text-right">{((merged.autoTriggerThreshold ?? 0.85) * 100).toFixed(0)}%</span>
              </div>
            )}
          </div>

          <div className="space-y-2">
            <h3 className="text-sm font-semibold flex items-center gap-2"><MessageSquare className="h-4 w-4" /> Custom System Prompt</h3>
            <Textarea value={merged.customSystemPrompt || ""} onChange={(e) => setFormData((p) => ({ ...p, customSystemPrompt: e.target.value }))} placeholder="Organization-specific instructions..." rows={3} data-testid="input-system-prompt" />
          </div>

          {availableTools.length > 0 && (
            <div className="space-y-2">
              <h3 className="text-sm font-semibold flex items-center gap-2"><Wrench className="h-4 w-4" /> Tool Management</h3>
                ...
                {availableTools.map((tool) => {
                  ...
                  return (
                    <div key={tool.name} className="flex items-center gap-1.5 text-xs">
                      ...
                      />
                      <span className="truncate" title={tool.description}>{tool.name}</span>
                    </div>
                  );
                })}
              </div>
        ...
        <DialogFooter className="gap-2">
          <Button variant="outline" onClick={() => resetMutation.mutate()} disabled={resetMutation.isPending} data-testid="button-reset-config">
            <RotateCcw className="h-4 w-4 mr-1" /> Reset Defaults
          </Button>
          <Button variant="outline" onClick={onClose}>Cancel</Button>
          <Button onClick={() => saveMutation.mutate()} disabled={saveMutation.isPending || Object.keys(formData).length === 0} data-testid="button-save-config">
...
export default function CopilotAdminPage() {
  ...
  return (
    <div className="min-h-screen" data-testid="copilot-admin-page">
      <ConfigDialog open={configOpen} onClose={() => setConfigOpen(false)} config={config} availableTools={availableTools} />

      <div className="px-6 py-4 flex items-center justify-between border-b">
        <div className="flex items-center gap-3">
          <Bot className="h-6 w-6 text-primary" />
          <div>
            <h1 className="text-xl font-bold" data-testid="text-page-title">AI Copilot Admin</h1>
            ...
            <div className="p-4 border-t space-y-3">
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  size="sm"
                  onClick={() => {
                    if (window.confirm("Permanently delete ALL conversations, messages, tool calls, and drafts?")) {
                      apiRequest("DELETE", "/api/agent/admin/conversations").then((data: any) => {
                        queryClient.invalidateQueries({ queryKey: ["/api/agent/admin/conversations"] });
