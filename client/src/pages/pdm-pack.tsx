    ...
    <Card>
      ...
      <CardContent>
        <div className="space-y-4">
          <div>
            <Label className="text-sm font-medium">Highest Deviation</Label>
            <p
              className="text-2xl font-bold mt-1"
              data-testid={`worst-z-${title.toLowerCase().replace(" ", "-")}`}
            >
              {Number.isFinite(result.worstZ) ? result.worstZ.toFixed(2) : "—"}
            </p>
          </div>
          <div>
                ...
                <div key={feature} className="flex justify-between items-center p-2 border rounded">
                  ...
                  <div className="text-right">
                    <div className="text-sm font-medium">
                      Z: {Number.isFinite(score) ? score.toFixed(2) : "—"}
                    </div>
          ...
          {result.explanation && (
            <div>
              ...
              <div className="mt-2 p-3 bg-muted rounded text-xs font-mono max-h-96 overflow-auto">
                <pre>{JSON.stringify(result.explanation, null, 2).slice(0, 4000)}</pre>
                {JSON.stringify(result.explanation).length > 4000 && (
                  <p className="text-yellow-500 mt-2">... (truncated, showing first 4000 chars)</p>
                )}
              </div>
            </div>
          )}
                ...
                >
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <Badge
                        variant={
                          getSeverityBadgeColor(alert.severity) as
                            | "default"
                            | "secondary"
                            | "destructive"
                            | "outline"
                      ...
                      <span className="font-medium text-sm">
                        {resolved.name}
                        {resolved.vessel ? ` — ${resolved.vessel}` : ""}
                      </span>
                    </div>
                    <p className="text-xs text-muted-foreground">
                      {alert.feature}: {alert.value.toFixed(2)} (Deviation:{" "}
                      {alert.scoreZ.toFixed(1)})
                    </p>
    ...
    <div className="min-h-screen">
      <div className="space-y-6">
        <div className="flex items-center justify-end px-6 py-2">
          <Badge
            variant={p.serviceStatus ? "outline" : "destructive"}
            className={p.serviceStatus ? "border-green-500 text-green-500" : ""}
            data-testid="service-status"
          >
            {p.serviceStatus ? "Monitoring Active" : "Monitoring Issue"}
          </Badge>
        </div>
            ...
            <Card>
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-muted-foreground text-sm">Monitoring Status</p>
                    <p
                      className="text-2xl font-bold text-foreground mt-1"
                      data-testid="metric-service-status"
                    >
                      {p.serviceStatus ? "Active" : "Offline"}
                    </p>
                  </div>
                  <div
                    className={`${p.serviceStatus ? "bg-green-500/20" : "bg-red-500/20"} p-3 rounded-lg`}
                  >
                    <Activity
                      className={p.serviceStatus ? "text-green-500" : "text-red-500"}
                      size={20}
                    />
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p
                      ...
                      data-testid="metric-critical-alerts"
                    >
                      {p.criticalCount}
                    </p>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p
                      ...
                      data-testid="metric-warning-alerts"
                    >
                      {p.warningCount}
                    </p>
                  </div>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p
                      className="text-2xl font-bold text-foreground mt-1"
                      data-testid="metric-total-alerts"
                    >
                      {p.recentAlerts.length}
                    </p>
                  </div>
          ...
          <Tabs value={p.activeTab} onValueChange={p.setActiveTab} className="space-y-6">
            ...
            <TabsContent value="overview" className="space-y-6">
              <RecentAlertsPanel
                ...
                getSeverityBadgeColor={p.getSeverityBadgeColor}
              />
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground mt-1">Diagnostic Engine v1</p>
                      </div>
                      <div>
                        <Label className="text-sm font-medium">Monitored Parameters</Label>
                        <div className="flex flex-wrap gap-2 mt-2">
                          {p.healthData?.features?.map((feature: string) => (
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground mt-1">
                          {p.healthData?.timestamp ? formatDate(p.healthData.timestamp) : "Unknown"}
                        </p>
                      </div>
                    </div>
                  </CardContent>
            ...
            <TabsContent value="bearing-analysis" className="space-y-6">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <Card>
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                      ...
                      <InfoTooltip content="Paste a series of vibration measurements (like numbers from a sensor). The system will check if the vibrations are normal or indicate potential bearing problems." />
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
                    <Form {...p.bearingForm}>
                      <form
                        onSubmit={p.bearingForm.handleSubmit((data) =>
                          p.bearingAnalysisMutation.mutate(data)
                        )}
                        className="space-y-4"
                      >
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel>Vibration Series (comma-separated)</FormLabel>
                              <FormControl>
                                <Textarea
                                  {...field}
                                  placeholder="0.1, 0.2, 0.15, 0.18, 0.22, 0.19, 0.21, 0.17, 0.16, 0.20"
                                  rows={4}
                                  data-testid="input-vibration-series"
                                />
                              </FormControl>
                              <FormMessage />
                            </FormItem>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem>
                                <FormLabel>Shaft RPM (optional)</FormLabel>
                                <FormControl>
                                  <Input
                                    {...field}
                                    type="number"
                            ...
                            <FormItem className="flex items-center gap-2">
                              <FormControl>
                                <Checkbox
                                  ...
                                  data-testid="checkbox-auto-baseline"
                                />
                              </FormControl>
                              <FormLabel className="!mt-0">
                                Auto-update baseline statistics
                              </FormLabel>
                            </FormItem>
                          )}
                        />
                        <Button
                          type="submit"
                          className="w-full"
                          disabled={p.bearingAnalysisMutation.isPending}
                          data-testid="button-analyze-bearing"
                        >
                          <PlayCircle className="mr-2 h-4 w-4" />
                          {p.bearingAnalysisMutation.isPending
                            ? "Analyzing..."
                ...
                <AnalysisResultsCard
                  result={p.bearingAnalysisResult}
                  title="Bearing"
                  isLoading={p.bearingAnalysisMutation.isPending}
                />
              </div>
            </TabsContent>
            ...
            <TabsContent value="pump-analysis" className="space-y-6">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <Card>
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                      ...
                      <InfoTooltip content="Enter flow, pressure, or electrical current readings from a pump. The system will detect if the pump is operating normally or showing signs of wear." />
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
                    <Form {...p.pumpForm}>
                      <form
                        onSubmit={p.pumpForm.handleSubmit((data) =>
                          p.pumpAnalysisMutation.mutate(data)
                        )}
                        className="space-y-4"
                      >
                        <div className="grid grid-cols-2 gap-4">
                          <FormField
                            control={p.pumpForm.control}
                            name="vesselName"
                            render={({ field }) => (
                              <FormItem>
                                <FormLabel>Vessel Name</FormLabel>
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel>Flow Readings (comma-separated)</FormLabel>
                              <FormControl>
                                <Textarea
                                  {...field}
                                  placeholder="100, 102, 98, 101, 99, 100, 103, 97, 100, 101"
                                  rows={2}
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel>Pressure Readings (comma-separated)</FormLabel>
                              <FormControl>
                                <Textarea
                                  {...field}
                                  placeholder="50, 51, 49, 50, 52, 48, 50, 51, 49, 50"
                                  rows={2}
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel>Current Readings (comma-separated)</FormLabel>
                              <FormControl>
                                <Textarea
                                  {...field}
                                  placeholder="10, 10.2, 9.8, 10.1, 10, 9.9, 10.3, 10, 9.7, 10.1"
                                  rows={2}
                            ...
                            <FormItem className="flex items-center gap-2">
                              <FormControl>
                                <Checkbox
                                  checked={field.value}
                                  onCheckedChange={field.onChange}
                                  data-testid="checkbox-pump-baseline"
                                />
                              </FormControl>
                              <FormLabel className="!mt-0">
                                Auto-update baseline statistics
                        ...
                        <Button
                          type="submit"
                          className="w-full"
                          disabled={p.pumpAnalysisMutation.isPending}
                          data-testid="button-analyze-pump"
                        >
                          <PlayCircle className="mr-2 h-4 w-4" />
                          {p.pumpAnalysisMutation.isPending ? "Analyzing..." : "Run Pump Analysis"}
                        </Button>
                      </form>
                    </Form>
                  </CardContent>
                ...
                <AnalysisResultsCard
                  result={p.pumpAnalysisResult}
                  title="Pump"
                  isLoading={p.pumpAnalysisMutation.isPending}
                />
              </div>
            </TabsContent>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                    ...
                    Baseline Statistics
                    <InfoTooltip content="These are the 'normal' values learned from past measurements. The system compares new readings against these baselines to detect problems." />
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <Label htmlFor="baseline-vessel">Vessel</Label>
                        <Select value={p.selectedVessel} onValueChange={p.setSelectedVessel}>
                      ...
                      <div>
                        ...
                        <Select value={p.selectedAsset} onValueChange={p.setSelectedAsset}>
                    ...
                    {p.baselinesLoading ? (
                    ...
                    ) : p.baselines && p.baselines.length > 0 ? (
                      <div className="space-y-3 max-h-96 overflow-y-auto">
                        {p.baselines.map((baseline) => (
                      ...
                      <div className="text-center py-8">
                        <p className="text-muted-foreground">
                          No baseline data found for {p.selectedVessel} - {p.selectedAsset}
                        </p>
