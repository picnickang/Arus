...
-   **AI Copilot Agent**: Natural language chat interface (OpenAI function-calling) for fleet operations queries. 12 tools (equipment lookup, vessel overview, maintenance history, alerts, failure predictions, risk ranking, crew info, crew schedule, inventory status, draft work orders, fleet report generation). Tiered permission model: "strict" (all writes need approval — default), "balanced" (low-write auto-approved for maintenance roles, high-write needs approval), "autonomous" (admin-only, all auto-approved). Each tool has a `riskLevel` ("read", "low-write", "high-write"). Auto-approved actions create audit records with `approvalMode: "auto"`. SSE streaming, conversation persistence, and draft management. Multimodal input support (image/document file uploads via multer, base64 vision for images, text extraction for documents). Proactive suggestion engine (high-risk predictions, overdue maintenance, low stock). Scheduled autonomous runs (cron-based). Prediction Signal → Agent Automation Pipeline: `agentConfig` has `autoTriggerEnabled` (boolean, default false) and `autoTriggerThreshold` (real, default 0.85); when enabled, `SuggestionEngine.evaluateHighRiskPredictions` dispatches `AgentSignal` objects asynchronously via `setImmediate` to `AgentOrchestrator.processSignal()`, which creates a new conversation with provenance metadata (`triggerType`, `triggerId`, `signalType`, `failureProbability`) and marks the originating suggestion as acted. Safety layer (token budgets, input sanitization, rate checking, usage stats, RBAC — admin-only for config/usage/schedules/suggestions, maintenance roles for write tools). Confidence scoring on predictions/alerts with low-confidence warnings. OpenAI retry with structured error detection and exponential backoff. Frontend: floating action button + slide-out chat panel with file attachment UI. Admin page (`/copilot-admin`) with 5 tabs: Config (model selection, token budgets, permission tier selector, tool enable/disable management, automatic prediction response with enable switch and threshold slider, custom system prompt, reset to defaults), Usage (stats, approval rates, estimated cost, top tools, daily activity), Suggestions (generate/dismiss), Schedules (CRUD, manual trigger), Data (admin conversation listing, bulk purge). Config updates validated with Zod and audit-logged. Tool filtering: orchestrator respects `enabledTools` from config to restrict available tools at runtime. Deferred tool loading: two-tier tool system sends only a lightweight `listAvailableTools` meta-tool by default, loading full tool definitions on-demand when the model requests them. Tools are categorized (fleet, maintenance, alerts, predictions, crew, inventory, work-orders, analytics, files, knowledge-base). Activated tools persist in conversation metadata for reuse. Automatic fallback to full mode on refusal heuristics. Admin-configurable `deferredToolLoading` flag. Schema: 8 tables in `shared/schema/agent.ts`. Domain module: `server/domains/agent/` (hexagonal: domain/ports, application/{orchestrator,safety-service,suggestion-engine,scheduler-service}, infrastructure/repository, interfaces/routes, tools/{registry,equipment,maintenance,alert,prediction,crew,inventory,work-order,report}).
