...
-   **AI Copilot Agent**: Natural language chat interface (OpenAI function-calling) for fleet operations queries. 10 tools (equipment lookup, vessel overview, maintenance history, alerts, failure predictions, risk ranking, crew info, inventory status, draft work orders, fleet report generation). Human-in-the-loop approval for write operations. SSE streaming, conversation persistence, and draft management. Multimodal input support (image/document file uploads via multer, base64 vision for images, text extraction for documents). Proactive suggestion engine (high-risk predictions, overdue maintenance, low stock). Scheduled autonomous runs (cron-based). Safety layer (token budgets, input sanitization, rate checking, usage stats). Frontend: floating action button + slide-out chat panel with file attachment UI. Admin page (`/copilot-admin`) with 4 tabs: Config (model, token budgets, custom system prompt), Usage (stats, top tools, daily activity), Suggestions (generate/dismiss), Schedules (CRUD, manual trigger). Schema: 8 tables in `shared/schema/agent.ts`. Domain module: `server/domains/agent/` (hexagonal: domain/ports, application/{orchestrator,safety-service,suggestion-engine,scheduler-service}, infrastructure/repository, interfaces/routes, tools/{registry,equipment,maintenance,alert,prediction,crew,inventory,work-order,report}).
