...
-   **Component Decomposition**: Large feature components split into directories with `index.tsx` as the entry point and one file per subcomponent (e.g. `client/src/features/pdm/components/schedule-view/` — `ScheduleKPIStrip`, `FilterBar`, `GanttScheduleView`, `BlockedTasksSection`, `MoveTaskDialog`, `TaskDetailPanel`, `RulGauge`, `StatusComponents`, plus shared `constants.ts` and `utils.ts`). Module resolution falls through to `index.tsx`, so existing import paths remain stable.
