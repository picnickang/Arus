...
-   **Storage Architecture**: Domain-specific repositories in `server/db/`. Route-layer storage facade consumers (pdm-gap-fill, data-privacy, ml-routes) fully migrated to direct repo imports. ~29 service/background layer files still import from the frozen facade (`server/storage.ts`); guard script (`check-storage-imports.mjs`) enforces the allowlist and blocks new imports.
-   **Convergence Guardrails**: `check:guards` runs 5 scripts: schema drift validation (`validate-dual-schema.mjs`), storage import boundary (`check-storage-imports.mjs`), schema import boundary (`check-schema-imports.mjs`), cross-domain import boundary (`check-domain-boundaries.mjs`), and route registration pattern (`check-route-registration.mjs`). PdM route consolidation map at `scripts/pdm-route-map.json` with 2-phase retirement plan and client consumer counts. Redirect expiration tiers and usage tracking in `legacy-redirects.ts`. Drift burn-down at `scripts/drift-burndown.json`. `routeMigrations` deduped to 9 entries; all internal links use canonical routes.
-   **Route Registration**: All 28 route modules registered through `domain-router-registry.ts` via `legacy-router-wrappers.ts` bridge. Route-registration guardrail prevents inline `app.get/post("/api/...")` outside approved files.
-   **PdM Route Retirement**: Deprecated routes in `server/ml-routes/model-routes.ts` (Phase 1, 30d), `server/pdm/routes.ts` (Phase 2, 60d), and `server/routes/analytics/model-governance.ts` (Phase 2, 60d) serve as compatibility layers. Client hooks still reference deprecated endpoints; response shape mismatch (`sendSuccess` vs direct JSON) blocks direct URL swap. Migration tracked in `scripts/pdm-route-map.json`.
