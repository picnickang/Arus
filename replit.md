...
- **Telemetry Ingest Hardening**: sensor configurations are applied on the bulk ingest path (`server/telemetry-ingest-config.ts`, invoked by `telemetry-batch-writer.ts` before insert): gain → offset → bounds FLAG (reading kept; `arus_telemetry_ingest_config_total{outcome=below_min|above_max}` — range breaches are PdM signal, not garbage) → `enabled === false` DROP (`outcome=dropped_disabled`, warning-alerted in `ops/prometheus/rules.yaml`). Config maps are fetched per org under tenant context on a 30s TTL and fail open to pass-through, so a config-store hiccup can never halt ingestion. Natural-key conflict-skips on `(org_id, equipment_id, sensor_type, ts)` are surfaced via `arus_telemetry_natural_key_conflicts_total`: two genuine samples for one sensor in the same millisecond silently collapse to one stored row — fine at the documented 10 Hz design rate, a hazard for higher-rate sources; the escalation path is µs-resolution timestamps or a per-reading sequence component from the agent (see the dedup note in `docs/qa/load-test-results.md`).
- **Telemetry Lifecycle Jobs**: pg-boss crons (schedules + ordering rationale in `server/telemetry-cron-schedules.ts`): hourly rollups (`telemetry-rollup-hourly`, per-org under `withTenantContext`), daily partition maintenance (`telemetry-partition-maintenance-daily`, keeps monthly `equipment_telemetry` partitions 3 months ahead; table natively RANGE(ts)-partitioned by `migrations/0038`), daily retention (`telemetry-retention-daily`, partition-drop fast path + per-org batched PK-tuple deletes, gated by `TELEMETRY_RETENTION_ENABLED`).
- **Telemetry Post-Commit Fan-out**: after each batch-writer flush, the deduped latest-per-(equipment, sensor) set (a) streams to dashboards over the 250ms-throttled `telemetry_batch` WebSocket channel (producer in `server/telemetry-batch-writer.ts`; consumer `client/src/hooks/useWebSocket.ts` + `useLiveTelemetryInvalidation`), and (b) runs alert-threshold evaluation (`checkAndCreateAlerts`) per-org under tenant context — previously nothing on the ingest path evaluated thresholds, so ingested telemetry could never fire alerts.
- **Baseline-vs-Live Charts**: `GET /api/pdm/health/:equipmentId` (per-equipment health snapshot, `server/domains/pdm-platform/health/`) backs the PdM equipment page; `GET /api/telemetry/baseline/:equipmentId` computes each sensor's median ± 2σ operating envelope, rendered by `MultiSensorChart` as a band behind live series; Power-vs-STW propeller-law baseline chart mounted on the vessel-intelligence performance view. History responses stride-decimate above 1,000 points/series.
- **Route Contract Guard**: `npm run check:route-contract` (CI-enforced) extracts every client `/api` literal and matches it against the real Express route table (`scripts/dump-routes.ts` boots `registerRoutes()`); pre-existing drift is ratcheted in `scripts/route-contract-baseline.json` (128 entries after four resolution passes, fully triaged in `docs/qa/route-contract-triage.md`; per-surface retire-vs-implement recommendations awaiting owner sign-off in `docs/qa/route-contract-disposition-proposal.md`) — guards against the swagger-documented-but-never-implemented class of bug.
