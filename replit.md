...
-   **Unit Tests**: `npm run test:unit` runs ~219 tests in `tests/unit/` covering STCW compliance, prediction calibration, work order validation, shared validators, middleware smoke, domain registry, maritime converters, money utils, RUL engine, statistics/anomaly detection, and dual-schema parity. No database or network required.
