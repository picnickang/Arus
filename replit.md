...
-   **Storage Architecture**: `server/repositories.ts` is the canonical import for all data access — individual repos (`db*Storage`), domain services (`workOrderService`, `vesselService`), and the compatibility facade (`storage`). `server/storage.ts` is a retired re-export stub. Guard script blocks any new direct imports of the old storage.ts file.
-   **Convergence Guardrails**: `check:guards` runs 5 scripts: schema drift validation, storage import boundary (static + dynamic), schema import boundary, cross-domain import boundary, and route registration pattern. Drift burn-down at `scripts/drift-burndown.json` with monotonic guard (drift must never increase beyond 116). Redirects reduced to 12 entries (3 permanent + 9 route migrations). 43 automated tests in `test:dual-db`.
-   **Route Registration**: `domain-router-registry.ts` is the single route registration system. Two modes: registerFn (for function-based registration) and router-mount (for Express Router objects with mountPath). No wrapper layer — all routes import directly from source. Route-registration guardrail prevents inline `app.get/post("/api/...")` outside approved files.
