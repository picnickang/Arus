...
-   **Dynamic-Loader Map (CRITICAL for dead-code work)**: Three patterns make files appear "unused" to static analyzers (knip, ts-prune, IDE find-references) while being load-bearing at runtime. Touching them without checking these will cause silent route registration failures:
    1.  **Domain Router Registry** — `server/routes/domain-router-registry.ts` declares ~99 entries with `importPath: "../domains/<name>/index.js"` and loads them via `await import(config.importPath)`. The 97 real targets are pinned as `entry` paths in `knip.json` so knip resolves them. Adding/removing a registry entry must be matched in `knip.json`.
