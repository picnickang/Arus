...
- **Route Contract Guard**: `npm run check:route-contract` (CI-enforced) extracts every client `/api` literal and matches it against the real Express route table (`scripts/dump-routes.ts` boots `registerRoutes()`); pre-existing drift is ratcheted in `scripts/route-contract-baseline.json` (153 entries, fully triaged in `docs/qa/route-contract-triage.md`: 146 missing APIs behind live UI surfaces, 4 wrong-path, 2 synthetic cache keys, 1 dead client file) — guards against the swagger-documented-but-never-implemented class of bug.
