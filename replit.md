...
-   **Inventory & Work Orders**: Modernized UIs with virtualized tables, checklists, multi-supplier support, and out-of-stock purchase request workflows. Includes UX audit workflow fixes: Quick WO (4-field mobile creation at `/api/work-orders/quick`), WO completion with prediction feedback (`/api/work-orders/:id/complete-with-feedback`), cancellation with auto-void savings (`/api/work-orders/:id/cancel`), pending approvals banner, and role-based briefing redirect.
