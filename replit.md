...
- **Route Contract Guard**: `npm run check:route-contract` (CI-enforced) extracts every client `/api` literal and matches it against the real Express route table (`scripts/dump-routes.ts` boots `registerRoutes()`); pre-existing drift is ratcheted in `scripts/route-contract-baseline.json` (157 entries pending triage) — guards against the swagger-documented-but-never-implemented class of bug.
