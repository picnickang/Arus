...
-   **Inventory & Work Orders**: Modernized UIs with virtualized tables, checklists, multi-supplier support, and out-of-stock purchase request workflows. Includes UX audit workflow fixes: Quick WO (4-field mobile creation at `/api/work-orders/quick`), WO completion with prediction feedback (`/api/work-orders/:id/complete-with-feedback`), cancellation with auto-void savings (`/api/work-orders/:id/cancel`), pending approvals banner, and role-based briefing redirect. **Unified completion path**: All client completion calls route through `complete-with-feedback` workflow endpoint which delegates to the legacy `workOrderService.completeWorkOrder()` for stock consumption, completion record insertion, and inventory movements, then adds prediction feedback recording, savings validation, procurement cost aggregation, and domain event publishing (`WORK_ORDER_COMPLETED`, `WORK_ORDER_STATUS_CHANGED`). Legacy `POST /api/work-orders/:id/complete` route removed. Server enforces prediction feedback for predictive work orders. Both drawer and modal completion paths share the same feedback-aware completion action. ROLES config extracted to `client/src/config/roles.ts`.
...
-   **Storage Architecture**: Domain-specific repositories in `server/db/` (workorders, equipment, vessels, alerts, inventory, crew, sensors, telemetry, etc.) with a composed facade in `server/storage.ts` that delegates to repos. `server/repositories.ts` barrel file exports all repos, services, and adapters. All domain route files (`server/domains/`) now import directly from `server/repositories.ts` — zero `storage` facade usage remains in domain routes. The facade is retained only for ~50 service-layer consumers (compliance, scheduler, insights-engine, etc.). Database initialization logic lives in `server/bootstrap/services.ts`. The old `DatabaseStorage` class in `server/storage/db-storage.ts` is legacy dead code.
